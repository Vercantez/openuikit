...
- `cftest-stub-func-active.txt` (218 names)
...
`libCFTest relinked (220 stubbed)` = 218 func + 2 data. `CFStringCreateWithBytes`
...
| 9 | `NXFindBestFatArch`, `NXGetLocalArchInfo`, `getsectbynamefromheader_64`, `getsegbyname`, `dyld_image_path_containing_address`, `dlopen_preflight`, `_NSGetMachExecuteHeader`, `mach_vm_region`, `vm_purgable_control` | Mach-O / dyld / VM SPI this `libSystem` does not export. |
| 6 | `fprintf_l`, `strncasecmp_l`, `strtod_l`, `strtol_l`, `strtoll_l`, `strtoul_l` | xlocale `*_l` / `fprintf_l` not in this `libSystem`. |
...
| 1 | `__udivti3` | compiler-rt 128-bit division; not libSystem. |
