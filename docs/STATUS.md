...
Linux/arm64 under `machorun`.** Seventeen of the eighteen runnable fixtures
...
10_fat                   x     chained  PASS
                                        pass 17  fail 0  xfail 2  no-oracle 1
```
...
Rungs (j), (k) and (l) are about the userland rather than the loader: the
...
12-byte file whose `st_size` reads 471711007.

**Is not:** arbitrary macOS software. The corpus is C and minimal C++ against a
libSystem surface of 189 exports drawing 105 symbols from glibc. Objective-C does not run. C++ *exceptions* do
...
- **Slide.** `MACHORUN_NO_PREFERRED_BASE=1` forces every executable off its
  preferred `0x100000000` base. All 17 still pass, so nothing depends on
...
4. **libSystem breadth.** 189 exports covers the corpus. Everything beyond it
