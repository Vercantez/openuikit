...
| 7 | **Visual-effect rendering** | 20 / 11 | `UIVisualEffectView` / `UIBlurEffect` object and view semantics and the Canvas backdrop-filter primitive now exist. This screen still pins `LiquidGlass` off because `UIGlassEffect` and view-render integration do not; that missing integration is why the sheet platter, alerts and bar buttons remain flat. |
