...
need "$arm" "CMAKE_C_COMPILER=/tmp/swiftcore-print/shims/clang" "arm64 C is Darwin-link shim"
need "$x86" "CMAKE_C_COMPILER=/tmp/swiftcore-print/shims/clang" "x86 C is Darwin-link shim"
need "$arm" "SWIFT_NATIVE_CLANG_TOOLS_PATH=/tmp/swiftcore-print/shims" "arm64 native clang path is shims"
need "$x86" "SWIFT_NATIVE_CLANG_TOOLS_PATH=/tmp/swiftcore-print/shims" "x86 native clang path is shims"
need "$arm" "SWIFT_BUILD_RUNTIME_WITH_HOST_COMPILER=ON" "arm64 keeps host CXX (shim) against stdlib overwrite"
need "$x86" "SWIFT_BUILD_RUNTIME_WITH_HOST_COMPILER=ON" "x86 keeps host CXX (shim) against stdlib overwrite"
...
# PATH is deliberately NOT tested here: guest_arch.inc prefers the pinned
# prefixes (/opt/swift624, /opt/swift, /usr) over an ambient swiftc on PATH,
# so a host with any of those installed would make the case host-dependent.
