...
| `swiftObjectiveC` | not a CMake target on Linux | **yes** — `overlay_build_xcode_shells` compiles `overlays/ObjectiveC.swift` (6.2.4 has no `stdlib/public/Darwin/ObjectiveC`) with the Darwin overlay argv and links `libswiftObjectiveC.dylib` via the Darwin driver. |
| `_DarwinFoundation1/2/3`, `_errno` | not CMake targets on Linux | **yes** — re-export shells. Prefer sysroot `*.swiftinterface`; otherwise compile `overlays/_DarwinFoundation*.swift` / `_errno.swift` (`@_exported import` + Darwin.errno ABI in `_errno`). Shared-cache Apple dylibs have no `LC_DYLD_INFO` and are not used. |
...
| `_DarwinFoundation1/2/3`, `_errno` | in-tree shells | **built** (`overlay_build_xcode_shells`) |
| `swiftObjectiveC` | `overlays/ObjectiveC.swift` | **built** |

This cloud-agent VM is **4 vCPU / 15 GiB / no swap**. It produced the in-tree
x86_64 overlays above plus the five SDK overlays (`_DarwinFoundation1/2/3`,
`_errno`, `ObjectiveC`) from in-tree re-export shells / `ObjectiveC.swift`.
Apple dyld-cache extracts are a dead end (no `LC_DYLD_INFO`). Operator
overlays=1 on 16 vCPU:
...
(`-shared` plus `$SONAME_FLAG$SONAME` → `-Wl,-soname,libswiftX.so`, and
`CMAKE_SHARED_LIBRARY_SUFFIX=.so` so the Mach-O lands at
`lib/swift/macosx/x86_64/libswiftCore.so`). Neither that directory nor
the `.so` suffix is the object format — `stage_artifacts` ships the same
...
link prints `clangxx_darwin_link: cxx_runtime=<tbd>`. Overlay links are
...
llvm-project main) for `x86_64-apple-macos` and Darwin links
force-load `$W/build/libclang_rt.osx.a`. Each Darwin link prints
...
link diagnostic; PR #43 only printed that string when *ninja* returned 126.
`link_macho_dylib.sh` writes the same bytes to `.dylib` and copies `.so`.
A remaining ninja miss is `CANNOT_ELF_SO_LINUX_SHARED`, not an allowed gold wall.
...
`overlay_select_targets` still prints `CANNOT_STAGE_XCODE_DARWIN_OVERLAYS`
...
overlays remain the redistributable path; Apple dyld-cache copies under
`scratch/apple-x86-overlays` have no `LC_DYLD_INFO` and are not used.
...
scoreboard prints DarwinFoundation/_errno/ObjectiveC as **built** (side
recipe, `LC_DYLD_INFO` dylibs in `artifacts/swift-macosx/x86_64`); overall
rc reflects remaining real walls. This VM proves
...
against that sysroot, ninja `libswiftCore.so` as a Mach-O dylib (or
`CANNOT_ELF_SO_LINUX_SHARED`), and the five SDK overlay dylibs. Phase-2 stages those dylibs from
