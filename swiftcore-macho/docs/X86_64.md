...
| `_StringProcessing` / `Synchronization` / `_RegexParser` | fetch + explicit `-D` paths; CMake no longer sees `()` | `CANNOT_FETCH_STRING_PROCESSING_SOURCE`; `CANNOT_OVERLAY_TARGET_ABSENT` if the ninja target is missing |
| Observation | `-DSWIFT_ENABLE_EXPERIMENTAL_OBSERVATION=ON` with overlays | `CANNOT_OVERLAY_TARGET_ABSENT` if cmake did not create it |
| `swiftDarwin` | sysroot has Apple POSIX overlay headers **and** a Clang `Darwin` module. `stage_overlay_darwin.sh` prefers `scratch/sysroot_fe4-x86_64` when present; otherwise it stages Libm-2026 Intel `math.h` + CarbonHeaders-18.1 `MacTypes.h` and writes `Darwin.modulemap` over the headers Darwin.o / Synchronization / CFExecutor named (`sig_t`, `FILE`, `OSStatus`, `extern_proc`, `MAP_FAILED`, libm `f`/`l`, `os_unfair_lock_*`, `dlopen`/`dlsym`). **Do not** hand-write prototypes. | ninja failure is `OVERLAY swiftDarwin-macosx-* FAILED` (other overlays still run). |
