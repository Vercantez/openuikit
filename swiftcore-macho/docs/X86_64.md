...
**Operator 16 vCPU must confirm:** the overlay command gets **past**
`build.sh tbd` / CHECK 5 (it must not die at 96 lines on
`usr/lib-arm64-park/swift/libswiftCore.dylib`); cmake/ninja run; all five
selected overlays are attempted with scoreboard lines; produced dylibs/`*.so`
are listed; overall rc reflects any *real* compile/link wall (Darwin clang
module, gold `.so`, …). This VM proves the ninja graph, the scoreboard
harness, CHECK 5 park-skip, and `build_stdlib --print-flags` reaching cmake.
