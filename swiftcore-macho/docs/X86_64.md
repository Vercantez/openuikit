...
| `swiftDarwin` | sysroot has Apple POSIX overlay headers **and** a Clang `Darwin` module. Staging is **input-keyed** (`.overlay-sysroot-inputs`, like phase-2 `.phase2-stage-inputs`): recipe + sha of overlay-darwin `math.h` / `MacTypes.h` / modulemap recipe + machorun `sys/proc.h`. A pre-existing `$W/sdk/MacOSX.sdk` whose stamp mismatches is **displaced** (`MacOSX.sdk.stale-<utc>`), never `rm -rf`'d, and restaged into a fresh `MacOSX.sdk.overlay-<id>` directory. `stage_sdk.sh` / `configure.sh` print each required header's sha **before ninja** and refuse with `CANNOT_OVERLAY_SYSROOT_MATH_H` / `CANNOT_OVERLAY_SYSROOT_PROC_H` if `math.h` lacks `fmaxl` or `sys/proc.h` lacks `extern_proc`. The Darwin overlay compile's exact `-sdk` / `-isysroot` is printed from `ninja -t commands`. `stage_overlay_darwin.sh` prefers `scratch/sysroot_fe4-x86_64` when present, then repairs a FE `math.h` that lacks `fmaxl`. | ninja failure is `OVERLAY swiftDarwin-macosx-* FAILED first_error=…`. Synchronization's ninja graph **depends on** `libswiftDarwin.so` (`FAILED dep=swiftDarwin`). `_Concurrency` / `_StringProcessing` / `_Builtin_float` do **not**; a failure there prints their own `first_error`. |
...
overwritten; refuse-before-cmake; ninja `-sdk`/`-isysroot`; Darwin-dep vs
`first_error`), `scripts/test_darwin_overlay_syntax.sh`, `scripts/test_probe_machorun.sh`.
