...
public enum UIRenderer {
    ...
    public static func render(_ root: UIView, scale: CGFloat) -> Bitmap {
        // VENDOR-EDIT (swift-macho-linux slice): the `.layers` compositor
        // (LayerBridge.swift, quartz QZLayer tree) is not vendored in this
        // slice; the hand-written render pass is always used. It draws through
        // the same .quartz backend and matches the golden for static trees.
