...
// MARK: - The actor boundary

/// THE ONE ACTOR BOUNDARY OF THIS MODULE — every `@_cdecl` entry point in
/// EntryPoints.swift wraps its body in this, and none of them does anything
/// else about isolation.
///
/// Why it is needed: OpenUIKit's UI classes are `@MainActor`, exactly as the
/// iOS SDK's are (docs/ARCHITECTURE.md hard rule 5). A C function cannot be —
/// the C ABI has no place to put an executor, so `@_cdecl` code is nonisolated
/// by construction, and every `v.frame = …` in this module is a call from
/// nonisolated code into main-actor code.
///
/// Why `assumeIsolated` is the right crossing rather than a hop or a
/// suppression:
///
...
///     too. The facade calls in from the app's main thread, which is where the
///     `-layoutSubviews` / `-drawRect:` / target-action callbacks it exists to
///     serve already run.
///   * It is CHECKED. It traps if an ObjC caller ever dispatches an entry
///     point off-main — a bug in the caller that must not be silent, and the
///     kind of bug a hand-written bridge is otherwise very good at hiding.
///     `nonisolated(unsafe)` and an unchecked `@MainActor @_cdecl` would both
///     compile and both stay quiet.
///   * It cannot deadlock or reorder. It is a check plus a straight call — no
///     `await`, no queue hop — so an ObjC `[super layoutSubviews]` still
///     reaches the Swift implementation synchronously, which is what makes the
///     ObjC render byte-identical to the Swift one.
///
/// This is the same idiom, for the same reason, as the two crossings inside
/// OpenUIKit itself (Timer.swift, NotificationCenter.swift).
@inline(__always)
...
/// ISOLATION, and why this is the one place `nonisolated(unsafe)` earns its
/// keep. Everything else in this module reaches OpenUIKit through `oukMain`
/// (above), so the natural move is to make this table `@MainActor` too — it
/// is installed once, from the app's main thread. It was TRIED and reverted:
/// `peerOrphaned` is read from `deinit`, and a `deinit` is nonisolated by
/// language rule, so isolating the table puts a `MainActor.assumeIsolated`
/// trap inside four destructors. A destructor is the worst possible place for
/// a new trap and the best possible place for a hard-to-diagnose one.
///
/// What is actually stored is six `@convention(c)` function pointers and a
/// bool, written once before the first render and only read afterwards — the
/// case `nonisolated(unsafe)` exists for. Recorded in docs/KNOWN_GAPS.md
...
public func openuikit_clear_peer(_ h: UnsafeMutableRawPointer?) {
    ...
    p.oukPeer = nil
    // `superview` is main-actor state, so the domination check crosses the
    // boundary like everything else. Trapping off-main is correct here too:
    // this runs from the peer's `-dealloc`, and releasing the last reference
    // to a UIView off the main thread is a bug against real UIKit as well.
    guard oukMain({ () -> Bool in
        guard let v = o as? UIView, let parent = v.superview,
              let pp = parent as? OUKPeered, pp.oukPeer != nil else { return false }
        return true
    }) else { return }
