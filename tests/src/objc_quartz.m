/* objc_quartz -- the `objc_quartz` rung: the smallest program in which Objective-C draws.
 *
 * This fixture exists to be a BISECTION POINT, not a demonstration. Its sibling
 * objc_shapes is the real exercise -- inheritance, a category, a protocol,
 ...
 *
 * Three images are loaded, which is one more than any fixture before the `quartz` rung
 * needed: /usr/lib/libquartz.dylib, /usr/lib/libobjc.A.dylib and
 * /usr/lib/libSystem.B.dylib. That alone is new -- quartz proved quartz
 * under machorun with no ObjC anywhere, and objc proved ObjC under machorun
 * with no drawing anywhere. Neither proved they compose.
 *
 * Deliberately Foundation-free, exactly like objc: a root class with its
...
/* FNV-1a over the whole backing store. Same function as quartz's, and for
