...
| 4 | **TextKit attachments** | 17 | 3 | **Closed (agent/uikit-textkit).** `NSTextAttachment` + TextKit-1 `NSTextStorage` / `NSTextContainer` / `NSLayoutManager` are real objects over AttributedTextLayout. Inline image boxes follow the iOS 26.1 SE 2x measurements (docs/agent_reports/uikit-textkit.md). Ladder demand remains 12 apps / 168 uses (`NSTextAttachment` 12/168, WordPress-iOS 37). |
