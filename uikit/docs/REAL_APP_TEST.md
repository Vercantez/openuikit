...
| date | pass | note |
|---|---|---|
| 2026-09-04 | 108/108 | **iOS corner AA shoulder fit + oversized-disc bias closes `corner_radius`.** Probed rounded rects/capsules at radii 2/4/6.5/10/14/20 on SE 2x and iPhone 16 3x (`probe_corners_2x`, `probe_corners_3x`, window scenes at integer origins) and re-read tangent-row alpha + straight-edge recovery. The iOS cut now uses an explicit shoulder in the vendored quartz corner model (`rounded_or_rect`): `x(y) = x_circle(y) + shoulder*exp(-83.95*u)*(1 + 0.723*u)`, `u = y/r`, `shoulder = min(0.515*r + 0.245, 0.652*span_x)`, plus a measured oversized-disc radius bias `r_eff = r + 0.172` in the `radius > half` disc-intersection branch. Focused oracle flow: `corner_radius` **99.411 -> 99.500** (passes 99.5 bar), `collection_sections` 99.45 -> 99.691, `collection_horizontal` 99.46 -> 99.680, `collection_flow_lines` 99.45 -> 99.482, `collection_flow_grid` 99.64 -> 99.646, `collection_flow_3col_2x` 99.50 -> 99.506, `collection_dark` 99.75 -> 99.759; segmented remained non-dropping (`control_segmented` 98.631, `control_segmented_three_2x` 99.414). Fresh full iOS suite: **108/108 pass**; Catalyst gate: 119/119; real-app compares unchanged/no-drop (99.136 / 98.535 / 98.548). |
