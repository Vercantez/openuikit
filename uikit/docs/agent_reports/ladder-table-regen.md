...
not yet carry `full/ladder/target_scope.py`; `origin/main` (`a7f94380`) was
merged in so the worktree is at the current main head (`2eab84be`;
`uikit/Sources` identical to `5b2a7364`). Task: §9.6 ("Next rungs") was
...
over `uikit/Sources/OpenUIKit/**/*.swift`) run at the merged head produces
**435 names, byte-identical** to the committed
`openuikit_types-2026-09-10.txt` (`cmp` clean). Versus the 411 list it adds
24 names and removes none:
...
The committed file is kept as the list of record; the head it was verified
against is recorded in the census `_meta`
(`openuikit_source_commit = a7f94380…`, `openuikit_types_list`).
...
`uikit_sdk_types-2026-09-16.txt` (737), `openuikit_types-2026-09-10.txt`
(435), `uikit-union-2026-09-16.json`, `imports-full-2026-09-16.json`,
...
move, and only for Signal-iOS (185 → 187 implemented) and firefox-ios
(183 → 185). NetNewsWire's scoped run equals the netnewswire branch's
(0 / 0, 1,333 uses / 124 types / 429 files).
...
disagreement visible. New = this regeneration (435 list).
...
| firefox-ios | MID / 10 → MID / 10 | 6/18 → 5/8 → **3/4** | `UIMenuBuilder` 2 · `UICommandAlternate` 1 · `UITextItem` 1 | `UISwipeGestureRecognizer` (`a1c0c6b9`), `UIToolbarDelegate` and `NSCollectionLayoutAnchor` (`8427d5a0`) |
...
| WordPress-iOS | MID / 13 → MID / 13 | 9/51 → 5/26 → **5/26** | `UITextItem` 12 · `UIPopoverPresentationControllerSourceItem` 10 · `NSTextTab` 2 | `UISplitViewController` (`ca711a5e`) |
...
All eight are dated 2026-09-07 or 2026-09-09, i.e. before the §9.6 rows
were written on 2026-09-09/10 — the rows were stale on the day they were
committed, not overtaken later.
...
genuine gap is 97 uses under the 435 list, below the UIK=2 threshold of 100
(it was already 97 under the 427 list, so the committed scores JSON already
said 11; only the prose said 12). No other subscore moves.
...
uses, stub-able 40 / 143; **435 → blocking 50 / 262, stub-able 39 / 138**.
Top blocking by app-reach under 435: `NSIndexPath` 5 apps / 10 uses,
`UITextItem` 3 / 21, `NSTextLocation` 3 / 8, `NSTextLayoutManager` 3 / 7,
`UIMenuBuilder` 3 / 6, `NSToolbarItem` 2 / 85. The four names the old
paragraph led with are all declared.
...
- `full/ladder/APP_LADDER.md` §9.6: the nine table rows (same column set),
  ...
  split-view item for WordPress, the "implement UICollectionViewController
  first" for ios-oss, "123 total missing uses include 45 stubbable" for
  Telegram); each such row carries "(was: …, landed …)".
...
- firefox-ios and NetNewsWire were each run twice from scratch; the census
  JSON and the gap-classes JSON are byte-identical between runs (sha256 in
  `full/ladder/SHA256SUMS-nextrungs-2026-09-10.txt`, which also lists every
  per-app run output and the three committed combined files). A third
  NetNewsWire run with a repo-relative `--target-scope` path differs from
  the first only in `_scope.source` (the path string); its `apps` dict is
  asserted equal. The committed combined census uses the relative path.
...
- The `_union` block inside `gap-classes-nextrungs-2026-09-10.json` is what
  ...
  reflect the 435 list. It was not used; the union figures above were
...
python3 full/ladder/ladder_census.py /tmp/mini/firefox-ios \
    full/ladder/uikit_sdk_types-2026-09-16.txt full/ladder/openuikit_types-2026-09-10.txt census.json
