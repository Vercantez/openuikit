#!/bin/zsh
# ink_probe_sim.sh <outdir> — build Tools/oracle2/inkprobe for the iOS 26
# simulator, run it, and collect inkprobe.json (the Auto Layout tie-break
# oracle; see the probe's header). Same device/runtime as realapp_probe_sim.sh.
set -e
setopt null_glob
cd "$(dirname "$0")/.."
OUTDIR=${1:?usage: ink_probe_sim.sh <outdir>}
mkdir -p "$OUTDIR"
SDK=$(xcrun --show-sdk-path --sdk iphonesimulator)
APP=Tools/oracle2/InkProbe.app
...
swiftc -O -swift-version 5 -Xfrontend -default-isolation -Xfrontend MainActor -target arm64-apple-ios26.0-simulator -sdk "$SDK" \
  -module-name inkprobe Tools/oracle2/inkprobe/main.swift -o "$APP/inkprobe"
cp Tools/oracle2/InkProbe-Info.plist "$APP/Info.plist"; cp "${INK_KEYS:-/tmp/ink_keys.txt}" "$APP/keys.txt"
...
xcrun simctl uninstall "$UDID" com.openuikit.inkprobe 2>/dev/null || true
xcrun simctl install "$UDID" "$APP"
CONTAINER=$(xcrun simctl get_app_container "$UDID" com.openuikit.inkprobe data)
rm -f "$CONTAINER"/Documents/* 2>/dev/null || true
xcrun simctl launch --console-pty "$UDID" com.openuikit.inkprobe >/dev/null || true
for i in {1..600}; do [[ -f "$CONTAINER/Documents/DONE" ]] && break; sleep 1; done
[[ -f "$CONTAINER/Documents/DONE" ]] || { echo "ink_probe_sim: no DONE marker"; exit 1; }
cp "$CONTAINER"/Documents/glyph_ink_ios.json "$OUTDIR"/
echo "glyph_ink_ios.json in $OUTDIR"
