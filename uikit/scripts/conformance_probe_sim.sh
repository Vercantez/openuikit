...
#
# Compiles Sources/ConformanceApps/<app>/*.swift UNCHANGED against the iOS 26
# simulator SDK together with Tools/oracle2/confprobe/main.swift, installs the
# app on the 2x device, replays <app>/script.json on the main run loop and
# copies out, per capture time:
#
...
#
# The only generated file is the three-line glue below, which binds the
# generic probe to this app's `enum <App>App`. That convention — makeRoot(),
# perform(_:), windowSize — IS the conformance-app interface.
#
...
GLUE=$(mktemp -d /tmp/confprobe.XXXXXX)
cat > "$GLUE/Glue.swift" <<EOF
// GENERATED by scripts/conformance_probe_sim.sh — binds the generic probe to
// ${APPNAME}App's makeRoot() / perform(_:) / windowSize.
import UIKit
let confMakeRoot: @MainActor () -> UIViewController = ${APPNAME}App.makeRoot
let confPerform: @MainActor (String) -> Void = ${APPNAME}App.perform
let confWindowSize: CGSize = ${APPNAME}App.windowSize
EOF
...
swiftc -O -swift-version 5 -Xfrontend -default-isolation -Xfrontend MainActor \
  ...
  Tools/oracle2/confprobe/main.swift "$GLUE/Glue.swift" "$SRCDIR"/*.swift \
  -o "$APP/confprobe"
