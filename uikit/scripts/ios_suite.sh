...
# SKIP_CAPTURE=1 on a tree that never captured still needs the scale-patched
# scene copies openrender compares against (Linux trial: no simulator).
if [ ! -d "$WORK/scenes" ]; then
  split_scenes
fi
