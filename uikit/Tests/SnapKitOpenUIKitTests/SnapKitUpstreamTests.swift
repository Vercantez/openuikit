...
import UIKit
import OpenUIKit
// XCTest on macOS imports AppKit's NSLayoutConstraint; OpenUIKit's UIKit
// product also vends that name. SnapKit 5.7.0 Tests.swift is unmodified
// below except this typealias so the file type-checks against OpenUIKit.
...
class SnapKitTests: XCTestCase {
    ...
    func testPriorityShortcuts() {
        ...
        XCTAssertEqual(self.container.snp_constraints.count, 1, "Should have 1 constraint")
        XCTAssertEqual((self.container.snp_constraints.first as? LayoutConstraint)?.priority.rawValue, ConstraintPriority.required.value)
        
        view.snp.remakeConstraints { make in
            make.left.equalTo(1000.0).priority(.low)
        }
        XCTAssertEqual(self.container.snp_constraints.count, 1, "Should have 1 constraint")
        XCTAssertEqual((self.container.snp_constraints.first as? LayoutConstraint)?.priority.rawValue, ConstraintPriority.low.value)
        
        view.snp.remakeConstraints { make in
            make.left.equalTo(1000.0).priority(ConstraintPriority.low.value + 1)
        }
        XCTAssertEqual(self.container.snp_constraints.count, 1, "Should have 1 constraint")
        XCTAssertEqual((self.container.snp_constraints.first as? LayoutConstraint)?.priority.rawValue, ConstraintPriority.low.value + 1)
    }
