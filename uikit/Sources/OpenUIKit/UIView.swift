...
// NSObject-derived (objc-impl-chain1) so that `UIView.layoutSublayers(of:)`
// (TooltipView overrides it) stays an overridable `@objc` member on the
// Objective-C implementation route and `view.layer` reaches Objective-C. One
// shape on every route; the key-value members below override NSObject's
// where Foundation provides them.
@preconcurrency @MainActor
open class CALayer: NSObject {
    ...
    final func _setCompatibilityValue(_ value: Any?, forKey key: String) {
        switch key {
        case "isOpaque":
    ...
    final func _compatibilityValue(forKey key: String) -> Any? {
        switch key {
    ...
    /// (With Apple's Foundation these override NSObject's key-value coding
    /// entry points; corelibs Foundation on Linux has no overridable KVC on
    /// NSObject — measured swift:6.2-noble: "method does not override any
    /// method from its superclass" — and the guest ObjectiveC.NSObject has
    /// none, so those routes declare them fresh. Same bodies.)
#if canImport(ObjectiveC) && canImport(Foundation)
...
/// A conformance instead of an override point: `UIView._constraintBaselines()`
/// is `final` on the Objective-C implementation route (a tuple cannot be
/// declared in a header) and reads this hook. UILabel adopts it.
@MainActor
protocol _UIConstraintBaselineProviding: AnyObject {
    func _providedConstraintBaselines() -> (firstFromTop: CGFloat, lastFromBottom: CGFloat)?
}

/// Clip path for `_UIGlassMaterial` when the default (the layer's rounded
/// rect) is not the shape — the floating sheet's independent top/bottom
/// radii. Same reasoning as above: `Path` cannot appear in a header.
@MainActor
protocol _UIGlassPathProviding: AnyObject {
    func _providedGlassPath(in bounds: CGRect) -> Path
}

// OPENUIKIT_OBJC_IMPLEMENTATION (Darwin, set by Package.swift): UIView is
// DECLARED in Objective-C (Sources/OpenUIKitObjC/include/UIView.h) and this is
// its `@objc @implementation` (SE-0436), so an Objective-C app class can
// subclass it. Rules: (a) members not declared in the header are `final`
// (the compiler rejects anything else), (b) `superview`, `subviews` and
// `layer` are getters over `final` backing ivars because a readonly header
// property is get-only even here, (c) overridable members whose types are
// Swift-defined classes or structs live in the plain extension below as
// `@objc open`. The plain-Swift shape (native Linux, the guest route) is the
// `#else` block: ONE `open class UIView` body with the same members,
// GENERATED from this block by scripts/objc_impl_shell_check.py (Swift's
// `#if` cannot split a declaration's braces, so the copy is derived and
// byte-compared, never edited). docs/agent_reports/objc-impl-chain1.md.
#if OPENUIKIT_OBJC_IMPLEMENTATION
// objc-impl-shell: open class UIView: UIResponder
@objc @implementation extension UIView {
    ...
    /// OpenCoreGraphics' own affine transform (not CoreGraphics' on Darwin),
    /// so Swift-only.
    ...
    //
    // Readonly-with-internal-setter split (spike X3): the header declares
    // `superview` / `subviews` readonly, so the setters are the backing ivars.
    public var superview: UIView? { _superview }
    final weak var _superview: UIView?
    public var subviews: [UIView] { _subviews }
    final var _subviews: [UIView] = []
    ...
    /// Backing storage of `layer` (the public property is in the extension
    /// below, so Objective-C reaches it through the runtime).
    final lazy var _layerStorage: CALayer = {
    ...
    /// Storage of `backgroundColor` (the `open` property is in the extension below).
    final var _backgroundColor: UIColor?
    final func _setBackgroundColor(_ newValue: UIColor?) {
    ...
    @objc(hidden) public var isHidden = false
    @objc(opaque) public var isOpaque = true
    ...
    /// floating sheet provides independent top/bottom radii through
    /// `_UIGlassPathProviding`.
    final func _iosGlassPath(in bounds: CGRect) -> Path {
        if let provider = self as? _UIGlassPathProviding {
            return provider._providedGlassPath(in: bounds)
        }
    ...
    @objc(userInteractionEnabled) public var isUserInteractionEnabled = true
    public var clipsToBounds: Bool {
        get { layer.masksToBounds }
        set { layer.masksToBounds = newValue }
    }
    /// Swift enum without a raw type: Swift-only until it is an NS_ENUM.
    ...
    /// Swift OptionSet: Swift-only until it is an NS_OPTIONS.
    ...
    /// UILabel provides them through `_UIConstraintBaselineProviding`.
    final func _constraintBaselines() -> (firstFromTop: CGFloat, lastFromBottom: CGFloat)? {
        (self as? _UIConstraintBaselineProviding)?._providedConstraintBaselines()
    }
    ...
    /// Storage of `tintColor` (the `open` property is in the extension below).
    final var _tintColor: UIColor?
    final var _resolvedTintColor: UIColor {
        _tintColor ?? superview?.tintColor ?? .systemBlue
    }
    /// `traitCollection`'s value (the `open` property is in the extension
    /// below; UIWindow overrides it).
    final var _resolvedTraitCollection: UITraitCollection {
    ...
    /// UIView.h adopts NSCoding (that is what makes `initWithCoder:` a
    /// required initializer for every Swift subclass, as before); OpenUIKit
    /// does not archive views. Declared on every route so a subclass's
    /// `encode(with:)` is an override everywhere.
    ...
    public func insertSubview(_ view: UIView, belowSubview siblingSubview: UIView) {
        ...
        let index = _subviews.firstIndex(where: { $0 === siblingSubview })!
    ...
    final func _insertSubviewWithoutAdmissionCheck(_ view: UIView, at index: Int) {
        guard view !== self else { return }
        if view.superview === self {
            guard let oldIndex = _subviews.firstIndex(where: { $0 === view }) else { return }
            _subviews.remove(at: oldIndex)
            let adjustedIndex = oldIndex < index ? index - 1 : index
            _subviews.insert(view, at: Swift.max(0, Swift.min(adjustedIndex, _subviews.count)))
            setNeedsLayout()
            return
        }
        let oldWindow = view.window
        ...
        view._detachFromSuperviewWithoutCallbacks()
        view._superview = self
        _subviews.insert(view, at: Swift.max(0, Swift.min(index, _subviews.count)))
        view.setNeedsUpdateConstraints()
        setNeedsLayout()
        view.didMoveToSuperview()
        if oldWindow !== newWindow {
    ...
    private func _detachFromSuperviewWithoutCallbacks() {
        ...
        currentSuperview._subviews.removeAll { $0 === self }
        _superview = nil
    }
...
#if canImport(ObjectiveC) && !OPENUIKIT_OBJC_IMPLEMENTATION
    @objc(endEditing:)
#endif
    @discardableResult
    ...
    public func bringSubviewToFront(_ view: UIView) {
        guard let i = _subviews.firstIndex(where: { $0 === view }) else { return }
    ...
    /// Body of `userInterfaceLayoutDirection(for:relativeTo:)`.
    static func _layoutDirection(
    ...
    /// Body of `layoutSublayers(of:)`, the CALayerDelegate entry point.
    final func _layoutSublayers(of layer: CALayer) {
    ...
    /// Body of `hitTest(_:with:)`. UIKit recursion: a view is only reachable
    /// if EVERY ancestor on the path passes its own point(inside:) — subviews
    /// outside the parent's bounds are unreachable regardless of clipsToBounds
    /// (clipping is visual only, oracle-verified). Skips hidden views,
    /// alpha < 0.01 and disabled interaction (each prunes its whole subtree);
    /// subviews are tested front-to-back (reverse array order).
    final func _hitTest(_ point: CGPoint, with event: UIEvent?) -> UIView? {
    ...
    final weak var _managingViewController: UIViewController?
    ...
    public override var _firstResponderWindow: UIWindow? { window }

    // MARK: Rendering (view module: RenderPass.swift implements)

    /// Body of `drawContent(in:bounds:)`: the bridge to UIKit's app-facing
    /// drawing API — makes `canvas` the current graphics context and calls
    ...
    final func _drawContentBase(in canvas: Canvas, bounds: CGRect) {
...
@objc(OpenUIKitInternal) @implementation extension UIView {
    ...
    public var _defaultBaseLayoutMargins: UIEdgeInsets {
        UIEdgeInsets(top: 8, left: 8, bottom: 8, right: 8)
    }
}

/// Overridable members whose types are Swift-defined classes (UIColor,
/// CALayer, UIGestureRecognizer, UIEvent, Canvas) or a bridged struct
/// (UITraitCollection), or Swift `@objc` enums. They cannot be declared in
/// UIView.h — a forward-declared `@class UIColor` does not unify with the
/// Swift class (measured) — and a member of the implementation extension that
/// is not in the header must be `final`. As `@objc open` members of a plain
/// extension they are dispatched through the runtime, so the 89 Swift
/// subclasses override them unchanged and an Objective-C subclass can too;
/// the generated OpenUIKit-Swift.h carries them as a category with UIKit's
/// selectors.
extension UIView {
    /// The view's backing layer. Reaches Objective-C as a `CALayer *`.
    @objc public var layer: CALayer { _layerStorage }

    @objc open var backgroundColor: UIColor? {
        get { _backgroundColor }
        set { _setBackgroundColor(newValue) }
    }
    ...
    @objc(traitCollectionDidChange:)
    open func traitCollectionDidChange(
        _ previousTraitCollection: UITraitCollection?
    ) {}

    @objc open var traitCollection: UITraitCollection { _resolvedTraitCollection }
    ...
    @objc open var semanticContentAttribute: UISemanticContentAttribute {
    ...
    open class func userInterfaceLayoutDirection(
        ...
        _layoutDirection(for: semanticContentAttribute, relativeTo: layoutDirection)
    }
    ...
    @objc open var effectiveUserInterfaceLayoutDirection: UIUserInterfaceLayoutDirection {
    ...
    open func layoutSublayers(of layer: CALayer) { _layoutSublayers(of: layer) }
    ...
    @objc(drawContentIn:bounds:)
    open func drawContent(in canvas: Canvas, bounds: CGRect) {
        _drawContentBase(in: canvas, bounds: bounds)
    }
}
#else
...
open class UIView: UIResponder {
    ...
    /// Called after ``safeAreaInsets`` changes. Override to react; the
    /// default does nothing, like UIKit's. (Declared in the class body, not
    /// the guide extension: a non-extension method cannot be
    /// overridden.)
    open func safeAreaInsetsDidChange() {}
    ...
    /// The view's backing layer. Reaches Objective-C as a `CALayer *`.
    public var layer: CALayer { _layerStorage }
    ...
    open var traitCollection: UITraitCollection { _resolvedTraitCollection }
