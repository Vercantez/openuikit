...
// pocket-casts-ios podcasts/StorageAndDataUseViewController.swift (113 lines).
//
// The smallest of the app's settings screens, and the reason this round
// exists: its `settingsTable` outlet's `didSet` registers TWO xib cells, so
// nothing about it was reachable before Sources/OpenUIKit/UINib.swift. The
// screen's own view and table come out of
// StorageAndDataUseViewController.xib (fixtures/realapp/nibs), whose File's
// Owner is this controller.
//
// ADAPTED(objc-runtime): the `@IBOutlet` attribute and the two
// `#selector`/`@objc` lines, for the reason spelled out at the top of
// SwitchCell.swift and in docs/REAL_APP_TEST.md blocker 1.
...
class StorageAndDataUseViewController: PCViewController, UITableViewDelegate, UITableViewDataSource {
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        if indexPath.section == usageSection {
            ...
            cell.cellSwitch.addTarget(self, action: Selector.named("warnWhenNotOnWifiToggled:"), for: UIControl.Event.valueChanged)
    ...
    func warnWhenNotOnWifiToggled(_ sender: UISwitch) {
