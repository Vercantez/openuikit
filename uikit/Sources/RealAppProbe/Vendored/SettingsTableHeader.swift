...
// pocket-casts-ios podcasts/Common Components/SettingsTableHeader.swift
// (124 lines).
//
// Every section header on the storage screen is one of these, built in
// `tableView(_:viewForHeaderInSection:)`. It is the screen's only
// programmatic Auto Layout, and the only place a header's height comes from
// its own constraints rather than a delegate number, so it is the part of
// this variant that exercises `sectionHeaderHeight = automaticDimension`.
//
// ADAPTED(objc-runtime): none needed — the two `Selector` parameters are
// already values, and the header the storage screen builds passes neither.
