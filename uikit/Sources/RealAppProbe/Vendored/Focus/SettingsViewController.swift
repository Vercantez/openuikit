...
// ADAPTED(objc-runtime): `#selector` / `@objc` do not compile on native ELF
// (docs/OBJC_RUNTIME.md, docs/REAL_APP_TEST.md blocker 1). Portable spelling
// is `Selector.named` plus SelectorTables.swift; `private` is widened on the
// dispatched methods so the table can reference them. scripts/realapp_probe_sim.sh
// restores the upstream `#selector` / `@objc private` text for the Darwin
// oracle compile. Every other line is mozilla-mobile/focus-ios a2832521.
...
class SettingsViewController: UIViewController, UITableViewDataSource, UITableViewDelegate {
    ...
    private lazy var doneButton: UIBarButtonItem = {
        let doneButton = UIBarButtonItem(title: UIConstants.strings.done, style: .plain, target: self, action: Selector.named("dismissSettings"))
    ...
    override func viewDidLoad() {
        ...
        for (sectionIndex, toggleArray) in toggles {
            for (cellIndex, blockerToggle) in toggleArray {
                ...
                toggle.addTarget(self, action: Selector.named("toggleSwitched:"), for: .valueChanged)
                toggle.isOn = Settings.getToggle(blockerToggle.setting)
                toggles[sectionIndex]?[cellIndex] = blockerToggle
            }
        }

        NotificationCenter.default.addObserver(self, selector: Selector.named("applicationDidBecomeActive"), name: UIApplication.didBecomeActiveNotification, object: nil)
    }
        ...
        if let text = toggles[section]?.first?.value.subtitle {
            ...
            if section == getSectionIndex(.usageData) || section == getSectionIndex(.studies) || section == getSectionIndex(.search) {
                ...
                if section == getSectionIndex(.usageData) {
                    selector = Selector.named("tappedLearnMoreFooterWithGestureRecognizer:")
                } else if section == getSectionIndex(.search) {
                    selector = Selector.named("tappedLearnMoreSearchSuggestionsFooterWithGestureRecognizer:")
                } else if section == getSectionIndex(.studies) {
                    selector = Selector.named("tappedLearnMoreStudiesWithGestureRecognizer:")
                }
