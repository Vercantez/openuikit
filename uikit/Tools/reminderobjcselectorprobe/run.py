#!/usr/bin/env python3
"""Fail-closed unchanged-Reminder 4-to-2 Objective-C selector delta.
...
# The shared gate is loaded from the candidate checkout. Evidence runs must not
# mutate that checkout with an ignored __pycache__ side effect.
...
def main() -> int:
    ...
    if arguments.self_test:
        ...
        print("REMINDER_OBJC_SELECTOR_SELF_TEST_OK")
        return 0
    ...
    if arguments.before_output is not None or arguments.after_output is not None:
        ...
        print("diagnostics=4->2 removed=2 added=0 unchanged=2")
        print("REMINDER_OBJC_SELECTOR_CAPTURED_DELTA_OK")
        return 0
    ...
    if arguments.static_candidate:
        ...
        print("sources=22 call_sites=26 required_paths=26 app_edits=0")
        print("parser=counter-multiplicity diff_check=clean tamper_negatives=10")
        print("REMINDER_OBJC_SELECTOR_STATIC_OK")
        return 0
    ...
    with tempfile.TemporaryDirectory(prefix="reminder-objc-selector-probe-") as raw:
    ...
    print("remaining=Preview,Notification")
    print("REMINDER_OBJC_SELECTOR_DELTA_OK")
    return 0
...
if __name__ == "__main__":
    try:
        ...
        print(f"REMINDER_OBJC_SELECTOR_DELTA_FAILED: {error}", file=sys.stderr)
        raise SystemExit(1)
