...
tool_dir=${0:A:h}
udid=${1:-${OBJC_SELECTOR_SIMULATOR_UDID:-}}
if [[ -z "$udid" ]]; then
  print -u2 "usage: Tools/objcselectordispatchprobe/run.sh <booted-iOS-26.1-simulator-UDID>"
  exit 2
fi
...
scratch=$(mktemp -d /private/tmp/openuikit-objc-selector-native.XXXXXX)
trap 'rm -rf "$scratch"' EXIT
app="$scratch/ObjCSelectorDispatchProbe.app"
mkdir -p "$app"
...
xcrun --sdk iphonesimulator swiftc -O -swift-version 5 -parse-as-library \
  ...
  "$tool_dir/main.swift" -o "$app/objcselectordispatchprobe"

bundle_id=com.openuikit.objcselectordispatchprobe
...
output="$container/Documents/objcselectordispatchprobe.txt"
for _ in {1..60}; do
  [[ -f "$output" ]] && break
  sleep 0.1
done
...
xcrun simctl terminate "$udid" "$bundle_id" >/dev/null 2>&1 || true
xcrun simctl uninstall "$udid" "$bundle_id" >/dev/null 2>&1 || true
print "OBJC_SELECTOR_NATIVE_ORACLE_OK runtime=$runtime udid=$udid"
