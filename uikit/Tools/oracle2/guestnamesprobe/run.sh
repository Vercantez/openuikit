#!/bin/zsh
# guestnamesprobe -- the Apple side of Tools/guestprobes/guestnamesprobe.
#
...
trap cleanup EXIT
sim_lock_acquire
DEV=$(xcrun simctl create "iPhone 16-guestnamesprobe" "iPhone 16" com.apple.CoreSimulator.SimRuntime.iOS-26-1)
timeout 180 xcrun simctl boot "$DEV"
timeout 180 xcrun simctl bootstatus "$DEV" -b >/dev/null 2>&1
timeout 120 xcrun simctl spawn "$DEV" "$OUT/guestnamesprobe" > transcript-ios26.1.txt
