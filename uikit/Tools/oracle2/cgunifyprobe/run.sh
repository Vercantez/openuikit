#!/bin/zsh
# cgunifyprobe — Apple side of CGUnifyTests. Builds main.swift for the iOS 26.1
# simulator against Apple's UIKit, runs it in a throwaway iPhone 16 with
# `simctl spawn`, writes transcript-ios26.1.txt.
set -eu
cd "$(dirname "$0")"
SDK=$(xcrun --sdk iphonesimulator --show-sdk-path)
OUT=${OUT:-/tmp/cgunifyprobe}
mkdir -p "$OUT"
xcrun swiftc -target arm64-apple-ios26.1-simulator -sdk "$SDK" main.swift scenario/*.swift drawing/*.swift -o "$OUT/cgunifyprobe"
