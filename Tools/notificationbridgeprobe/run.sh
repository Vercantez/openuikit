...
tool_dir=${0:A:h}
udid=${1:-${NOTIFICATION_BRIDGE_SIMULATOR_UDID:-}}
if [[ -z $udid ]]; then
  print -u2 "usage: Tools/notificationbridgeprobe/run.sh <booted-iOS-26.1-simulator-UDID>"
  exit 2
fi
...
scratch=$(mktemp -d /private/tmp/openuikit-notification-native.XXXXXX)
trap 'rm -rf "$scratch"' EXIT
app=$scratch/NotificationBridgeProbe.app
mkdir -p $app
...
xcrun --sdk iphonesimulator swiftc -O -swift-version 5 -parse-as-library \
  ...
  $tool_dir/main.swift -o $app/notificationbridgeprobe

bundle_id=com.openuikit.notificationbridgeprobe
...
output=$container/Documents/notificationbridgeprobe.txt
for _ in {1..60}; do
  [[ -f $output ]] && break
  sleep 0.1
done
...
xcrun simctl terminate $udid $bundle_id >/dev/null 2>&1 || true
xcrun simctl uninstall $udid $bundle_id >/dev/null 2>&1 || true
print "NOTIFICATION_NATIVE_ORACLE_OK runtime=$runtime udid=$udid"
