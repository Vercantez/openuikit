#!/usr/bin/env python3
"""Fail-closed unchanged-Reminder delta for presentation and row-move APIs.
...
BASE_COMMIT = "0d08b8768db6b3a077192be04bcb13e18199c049"
...
ALLOWED_CHANGED_PATHS = frozenset({
    "Sources/OpenUIKit/UIAdaptivePresentation.swift",
    "Sources/OpenUIKit/UIPresentation.swift",
    "Sources/OpenUIKit/UITableView.swift",
    "Sources/OpenUIKit/UIViewController.swift",
    "Tests/OpenUIKitTests/DelegateProtocolTests.swift",
    "Tests/OpenUIKitTests/PresentationTableSourceCompatibilityTests.swift",
    "Tests/OpenUIKitTests/TableViewTests.swift",
    "Tools/presentationtableprobe/Info.plist",
    "Tools/presentationtableprobe/expected.txt",
    "Tools/presentationtableprobe/main.swift",
    "Tools/presentationtableprobe/run.sh",
    "Tools/reminderpresentationtableprobe/EntryPointShim.swift",
    "Tools/reminderpresentationtableprobe/run.py",
    "docs/APP_COMPAT.md",
    "docs/KNOWN_GAPS.md",
    "docs/ROADMAP.md",
})
# Requiring the complete allowlist prevents an evidence-only candidate from
# silently dropping source, tests, the native oracle, or the documentation.
...
def source_subject_hash(reminder: pathlib.Path) -> str:
    ...
    if len(CALL_SITE_LINES) != 23:
        fail(f"expected exact 23-call-site census, found {len(CALL_SITE_LINES)}")
...
def typecheck(reminder: pathlib.Path, uikit: pathlib.Path,
    ...
    command = [
        ...
        "-parse-as-library", "-module-name", "ReminderPresentationTableProbe",
...
def main() -> int:
    ...
    with tempfile.TemporaryDirectory(prefix="reminder-presentation-table-probe-") as raw:
        ...
        base_exit, base_output = typecheck(reminder_copy, base_copy, base_bin, shim)
    ...
    print("sources=22 call_sites=23 app_edits=0")
    print("diagnostics=12->7 removed=5 added=0 unchanged=7")
    ...
    print("tamper_negatives=missing-before,unexpected-before,missing-after,"
          "unexpected-after,source-hash,unexpected-changed-path,missing-changed-path")
    print("remaining=objc-selector,Preview,traits,text-optionality,Notification,"
          "endEditing-selector")
    print("REMINDER_PRESENTATION_TABLE_DELTA_OK")
    return 0
...
if __name__ == "__main__":
    try:
        ...
        print(f"REMINDER_PRESENTATION_TABLE_DELTA_FAILED: {error}", file=sys.stderr)
        raise SystemExit(1)
