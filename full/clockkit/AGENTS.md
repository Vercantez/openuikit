# ClockKit framework fan-out rules

This directory is an isolated clean-room starting point for the Linux `ClockKit`
port. Work only inside `full/clockkit/`. Do not edit application
...
- Implement a real Linux module named `ClockKit` and a loadable
  `libClockKit.dylib`. Put implementation Swift files in this framework
  directory, outside `tests/`, and list each one as a repo-relative path in
  `clockkit_guest_sources.txt`.
- Create `coverage.tsv` with the exact header
  ...
  At least 7 rows must be `implemented` or `declared` for the `medium-full`
...
- Create a nonempty `README.md` describing what is real, fail-closed, and still
  deferred. The primary implementation file must be `ClockKit.swift`.
- Create `tests/agent/ClockKitRuntime.swift`. It must import `ClockKit`, exercise
  meaningful implemented behavior, and print exactly `CLOCKKIT_AGENT_RUNTIME_OK` on success.
