...
/// ISO/IEC 18004 data codewords after mode/count/payload/terminator/pad,
/// before Reed-Solomon ECC. Used by the barcode tests against known values.
func ciQRDataCodewords(_ message: Data, level: String) -> [UInt8]? {
    let lv = ciQRLevel(level)
    let alphanumeric = ciQRAlphanumericBits(message)
    let bits: [UInt8]
    let isAlpha: Bool
    ...
    guard let version = ciQRPickVersion(bitCount: bits.count, level: lv, alphanumeric: isAlpha) else {
        return nil
    }
    let spec = ciQRSpec(version, lv)
    return ciQRPad(bits, dataCodewords: spec.dataCW)
}
