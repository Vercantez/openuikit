# Port `MapKit` to Linux

Build a substantial, honest starting implementation of Apple's public `MapKit`
...
The immutable seed contains 1153 unique public precise identifiers and
2222 symbol-graph relationships. The acceptance floor is
116 nondeferred (`implemented` or `declared`) identifiers: min(150, max(50, ceil(10% of exact public symbols))).
...
Deliver all of the following in `full/mapkit/`:

1. Linux Swift sources for module `MapKit` and `libMapKit.dylib`, with every
   implementation source listed in `mapkit_guest_sources.txt`.
2. Complete exact-ID `coverage.tsv` using only the five allowed statuses.
   `implemented` evidence has the exact form
   `test:full/mapkit/tests/agent/*Tests.swift#testName`; `declared` evidence has
   the exact form `source:full/mapkit/<product-source>.swift#Symbol`. Every cited
   test/function and product source/anchor must exist, and the product source
   must be listed in `mapkit_guest_sources.txt`.
3. Nonempty `MapKit.swift` and `README.md`, plus `oracle-questions.tsv` with
...
4. Top-level synchronous no-argument functions named `test*` in
   `tests/agent/*Tests.swift`, plus `tests/agent/MapKitLoadSmoke.swift` whose
   exact content is `import MapKit`, one blank line, and
   `let frameworkLoadSmokeMarker = "MAPKIT_AGENT_RUNTIME_OK"`. The sealed gate derives a runner
   from `implemented` coverage, loads `libMapKit.dylib`, and calls each cited
   test exactly once before accepting marker-only output. The load/marker check
   is not behavioral evidence by itself.
5. If dependencies are declared, `tests/agent/MapKitDependencyIdentity.swift`
