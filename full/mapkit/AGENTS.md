# MapKit framework fan-out rules

This directory is an isolated clean-room starting point for the Linux `MapKit`
port. Work only inside `full/mapkit/`. Do not edit application
...
The extractor also emitted cross-import overlay graphs for `_MapKit_SwiftUI`; those separate
...
repeat a precise identifier. `reference/public-surface.tsv` contains
...
- Implement a real Linux module named `MapKit` and a loadable
  `libMapKit.dylib`. Put implementation Swift files in this framework
  directory, outside `tests/`, and list each one as a repo-relative path in
  `mapkit_guest_sources.txt`.
- Create `coverage.tsv` with the exact header
  ...
  At least 116 rows must be `implemented` or `declared` for the `large-partitioned`
  ...
  `test:full/mapkit/tests/agent/*Tests.swift#testName`; define that exact `test*`
  function for the sealed runner to call. Every `declared` row must cite
  `source:full/mapkit/<product-source>.swift#Symbol`; the source must be listed in
  `mapkit_guest_sources.txt` and contain the exact identifier anchor. Deferred,
...
- Create a nonempty `README.md` describing what is real, fail-closed, and still
  deferred. The primary implementation file must be `MapKit.swift`.
- Put focused behavioral checks in `tests/agent/*Tests.swift` as top-level,
  synchronous, no-argument functions named `test*`. Create
  `tests/agent/MapKitLoadSmoke.swift` with exactly these three logical lines
  (including the blank line): `import MapKit`, a blank line, and
  `let frameworkLoadSmokeMarker = "MAPKIT_AGENT_RUNTIME_OK"`. The sealed gate generates the
...
- When dependencies are listed below, create
  `tests/agent/MapKitDependencyIdentity.swift`. Import `MapKit` and every
  declared dependency and pass genuine dependency values through public
  `MapKit` APIs. This probe is for the clean EC2 integration build; the isolated
