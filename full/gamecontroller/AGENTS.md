# GameController framework fan-out rules

This directory is an isolated clean-room starting point for the Linux `GameController`
port. Work only inside `full/gamecontroller/`. Do not edit application
...
- Implement a real Linux module named `GameController` and a loadable
  `libGameController.dylib`. Put implementation Swift files in this framework
  directory, outside `tests/`, and list each one as a repo-relative path in
  `gamecontroller_guest_sources.txt`.
- Create `coverage.tsv` with the exact header
  ...
  At least 127 rows must be `implemented` or `declared` for the `large-partitioned`
...
- Create a nonempty `README.md` describing what is real, fail-closed, and still
  deferred. The primary implementation file must be `GameController.swift`.
- Create `tests/agent/GameControllerRuntime.swift`. It must import `GameController`, exercise
  meaningful implemented behavior, and print exactly `GAMECONTROLLER_AGENT_RUNTIME_OK` on success.
