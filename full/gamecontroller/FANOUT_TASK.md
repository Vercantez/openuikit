# Port `GameController` to Linux

Build a substantial, honest starting implementation of Apple's public `GameController`
...
The immutable seed contains 1268 unique public precise identifiers and
1476 symbol-graph relationships. The acceptance floor is
127 nondeferred (`implemented` or `declared`) identifiers: min(150, max(50, ceil(10% of exact public symbols))).
...
Deliver all of the following in `full/gamecontroller/`:

1. Linux Swift sources for module `GameController` and `libGameController.dylib`, with every
   implementation source listed in `gamecontroller_guest_sources.txt`.
...
3. Nonempty `GameController.swift` and `README.md`, plus `oracle-questions.tsv` with
   header `precise	question	risk	reason` and at least one question; use
   `module` only for cross-cutting items.
4. `tests/agent/GameControllerRuntime.swift`, meaningful focused tests, and the exact
   success marker `GAMECONTROLLER_AGENT_RUNTIME_OK`.
