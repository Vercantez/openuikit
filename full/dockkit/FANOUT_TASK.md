# Port `DockKit` to Linux

Build a substantial, honest starting implementation of Apple's public `DockKit`
...
The immutable seed contains 349 unique public precise identifiers and
446 symbol-graph relationships. The acceptance floor is
175 nondeferred (`implemented` or `declared`) identifiers: ceil(50% of exact public symbols).
...
Deliver all of the following in `full/dockkit/`:

1. Linux Swift sources for module `DockKit` and `libDockKit.dylib`, with every
   implementation source listed in `dockkit_guest_sources.txt`.
2. Complete exact-ID `coverage.tsv` using only the five allowed statuses.
   `implemented` evidence has the exact form
   `test:full/dockkit/tests/agent/*Tests.swift#testName`; `declared` evidence has
   the exact form `source:full/dockkit/<product-source>.swift#Symbol`. Every cited
   test/function and product source/anchor must exist, and the product source
   must be listed in `dockkit_guest_sources.txt`.
3. Nonempty `DockKit.swift` and `README.md`, plus `oracle-questions.tsv` with
...
4. Top-level synchronous no-argument functions named `test*` in
   `tests/agent/*Tests.swift`, plus `tests/agent/DockKitLoadSmoke.swift` whose
   exact content is `import DockKit`, one blank line, and
   `let frameworkLoadSmokeMarker = "DOCKKIT_AGENT_RUNTIME_OK"`. The sealed gate derives a runner
   from `implemented` coverage, loads `libDockKit.dylib`, and calls each cited
   test exactly once before accepting marker-only output. The load/marker check
   is not behavioral evidence by itself.
5. If dependencies are declared, `tests/agent/DockKitDependencyIdentity.swift`
