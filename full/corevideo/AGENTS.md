# CoreVideo framework fan-out rules

This directory is an isolated clean-room starting point for the Linux `CoreVideo`
port. Work only inside `full/corevideo/`. Do not edit application
...
- Implement a real Linux module named `CoreVideo` and a loadable
  `libCoreVideo.dylib`. Put implementation Swift files in this framework
  directory, outside `tests/`, and list each one as a repo-relative path in
  `corevideo_guest_sources.txt`.
- Create `coverage.tsv` with the exact header
  ...
  At least 98 rows must be `implemented` or `declared` for the `large-partitioned`
...
- Create a nonempty `README.md` describing what is real, fail-closed, and still
  deferred. The primary implementation file must be `CoreVideo.swift`.
- Create `tests/agent/CoreVideoRuntime.swift`. It must import `CoreVideo`, exercise
  meaningful implemented behavior, and print exactly `COREVIDEO_AGENT_RUNTIME_OK` on success.
