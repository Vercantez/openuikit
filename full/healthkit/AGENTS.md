# HealthKit framework fan-out rules

This directory is an isolated clean-room starting point for the Linux `HealthKit`
port. Work only inside `full/healthkit/`. Do not edit application
...
- Implement a real Linux module named `HealthKit` and a loadable
  `libHealthKit.dylib`. Put implementation Swift files in this framework
  directory, outside `tests/`, and list each one as a repo-relative path in
  `healthkit_guest_sources.txt`.
...
- Create a nonempty `README.md` describing what is real, fail-closed, and still
  deferred. The primary implementation file must be `HealthKit.swift`.
- Create `tests/agent/HealthKitRuntime.swift`. It must import `HealthKit`, exercise
  meaningful implemented behavior, and print exactly `HEALTHKIT_AGENT_RUNTIME_OK` on success.
