# CFNetwork framework fan-out rules

This directory is an isolated clean-room starting point for the Linux `CFNetwork`
port. Work only inside `full/cfnetwork/`. Do not edit application
...
- Implement a real Linux module named `CFNetwork` and a loadable
  `libCFNetwork.dylib`. Put implementation Swift files in this framework
  directory, outside `tests/`, and list each one as a repo-relative path in
  `cfnetwork_guest_sources.txt`.
...
- Create a nonempty `README.md` describing what is real, fail-closed, and still
  deferred. The primary implementation file must be `CFNetwork.swift`.
- Create `tests/agent/CFNetworkRuntime.swift`. It must import `CFNetwork`, exercise
  meaningful implemented behavior, and print exactly `CFNETWORK_AGENT_RUNTIME_OK` on success.
