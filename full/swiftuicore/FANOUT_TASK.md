# Port `SwiftUICore` to Linux

Build a substantial, honest starting implementation of Apple's public `SwiftUICore`
...
The immutable seed contains 16770 unique public precise identifiers and
18265 symbol-graph relationships. The acceptance floor is
...
Deliver all of the following in `full/swiftuicore/`:

1. Linux Swift sources for module `SwiftUICore` and `libSwiftUICore.dylib`, with every
   implementation source listed in `swiftuicore_guest_sources.txt`.
...
3. Nonempty `SwiftUICore.swift` and `README.md`, plus `oracle-questions.tsv` with
   header `precise	question	risk	reason` and at least one question; use
   `module` only for cross-cutting items.
4. `tests/agent/SwiftUICoreRuntime.swift`, meaningful focused tests, and the exact
   success marker `SWIFTUICORE_AGENT_RUNTIME_OK`.
