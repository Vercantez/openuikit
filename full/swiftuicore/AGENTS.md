# SwiftUICore framework fan-out rules

This directory is an isolated clean-room starting point for the Linux `SwiftUICore`
port. Work only inside `full/swiftuicore/`. Do not edit application
...
- Implement a real Linux module named `SwiftUICore` and a loadable
  `libSwiftUICore.dylib`. Put implementation Swift files in this framework
  directory, outside `tests/`, and list each one as a repo-relative path in
  `swiftuicore_guest_sources.txt`.
...
- Create a nonempty `README.md` describing what is real, fail-closed, and still
  deferred. The primary implementation file must be `SwiftUICore.swift`.
- Create `tests/agent/SwiftUICoreRuntime.swift`. It must import `SwiftUICore`, exercise
  meaningful implemented behavior, and print exactly `SWIFTUICORE_AGENT_RUNTIME_OK` on success.
