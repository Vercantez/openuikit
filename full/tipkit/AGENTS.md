# TipKit framework fan-out rules

This directory is an isolated clean-room starting point for the Linux `TipKit`
port. Work only inside `full/tipkit/`. Do not edit application
...
- Implement a real Linux module named `TipKit` and a loadable
  `libTipKit.dylib`. Put implementation Swift files in this framework
  directory, outside `tests/`, and list each one as a repo-relative path in
  `tipkit_guest_sources.txt`.
- Create `coverage.tsv` with the exact header
  ...
  At least 104 rows must be `implemented` or `declared` for the `large-partitioned`
...
- Create a nonempty `README.md` describing what is real, fail-closed, and still
  deferred. The primary implementation file must be `TipKit.swift`.
- Create `tests/agent/TipKitRuntime.swift`. It must import `TipKit`, exercise
  meaningful implemented behavior, and print exactly `TIPKIT_AGENT_RUNTIME_OK` on success.
