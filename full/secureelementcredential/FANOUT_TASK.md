# Port `SecureElementCredential` to Linux

Build a substantial, honest starting implementation of Apple's public `SecureElementCredential`
...
The immutable seed contains 155 unique public precise identifiers and
206 symbol-graph relationships. The acceptance floor is
78 nondeferred (`implemented` or `declared`) identifiers: ceil(50% of exact public symbols).
...
Deliver all of the following in `full/secureelementcredential/`:

1. Linux Swift sources for module `SecureElementCredential` and `libSecureElementCredential.dylib`, with every
   implementation source listed in `secureelementcredential_guest_sources.txt`.
2. Complete exact-ID `coverage.tsv` using only the five allowed statuses.
   `implemented` evidence has the exact form
   `test:full/secureelementcredential/tests/agent/*Tests.swift#testName`; `declared` evidence has
   the exact form `source:full/secureelementcredential/<product-source>.swift#Symbol`. Every cited
   test/function and product source/anchor must exist, and the product source
   must be listed in `secureelementcredential_guest_sources.txt`.
3. Nonempty `SecureElementCredential.swift` and `README.md`, plus `oracle-questions.tsv` with
...
4. Top-level synchronous no-argument functions named `test*` in
   `tests/agent/*Tests.swift`, plus `tests/agent/SecureElementCredentialLoadSmoke.swift` whose
   exact content is `import SecureElementCredential`, one blank line, and
   `let frameworkLoadSmokeMarker = "SECUREELEMENTCREDENTIAL_AGENT_RUNTIME_OK"`. The sealed gate derives a runner
   from `implemented` coverage, loads `libSecureElementCredential.dylib`, and calls each cited
   test exactly once before accepting marker-only output. The load/marker check
   is not behavioral evidence by itself.
5. If dependencies are declared, `tests/agent/SecureElementCredentialDependencyIdentity.swift`
