# SecureElementCredential framework fan-out rules

This directory is an isolated clean-room starting point for the Linux `SecureElementCredential`
port. Work only inside `full/secureelementcredential/`. Do not edit application
...
- Implement a real Linux module named `SecureElementCredential` and a loadable
  `libSecureElementCredential.dylib`. Put implementation Swift files in this framework
  directory, outside `tests/`, and list each one as a repo-relative path in
  `secureelementcredential_guest_sources.txt`.
- Create `coverage.tsv` with the exact header
  ...
  At least 78 rows must be `implemented` or `declared` for the `medium-full`
  ...
  `test:full/secureelementcredential/tests/agent/*Tests.swift#testName`; define that exact `test*`
  function for the sealed runner to call. Every `declared` row must cite
  `source:full/secureelementcredential/<product-source>.swift#Symbol`; the source must be listed in
  `secureelementcredential_guest_sources.txt` and contain the exact identifier anchor. Deferred,
...
- Create a nonempty `README.md` describing what is real, fail-closed, and still
  deferred. The primary implementation file must be `SecureElementCredential.swift`.
- Put focused behavioral checks in `tests/agent/*Tests.swift` as top-level,
  synchronous, no-argument functions named `test*`. Create
  `tests/agent/SecureElementCredentialLoadSmoke.swift` with exactly these three logical lines
  (including the blank line): `import SecureElementCredential`, a blank line, and
  `let frameworkLoadSmokeMarker = "SECUREELEMENTCREDENTIAL_AGENT_RUNTIME_OK"`. The sealed gate generates the
...
- When dependencies are listed below, create
  `tests/agent/SecureElementCredentialDependencyIdentity.swift`. Import `SecureElementCredential` and every
  declared dependency and pass genuine dependency values through public
  `SecureElementCredential` APIs. This probe is for the clean EC2 integration build; the isolated
