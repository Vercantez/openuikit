# Port `ManagedApp` to Linux

Build a substantial, honest starting implementation of Apple's public `ManagedApp`
...
The immutable seed contains 47 unique public precise identifiers and
61 symbol-graph relationships. The acceptance floor is
38 nondeferred (`implemented` or `declared`) identifiers: ceil(80% of exact public symbols).
...
Deliver all of the following in `full/managedapp/`:

1. Linux Swift sources for module `ManagedApp` and `libManagedApp.dylib`, with every
   implementation source listed in `managedapp_guest_sources.txt`.
2. Complete exact-ID `coverage.tsv` using only the five allowed statuses.
   `implemented` evidence has the exact form
   `test:full/managedapp/tests/agent/*Tests.swift#testName`; `declared` evidence has
   the exact form `source:full/managedapp/<product-source>.swift#Symbol`. Every cited
   test/function and product source/anchor must exist, and the product source
   must be listed in `managedapp_guest_sources.txt`.
3. Nonempty `ManagedApp.swift` and `README.md`, plus `oracle-questions.tsv` with
...
4. Top-level synchronous no-argument functions named `test*` in
   `tests/agent/*Tests.swift`, plus `tests/agent/ManagedAppLoadSmoke.swift` whose
   exact content is `import ManagedApp`, one blank line, and
   `let frameworkLoadSmokeMarker = "MANAGEDAPP_AGENT_RUNTIME_OK"`. The sealed gate derives a runner
   from `implemented` coverage, loads `libManagedApp.dylib`, and calls each cited
   test exactly once before accepting marker-only output. The load/marker check
   is not behavioral evidence by itself.
5. If dependencies are declared, `tests/agent/ManagedAppDependencyIdentity.swift`
