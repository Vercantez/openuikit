# Port `MultipeerConnectivity` to Linux

Build a substantial, honest starting implementation of Apple's public `MultipeerConnectivity`
...
The immutable seed contains 132 unique public precise identifiers and
209 symbol-graph relationships. The acceptance floor is
106 nondeferred (`implemented` or `declared`) identifiers: ceil(80% of exact public symbols).
...
Deliver all of the following in `full/multipeerconnectivity/`:

1. Linux Swift sources for module `MultipeerConnectivity` and `libMultipeerConnectivity.dylib`, with every
   implementation source listed in `multipeerconnectivity_guest_sources.txt`.
...
3. Nonempty `MultipeerConnectivity.swift` and `README.md`, plus `oracle-questions.tsv` with
   header `precise	question	risk	reason` and at least one question; use
   `module` only for cross-cutting items.
4. `tests/agent/MultipeerConnectivityRuntime.swift`, meaningful focused tests, and the exact
   success marker `MULTIPEERCONNECTIVITY_AGENT_RUNTIME_OK`.
