# MultipeerConnectivity framework fan-out rules

This directory is an isolated clean-room starting point for the Linux `MultipeerConnectivity`
port. Work only inside `full/multipeerconnectivity/`. Do not edit application
...
- Implement a real Linux module named `MultipeerConnectivity` and a loadable
  `libMultipeerConnectivity.dylib`. Put implementation Swift files in this framework
  directory, outside `tests/`, and list each one as a repo-relative path in
  `multipeerconnectivity_guest_sources.txt`.
- Create `coverage.tsv` with the exact header
  ...
  At least 106 rows must be `implemented` or `declared` for the `leaf-full`
...
- Create a nonempty `README.md` describing what is real, fail-closed, and still
  deferred. The primary implementation file must be `MultipeerConnectivity.swift`.
- Create `tests/agent/MultipeerConnectivityRuntime.swift`. It must import `MultipeerConnectivity`, exercise
  meaningful implemented behavior, and print exactly `MULTIPEERCONNECTIVITY_AGENT_RUNTIME_OK` on success.
