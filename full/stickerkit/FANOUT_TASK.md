# Port `StickerKit` to Linux

Build a substantial, honest starting implementation of Apple's public `StickerKit`
...
Deliver all of the following in `full/stickerkit/`:

1. Linux Swift sources for module `StickerKit` and `libStickerKit.dylib`, with every
   implementation source listed in `stickerkit_guest_sources.txt`.
2. Complete exact-ID `coverage.tsv` using only the five allowed statuses.
   `implemented` evidence has the exact form
   `test:full/stickerkit/tests/agent/*Tests.swift#testName`; `declared` evidence has
   the exact form `source:full/stickerkit/<product-source>.swift#Symbol`. Every cited
   test/function and product source/anchor must exist, and the product source
   must be listed in `stickerkit_guest_sources.txt`.
3. Nonempty `StickerKit.swift` and `README.md`, plus `oracle-questions.tsv` with
...
4. Top-level synchronous no-argument functions named `test*` in
   `tests/agent/*Tests.swift`, plus `tests/agent/StickerKitLoadSmoke.swift` whose
   exact content is `import StickerKit`, one blank line, and
   `let frameworkLoadSmokeMarker = "STICKERKIT_AGENT_RUNTIME_OK"`. The sealed gate derives a runner
   from `implemented` coverage, loads `libStickerKit.dylib`, and calls each cited
   test exactly once before accepting marker-only output. The load/marker check
   is not behavioral evidence by itself.
5. If dependencies are declared, `tests/agent/StickerKitDependencyIdentity.swift`
