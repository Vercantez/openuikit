# Port `GameKit` to Linux

Build a substantial, honest starting implementation of Apple's public `GameKit`
...
The immutable seed contains 919 unique public precise identifiers and
1460 symbol-graph relationships. The acceptance floor is
460 nondeferred (`implemented` or `declared`) identifiers: ceil(50% of exact public symbols).
...
Deliver all of the following in `full/gamekit/`:

1. Linux Swift sources for module `GameKit` and `libGameKit.dylib`, with every
   implementation source listed in `gamekit_guest_sources.txt`.
2. Complete exact-ID `coverage.tsv` using only the five allowed statuses.
   `implemented` evidence has the exact form
   `test:full/gamekit/tests/agent/*Tests.swift#testName`; `declared` evidence has
   the exact form `source:full/gamekit/<product-source>.swift#Symbol`. Every cited
   test/function and product source/anchor must exist, and the product source
   must be listed in `gamekit_guest_sources.txt`.
3. Nonempty `GameKit.swift` and `README.md`, plus `oracle-questions.tsv` with
...
4. Top-level synchronous no-argument functions named `test*` in
   `tests/agent/*Tests.swift`, plus `tests/agent/GameKitLoadSmoke.swift` whose
   exact content is `import GameKit`, one blank line, and
   `let frameworkLoadSmokeMarker = "GAMEKIT_AGENT_RUNTIME_OK"`. The sealed gate derives a runner
   from `implemented` coverage, loads `libGameKit.dylib`, and calls each cited
   test exactly once before accepting marker-only output. The load/marker check
   is not behavioral evidence by itself.
5. If dependencies are declared, `tests/agent/GameKitDependencyIdentity.swift`
