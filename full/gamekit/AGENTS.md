# GameKit framework fan-out rules

This directory is an isolated clean-room starting point for the Linux `GameKit`
port. Work only inside `full/gamekit/`. Do not edit application
...
- Implement a real Linux module named `GameKit` and a loadable
  `libGameKit.dylib`. Put implementation Swift files in this framework
  directory, outside `tests/`, and list each one as a repo-relative path in
  `gamekit_guest_sources.txt`.
- Create `coverage.tsv` with the exact header
  ...
  At least 460 rows must be `implemented` or `declared` for the `medium-full`
  ...
  `test:full/gamekit/tests/agent/*Tests.swift#testName`; define that exact `test*`
  function for the sealed runner to call. Every `declared` row must cite
  `source:full/gamekit/<product-source>.swift#Symbol`; the source must be listed in
  `gamekit_guest_sources.txt` and contain the exact identifier anchor. Deferred,
...
- Create a nonempty `README.md` describing what is real, fail-closed, and still
  deferred. The primary implementation file must be `GameKit.swift`.
- Put focused behavioral checks in `tests/agent/*Tests.swift` as top-level,
  synchronous, no-argument functions named `test*`. Create
  `tests/agent/GameKitLoadSmoke.swift` with exactly these three logical lines
  (including the blank line): `import GameKit`, a blank line, and
  `let frameworkLoadSmokeMarker = "GAMEKIT_AGENT_RUNTIME_OK"`. The sealed gate generates the
...
- When dependencies are listed below, create
  `tests/agent/GameKitDependencyIdentity.swift`. Import `GameKit` and every
  declared dependency and pass genuine dependency values through public
  `GameKit` APIs. This probe is for the clean EC2 integration build; the isolated
