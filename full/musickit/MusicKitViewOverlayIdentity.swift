...
// Identity View modifiers synthesized onto ArtworkImage: View.
// Linux has no SwiftUI layout engine; these compile as no-op identity
// (`Self`) returns so the overlay census can be pinned without inventing
// Apple layout. Each method covers every synthesized `s:7SwiftUI4ViewPAAE...`
// specialization sharing its base name. Compiled wherever the local
// lookalike View is in effect (isolated host and macOS, mirroring
// MusicKitLookalikes).
