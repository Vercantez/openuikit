# Social framework fan-out rules

This directory is an isolated clean-room starting point for the Linux `Social`
port. Work only inside `full/social/`. Do not edit application
...
- Implement a real Linux module named `Social` and a loadable
  `libSocial.dylib`. Put implementation Swift files in this framework
  directory, outside `tests/`, and list each one as a repo-relative path in
  `social_guest_sources.txt`.
- Create `coverage.tsv` with the exact header
  ...
  At least 54 rows must be `implemented` or `declared` for the `leaf-full`
...
- Create a nonempty `README.md` describing what is real, fail-closed, and still
  deferred. The primary implementation file must be `Social.swift`.
- Create `tests/agent/SocialRuntime.swift`. It must import `Social`, exercise
  meaningful implemented behavior, and print exactly `SOCIAL_AGENT_RUNTIME_OK` on success.
