...
if ! grep -Eq 'canonical UIKit module|Accounts module|dependency blocker|integration blocker' \
...
lookalike_output=$(
    LD_LIBRARY_PATH="$LOOKALIKE${LD_LIBRARY_PATH:+:$LD_LIBRARY_PATH}" \
        "$LOOKALIKE/social-lookalike-consumer"
)
printf '%s\n' "$lookalike_output"
...
if [ "$accounts_status" -eq 0 ]; then
    die 'production Social compiled without Accounts by inventing Social.ACAccount'
fi
