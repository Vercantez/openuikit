# Port `Social` to Linux

Build a substantial, honest starting implementation of Apple's public `Social`
...
The immutable seed contains 67 unique public precise identifiers and
122 symbol-graph relationships. The acceptance floor is
54 nondeferred (`implemented` or `declared`) identifiers: ceil(80% of exact public symbols).
...
Deliver all of the following in `full/social/`:

1. Linux Swift sources for module `Social` and `libSocial.dylib`, with every
   implementation source listed in `social_guest_sources.txt`.
...
3. Nonempty `Social.swift` and `README.md`, plus `oracle-questions.tsv` with
   header `precise	question	risk	reason` and at least one question; use
   `module` only for cross-cutting items.
4. `tests/agent/SocialRuntime.swift`, meaningful focused tests, and the exact
   success marker `SOCIAL_AGENT_RUNTIME_OK`.
