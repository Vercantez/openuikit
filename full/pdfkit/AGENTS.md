# PDFKit framework fan-out rules

This directory is an isolated clean-room starting point for the Linux `PDFKit`
port. Work only inside `full/pdfkit/`. Do not edit application
...
- Implement a real Linux module named `PDFKit` and a loadable
  `libPDFKit.dylib`. Put implementation Swift files in this framework
  directory, outside `tests/`, and list each one as a repo-relative path in
  `pdfkit_guest_sources.txt`.
- Create `coverage.tsv` with the exact header
  ...
  At least 68 rows must be `implemented` or `declared` for the `large-partitioned`
...
- Create a nonempty `README.md` describing what is real, fail-closed, and still
  deferred. The primary implementation file must be `PDFKit.swift`.
- Create `tests/agent/PDFKitRuntime.swift`. It must import `PDFKit`, exercise
  meaningful implemented behavior, and print exactly `PDFKIT_AGENT_RUNTIME_OK` on success.
