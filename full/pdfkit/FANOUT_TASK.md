# Port `PDFKit` to Linux

Build a substantial, honest starting implementation of Apple's public `PDFKit`
...
The immutable seed contains 679 unique public precise identifiers and
962 symbol-graph relationships. The acceptance floor is
68 nondeferred (`implemented` or `declared`) identifiers: min(150, max(50, ceil(10% of exact public symbols))).
...
Deliver all of the following in `full/pdfkit/`:

1. Linux Swift sources for module `PDFKit` and `libPDFKit.dylib`, with every
   implementation source listed in `pdfkit_guest_sources.txt`.
...
3. Nonempty `PDFKit.swift` and `README.md`, plus `oracle-questions.tsv` with
   header `precise	question	risk	reason` and at least one question; use
   `module` only for cross-cutting items.
4. `tests/agent/PDFKitRuntime.swift`, meaningful focused tests, and the exact
   success marker `PDFKIT_AGENT_RUNTIME_OK`.
