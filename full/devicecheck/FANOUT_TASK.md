# Port `DeviceCheck` to Linux

Build a substantial, honest starting implementation of Apple's public `DeviceCheck`
...
The immutable seed contains 41 unique public precise identifiers and
68 symbol-graph relationships. The acceptance floor is
21 nondeferred (`implemented` or `declared`) identifiers: ceil(50% of exact public symbols).
...
Deliver all of the following in `full/devicecheck/`:

1. Linux Swift sources for module `DeviceCheck` and `libDeviceCheck.dylib`, with every
   implementation source listed in `devicecheck_guest_sources.txt`.
2. Complete exact-ID `coverage.tsv` using only the five allowed statuses.
   `implemented` evidence has the exact form
   `test:full/devicecheck/tests/agent/*Tests.swift#testName`; `declared` evidence has
   the exact form `source:full/devicecheck/<product-source>.swift#Symbol`. Every cited
   test/function and product source/anchor must exist, and the product source
   must be listed in `devicecheck_guest_sources.txt`.
3. Nonempty `DeviceCheck.swift` and `README.md`, plus `oracle-questions.tsv` with
...
4. Top-level synchronous no-argument functions named `test*` in
   `tests/agent/*Tests.swift`, plus `tests/agent/DeviceCheckLoadSmoke.swift` whose
   exact content is `import DeviceCheck`, one blank line, and
   `let frameworkLoadSmokeMarker = "DEVICECHECK_AGENT_RUNTIME_OK"`. The sealed gate derives a runner
   from `implemented` coverage, loads `libDeviceCheck.dylib`, and calls each cited
   test exactly once before accepting marker-only output. The load/marker check
   is not behavioral evidence by itself.
5. If dependencies are declared, `tests/agent/DeviceCheckDependencyIdentity.swift`
