...
2. Complete exact-ID `coverage.tsv` using only the five allowed statuses.
   ...
   `test:full/quicklookthumbnailing/tests/agent/*Tests.swift#testName`; `declared` evidence has
   the exact form `source:full/quicklookthumbnailing/<product-source>.swift#Symbol`. Every cited
   test/function and product source/anchor must exist, and the product source
   must be listed in `quicklookthumbnailing_guest_sources.txt`.
...
4. Top-level synchronous no-argument functions named `test*` in
   `tests/agent/*Tests.swift`, plus `tests/agent/QuickLookThumbnailingLoadSmoke.swift` whose
   exact content is `import QuickLookThumbnailing`, one blank line, and
   `let frameworkLoadSmokeMarker = "QUICKLOOKTHUMBNAILING_AGENT_RUNTIME_OK"`. The sealed gate derives a runner
   from `implemented` coverage, loads `libQuickLookThumbnailing.dylib`, and calls each cited
   test exactly once before accepting marker-only output. The load/marker check
   is not behavioral evidence by itself.
5. If dependencies are declared, `tests/agent/QuickLookThumbnailingDependencyIdentity.swift`
