# Port `AdAttributionKit` to Linux

Build a substantial, honest starting implementation of Apple's public `AdAttributionKit`
...
The immutable seed contains 76 unique public precise identifiers and
107 symbol-graph relationships. The acceptance floor is
61 nondeferred (`implemented` or `declared`) identifiers: ceil(80% of exact public symbols).
...
Deliver all of the following in `full/adattributionkit/`:

1. Linux Swift sources for module `AdAttributionKit` and `libAdAttributionKit.dylib`, with every
   implementation source listed in `adattributionkit_guest_sources.txt`.
...
3. Nonempty `AdAttributionKit.swift` and `README.md`, plus `oracle-questions.tsv` with
   header `precise	question	risk	reason` and at least one question; use
   `module` only for cross-cutting items.
4. `tests/agent/AdAttributionKitRuntime.swift`, meaningful focused tests, and the exact
   success marker `ADATTRIBUTIONKIT_AGENT_RUNTIME_OK`.
