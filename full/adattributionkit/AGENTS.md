# AdAttributionKit framework fan-out rules

This directory is an isolated clean-room starting point for the Linux `AdAttributionKit`
port. Work only inside `full/adattributionkit/`. Do not edit application
...
- Implement a real Linux module named `AdAttributionKit` and a loadable
  `libAdAttributionKit.dylib`. Put implementation Swift files in this framework
  directory, outside `tests/`, and list each one as a repo-relative path in
  `adattributionkit_guest_sources.txt`.
- Create `coverage.tsv` with the exact header
  ...
  At least 61 rows must be `implemented` or `declared` for the `leaf-full`
...
- Create a nonempty `README.md` describing what is real, fail-closed, and still
  deferred. The primary implementation file must be `AdAttributionKit.swift`.
- Create `tests/agent/AdAttributionKitRuntime.swift`. It must import `AdAttributionKit`, exercise
  meaningful implemented behavior, and print exactly `ADATTRIBUTIONKIT_AGENT_RUNTIME_OK` on success.
