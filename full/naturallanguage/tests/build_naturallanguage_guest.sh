...
IMAGE=${NATURALLANGUAGE_IMAGE:-swift-macho-spike:python3-nosde-preflight-20260830}
OUTPUT=${1:-}

die() {
    echo "build_naturallanguage_guest: $*" >&2
    exit 2
}
...
[ -z "$(git -C "$ROOT" status --porcelain=v1 --untracked-files=all -- \
    full/naturallanguage)" ] \
    || die 'NaturalLanguage source tranche must be committed and clean'

if [ -z "$OUTPUT" ]; then
    OUTPUT=$(mktemp -d /private/tmp/naturallanguage-arm64-proof.XXXXXX)
else
    case "$OUTPUT" in
        /private/tmp/naturallanguage-arm64-proof-[A-Za-z0-9._-]*|/private/tmp/naturallanguage-arm64-proof.[A-Za-z0-9._-]*) ;;
...
docker run --rm \
    ...
    -v "$OUTPUT:/out" \
    "$IMAGE" \
    bash /w/full/naturallanguage/tests/build_naturallanguage_guest_in_container.sh

printf 'NATURALLANGUAGE_GUEST_OUTPUT=%s\n' "$OUTPUT"
