# Port `ExposureNotification` to Linux

Build a substantial, honest starting implementation of Apple's public `ExposureNotification`
...
The immutable seed contains 264 unique public precise identifiers and
344 symbol-graph relationships. The acceptance floor is
132 nondeferred (`implemented` or `declared`) identifiers: ceil(50% of exact public symbols).
...
Deliver all of the following in `full/exposurenotification/`:

1. Linux Swift sources for module `ExposureNotification` and `libExposureNotification.dylib`, with every
   implementation source listed in `exposurenotification_guest_sources.txt`.
2. Complete exact-ID `coverage.tsv` using only the five allowed statuses.
   `implemented` evidence has the exact form
   `test:full/exposurenotification/tests/agent/*Tests.swift#testName`; `declared` evidence has
   the exact form `source:full/exposurenotification/<product-source>.swift#Symbol`. Every cited
   test/function and product source/anchor must exist, and the product source
   must be listed in `exposurenotification_guest_sources.txt`.
3. Nonempty `ExposureNotification.swift` and `README.md`, plus `oracle-questions.tsv` with
...
4. Top-level synchronous no-argument functions named `test*` in
   `tests/agent/*Tests.swift`, plus `tests/agent/ExposureNotificationLoadSmoke.swift` whose
   exact content is `import ExposureNotification`, one blank line, and
   `let frameworkLoadSmokeMarker = "EXPOSURENOTIFICATION_AGENT_RUNTIME_OK"`. The sealed gate derives a runner
   from `implemented` coverage, loads `libExposureNotification.dylib`, and calls each cited
   test exactly once before accepting marker-only output. The load/marker check
   is not behavioral evidence by itself.
5. If dependencies are declared, `tests/agent/ExposureNotificationDependencyIdentity.swift`
