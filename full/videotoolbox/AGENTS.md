# VideoToolbox framework fan-out rules

This directory is an isolated clean-room starting point for the Linux `VideoToolbox`
port. Work only inside `full/videotoolbox/`. Do not edit application
...
- Implement a real Linux module named `VideoToolbox` and a loadable
  `libVideoToolbox.dylib`. Put implementation Swift files in this framework
  directory, outside `tests/`, and list each one as a repo-relative path in
  `videotoolbox_guest_sources.txt`.
- Create `coverage.tsv` with the exact header
  ...
  At least 96 rows must be `implemented` or `declared` for the `large-partitioned`
...
- Create a nonempty `README.md` describing what is real, fail-closed, and still
  deferred. The primary implementation file must be `VideoToolbox.swift`.
- Create `tests/agent/VideoToolboxRuntime.swift`. It must import `VideoToolbox`, exercise
  meaningful implemented behavior, and print exactly `VIDEOTOOLBOX_AGENT_RUNTIME_OK` on success.
