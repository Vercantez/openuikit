# Port `VideoToolbox` to Linux

Build a substantial, honest starting implementation of Apple's public `VideoToolbox`
...
The immutable seed contains 482 unique public precise identifiers and
20 symbol-graph relationships. The acceptance floor is
...
Deliver all of the following in `full/videotoolbox/`:

1. Linux Swift sources for module `VideoToolbox` and `libVideoToolbox.dylib`, with every
   implementation source listed in `videotoolbox_guest_sources.txt`.
...
3. Nonempty `VideoToolbox.swift` and `README.md`, plus `oracle-questions.tsv` with
   header `precise	question	risk	reason` and at least one question; use
   `module` only for cross-cutting items.
4. `tests/agent/VideoToolboxRuntime.swift`, meaningful focused tests, and the exact
   success marker `VIDEOTOOLBOX_AGENT_RUNTIME_OK`.
...
Reconstruction note: the original Xcode 26.1 extractor seed for this wave-3 lane was not present on `origin/main` and the platform branch could not be fetched. Precise IDs in this dossier come from pinned dotnet/macios VideoToolbox bindings, not a fresh `swift-symbolgraph-extract` run.
