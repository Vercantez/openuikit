# Port `BrowserKit` to Linux

Build a substantial, honest starting implementation of Apple's public `BrowserKit`
...
21 symbol-graph relationships. The acceptance floor is
...
Deliver all of the following in `full/browserkit/`:

1. Linux Swift sources for module `BrowserKit` and `libBrowserKit.dylib`, with every
   implementation source listed in `browserkit_guest_sources.txt`.
2. Complete exact-ID `coverage.tsv` using only the five allowed statuses.
   `implemented` evidence has the exact form
   `test:full/browserkit/tests/agent/*Tests.swift#testName`; `declared` evidence has
   the exact form `source:full/browserkit/<product-source>.swift#Symbol`. Every cited
   test/function and product source/anchor must exist, and the product source
   must be listed in `browserkit_guest_sources.txt`.
3. Nonempty `BrowserKit.swift` and `README.md`, plus `oracle-questions.tsv` with
...
4. Top-level synchronous no-argument functions named `test*` in
   `tests/agent/*Tests.swift`, plus `tests/agent/BrowserKitLoadSmoke.swift` whose
   exact content is `import BrowserKit`, one blank line, and
   `let frameworkLoadSmokeMarker = "BROWSERKIT_AGENT_RUNTIME_OK"`. The sealed gate derives a runner
   from `implemented` coverage, loads `libBrowserKit.dylib`, and calls each cited
   test exactly once before accepting marker-only output. The load/marker check
   is not behavioral evidence by itself.
5. If dependencies are declared, `tests/agent/BrowserKitDependencyIdentity.swift`
