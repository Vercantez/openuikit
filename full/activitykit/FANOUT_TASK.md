# Port `ActivityKit` to Linux

Build a substantial, honest starting implementation of Apple's public `ActivityKit`
...
The immutable seed contains 320 unique public precise identifiers and
364 symbol-graph relationships. The acceptance floor is
160 nondeferred (`implemented` or `declared`) identifiers: ceil(50% of exact public symbols).
...
Deliver all of the following in `full/activitykit/`:

1. Linux Swift sources for module `ActivityKit` and `libActivityKit.dylib`, with every
   implementation source listed in `activitykit_guest_sources.txt`.
...
3. Nonempty `ActivityKit.swift` and `README.md`, plus `oracle-questions.tsv` with
   header `precise	question	risk	reason` and at least one question; use
   `module` only for cross-cutting items.
4. `tests/agent/ActivityKitRuntime.swift`, meaningful focused tests, and the exact
   success marker `ACTIVITYKIT_AGENT_RUNTIME_OK`.
