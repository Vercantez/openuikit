# ActivityKit framework fan-out rules

This directory is an isolated clean-room starting point for the Linux `ActivityKit`
port. Work only inside `full/activitykit/`. Do not edit application
...
- Implement a real Linux module named `ActivityKit` and a loadable
  `libActivityKit.dylib`. Put implementation Swift files in this framework
  directory, outside `tests/`, and list each one as a repo-relative path in
  `activitykit_guest_sources.txt`.
- Create `coverage.tsv` with the exact header
  ...
  At least 160 rows must be `implemented` or `declared` for the `medium-full`
...
- Create a nonempty `README.md` describing what is real, fail-closed, and still
  deferred. The primary implementation file must be `ActivityKit.swift`.
- Create `tests/agent/ActivityKitRuntime.swift`. It must import `ActivityKit`, exercise
  meaningful implemented behavior, and print exactly `ACTIVITYKIT_AGENT_RUNTIME_OK` on success.
