# MetalPerformanceShaders framework fan-out rules

This directory is an isolated clean-room starting point for the Linux `MetalPerformanceShaders`
port. Work only inside `full/metalperformanceshaders/`. Do not edit application
...
- Implement a real Linux module named `MetalPerformanceShaders` and a loadable
  `libMetalPerformanceShaders.dylib`. Put implementation Swift files in this framework
  directory, outside `tests/`, and list each one as a repo-relative path in
  `metalperformanceshaders_guest_sources.txt`.
...
- Create a nonempty `README.md` describing what is real, fail-closed, and still
  deferred. The primary implementation file must be `MetalPerformanceShaders.swift`.
- Create `tests/agent/MetalPerformanceShadersRuntime.swift`. It must import `MetalPerformanceShaders`, exercise
  meaningful implemented behavior, and print exactly `METALPERFORMANCESHADERS_AGENT_RUNTIME_OK` on success.
