# BrowserEngineCore framework fan-out rules

This directory is an isolated clean-room starting point for the Linux `BrowserEngineCore`
port. Work only inside `full/browserenginecore/`. Do not edit application
...
- Implement a real Linux module named `BrowserEngineCore` and a loadable
  `libBrowserEngineCore.dylib`. Put implementation Swift files in this framework
  directory, outside `tests/`, and list each one as a repo-relative path in
  `browserenginecore_guest_sources.txt`.
- Create `coverage.tsv` with the exact header
  ...
  At least 12 rows must be `implemented` or `declared` for the `leaf-full`
  ...
  `test:full/browserenginecore/tests/agent/*Tests.swift#testName`; define that exact `test*`
  function for the sealed runner to call. Every `declared` row must cite
  `source:full/browserenginecore/<product-source>.swift#Symbol`; the source must be listed in
  `browserenginecore_guest_sources.txt` and contain the exact identifier anchor. Deferred,
...
- Create a nonempty `README.md` describing what is real, fail-closed, and still
  deferred. The primary implementation file must be `BrowserEngineCore.swift`.
- Put focused behavioral checks in `tests/agent/*Tests.swift` as top-level,
  synchronous, no-argument functions named `test*`. Create
  `tests/agent/BrowserEngineCoreLoadSmoke.swift` with exactly these three logical lines
  (including the blank line): `import BrowserEngineCore`, a blank line, and
  `let frameworkLoadSmokeMarker = "BROWSERENGINECORE_AGENT_RUNTIME_OK"`. The sealed gate generates the
...
- When dependencies are listed below, create
  `tests/agent/BrowserEngineCoreDependencyIdentity.swift`. Import `BrowserEngineCore` and every
  declared dependency and pass genuine dependency values through public
  `BrowserEngineCore` APIs. This probe is for the clean EC2 integration build; the isolated
