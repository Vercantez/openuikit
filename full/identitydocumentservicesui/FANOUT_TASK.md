# Port `IdentityDocumentServicesUI` to Linux

Build a substantial, honest starting implementation of Apple's public `IdentityDocumentServicesUI`
...
The immutable seed contains 31 unique public precise identifiers and
35 symbol-graph relationships. The acceptance floor is
25 nondeferred (`implemented` or `declared`) identifiers: ceil(80% of exact public symbols).
...
Deliver all of the following in `full/identitydocumentservicesui/`:

1. Linux Swift sources for module `IdentityDocumentServicesUI` and `libIdentityDocumentServicesUI.dylib`, with every
   implementation source listed in `identitydocumentservicesui_guest_sources.txt`.
2. Complete exact-ID `coverage.tsv` using only the five allowed statuses.
   `implemented` evidence has the exact form
   `test:full/identitydocumentservicesui/tests/agent/*Tests.swift#testName`; `declared` evidence has
   the exact form `source:full/identitydocumentservicesui/<product-source>.swift#Symbol`. Every cited
   test/function and product source/anchor must exist, and the product source
   must be listed in `identitydocumentservicesui_guest_sources.txt`.
3. Nonempty `IdentityDocumentServicesUI.swift` and `README.md`, plus `oracle-questions.tsv` with
...
4. Top-level synchronous no-argument functions named `test*` in
   `tests/agent/*Tests.swift`, plus `tests/agent/IdentityDocumentServicesUILoadSmoke.swift` whose
   exact content is `import IdentityDocumentServicesUI`, one blank line, and
   `let frameworkLoadSmokeMarker = "IDENTITYDOCUMENTSERVICESUI_AGENT_RUNTIME_OK"`. The sealed gate derives a runner
   from `implemented` coverage, loads `libIdentityDocumentServicesUI.dylib`, and calls each cited
   test exactly once before accepting marker-only output. The load/marker check
   is not behavioral evidence by itself.
5. If dependencies are declared, `tests/agent/IdentityDocumentServicesUIDependencyIdentity.swift`
