# Port `DeviceActivity` to Linux

Build a substantial, honest starting implementation of Apple's public `DeviceActivity`
...
The immutable seed contains 1036 unique public precise identifiers and
1099 symbol-graph relationships. The acceptance floor is
829 nondeferred (`implemented` or `declared`) identifiers: ceil(80% of exact public symbols).
...
Deliver all of the following in `full/deviceactivity/`:

1. Linux Swift sources for module `DeviceActivity` and `libDeviceActivity.dylib`, with every
   implementation source listed in `deviceactivity_guest_sources.txt`.
2. Complete exact-ID `coverage.tsv` using only the five allowed statuses.
   `implemented` evidence has the exact form
   `test:full/deviceactivity/tests/agent/*Tests.swift#testName`; `declared` evidence has
   the exact form `source:full/deviceactivity/<product-source>.swift#Symbol`. Every cited
   test/function and product source/anchor must exist, and the product source
   must be listed in `deviceactivity_guest_sources.txt`.
3. Nonempty `DeviceActivity.swift` and `README.md`, plus `oracle-questions.tsv` with
...
4. Top-level synchronous no-argument functions named `test*` in
   `tests/agent/*Tests.swift`, plus `tests/agent/DeviceActivityLoadSmoke.swift` whose
   exact content is `import DeviceActivity`, one blank line, and
   `let frameworkLoadSmokeMarker = "DEVICEACTIVITY_AGENT_RUNTIME_OK"`. The sealed gate derives a runner
   from `implemented` coverage, loads `libDeviceActivity.dylib`, and calls each cited
   test exactly once before accepting marker-only output. The load/marker check
   is not behavioral evidence by itself.
5. If dependencies are declared, `tests/agent/DeviceActivityDependencyIdentity.swift`
