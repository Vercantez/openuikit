# Port `TelephonyMessagingKit` to Linux

Build a substantial, honest starting implementation of Apple's public `TelephonyMessagingKit`
...
The immutable seed contains 981 unique public precise identifiers and
1547 symbol-graph relationships. The acceptance floor is
785 nondeferred (`implemented` or `declared`) identifiers: ceil(80% of exact public symbols).
...
Deliver all of the following in `full/telephonymessagingkit/`:

1. Linux Swift sources for module `TelephonyMessagingKit` and `libTelephonyMessagingKit.dylib`, with every
   implementation source listed in `telephonymessagingkit_guest_sources.txt`.
2. Complete exact-ID `coverage.tsv` using only the five allowed statuses.
   `implemented` evidence has the exact form
   `test:full/telephonymessagingkit/tests/agent/*Tests.swift#testName`; `declared` evidence has
   the exact form `source:full/telephonymessagingkit/<product-source>.swift#Symbol`. Every cited
   test/function and product source/anchor must exist, and the product source
   must be listed in `telephonymessagingkit_guest_sources.txt`.
3. Nonempty `TelephonyMessagingKit.swift` and `README.md`, plus `oracle-questions.tsv` with
...
4. Top-level synchronous no-argument functions named `test*` in
   `tests/agent/*Tests.swift`, plus `tests/agent/TelephonyMessagingKitLoadSmoke.swift` whose
   exact content is `import TelephonyMessagingKit`, one blank line, and
   `let frameworkLoadSmokeMarker = "TELEPHONYMESSAGINGKIT_AGENT_RUNTIME_OK"`. The sealed gate derives a runner
   from `implemented` coverage, loads `libTelephonyMessagingKit.dylib`, and calls each cited
   test exactly once before accepting marker-only output. The load/marker check
   is not behavioral evidence by itself.
5. If dependencies are declared, `tests/agent/TelephonyMessagingKitDependencyIdentity.swift`
