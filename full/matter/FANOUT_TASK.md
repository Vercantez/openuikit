# Port `Matter` to Linux

Build a substantial, honest starting implementation of Apple's public `Matter`
...
The immutable seed contains 28462 unique public precise identifiers and
45602 symbol-graph relationships. The acceptance floor is
...
Deliver all of the following in `full/matter/`:

1. Linux Swift sources for module `Matter` and `libMatter.dylib`, with every
   implementation source listed in `matter_guest_sources.txt`.
2. Complete exact-ID `coverage.tsv` using only the five allowed statuses.
   `implemented` evidence has the exact form
   `test:full/matter/tests/agent/*Tests.swift#testName`; `declared` evidence has
   the exact form `source:full/matter/<product-source>.swift#Symbol`. Every cited
   test/function and product source/anchor must exist, and the product source
   must be listed in `matter_guest_sources.txt`.
3. Nonempty `Matter.swift` and `README.md`, plus `oracle-questions.tsv` with
...
4. Top-level synchronous no-argument functions named `test*` in
   `tests/agent/*Tests.swift`, plus `tests/agent/MatterLoadSmoke.swift` whose
   exact content is `import Matter`, one blank line, and
   `let frameworkLoadSmokeMarker = "MATTER_AGENT_RUNTIME_OK"`. The sealed gate derives a runner
   from `implemented` coverage, loads `libMatter.dylib`, and calls each cited
   test exactly once before accepting marker-only output. The load/marker check
   is not behavioral evidence by itself.
5. If dependencies are declared, `tests/agent/MatterDependencyIdentity.swift`
