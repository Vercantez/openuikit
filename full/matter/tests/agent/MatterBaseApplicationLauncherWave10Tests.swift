...
func testBaseApplicationLauncherClassCacheFailClosed() {
    ...
    MTRBaseClusterApplicationLauncher.readAttributeAttributeList(withClusterStateCache: MTRClusterStateCacheContainer(), endpoint: n(1), queue: DispatchQueue.global(), completion: { _, err in mtrExpectInvalidState(err) })
    MTRBaseClusterApplicationLauncher.readAttributeCatalogList(withAttributeCache: MTRAttributeCacheContainer(), endpoint: n(1), queue: DispatchQueue.global(), completionHandler: { _, err in mtrExpectInvalidState(err) })
    MTRBaseClusterApplicationLauncher.readAttributeCatalogList(withClusterStateCache: MTRClusterStateCacheContainer(), endpoint: n(1), queue: DispatchQueue.global(), completion: { _, err in mtrExpectInvalidState(err) })
    MTRBaseClusterApplicationLauncher.readAttributeClusterRevision(withAttributeCache: MTRAttributeCacheContainer(), endpoint: n(1), queue: DispatchQueue.global(), completionHandler: { _, err in mtrExpectInvalidState(err) })
    MTRBaseClusterApplicationLauncher.readAttributeClusterRevision(withClusterStateCache: MTRClusterStateCacheContainer(), endpoint: n(1), queue: DispatchQueue.global(), completion: { _, err in mtrExpectInvalidState(err) })
    MTRBaseClusterApplicationLauncher.readAttributeCurrentApp(withAttributeCache: MTRAttributeCacheContainer(), endpoint: n(1), queue: DispatchQueue.global(), completionHandler: { _, err in mtrExpectInvalidState(err) })
    MTRBaseClusterApplicationLauncher.readAttributeCurrentApp(withClusterStateCache: MTRClusterStateCacheContainer(), endpoint: n(1), queue: DispatchQueue.global(), completion: { _, err in mtrExpectInvalidState(err) })
    MTRBaseClusterApplicationLauncher.readAttributeFeatureMap(withAttributeCache: MTRAttributeCacheContainer(), endpoint: n(1), queue: DispatchQueue.global(), completionHandler: { _, err in mtrExpectInvalidState(err) })
    MTRBaseClusterApplicationLauncher.readAttributeFeatureMap(withClusterStateCache: MTRClusterStateCacheContainer(), endpoint: n(1), queue: DispatchQueue.global(), completion: { _, err in mtrExpectInvalidState(err) })
    MTRBaseClusterApplicationLauncher.readAttributeGeneratedCommandList(withAttributeCache: MTRAttributeCacheContainer(), endpoint: n(1), queue: DispatchQueue.global(), completionHandler: { _, err in mtrExpectInvalidState(err) })
...
func testBaseApplicationLauncherCommandFailClosed() {
    ...
    cluster.hideApp(completion: { _, err in mtrExpectInvalidState(err) })
    cluster.hideApp(with: MTRApplicationLauncherClusterHideAppParams(), completion: { _, err in mtrExpectInvalidState(err) })
    cluster.hideApp(with: MTRApplicationLauncherClusterHideAppParams(), completionHandler: { _, err in mtrExpectInvalidState(err) })
    cluster.launchApp(completion: { _, err in mtrExpectInvalidState(err) })
    cluster.launchApp(with: MTRApplicationLauncherClusterLaunchAppParams(), completion: { _, err in mtrExpectInvalidState(err) })
    cluster.launchApp(with: MTRApplicationLauncherClusterLaunchAppParams(), completionHandler: { _, err in mtrExpectInvalidState(err) })
    cluster.stopApp(completion: { _, err in mtrExpectInvalidState(err) })
    cluster.stopApp(with: MTRApplicationLauncherClusterStopAppParams(), completion: { _, err in mtrExpectInvalidState(err) })
...
func testBaseApplicationLauncherReadFailClosed() {
    ...
    cluster.readAttributeCatalogList(completion: { _, err in mtrExpectInvalidState(err) })
    cluster.readAttributeCatalogList(completionHandler: { _, err in mtrExpectInvalidState(err) })
    ...
    cluster.readAttributeCurrentApp(completion: { _, err in mtrExpectInvalidState(err) })
    cluster.readAttributeCurrentApp(completionHandler: { _, err in mtrExpectInvalidState(err) })
...
func testBaseApplicationLauncherSubscribeFailClosed() {
    ...
    cluster.subscribeAttributeCatalogList(withMinInterval: n(1), maxInterval: n(1), params: MTRSubscribeParams.new(), subscriptionEstablished: nil as MTRSubscriptionEstablishedHandler?, reportHandler: { _, err in mtrExpectInvalidState(err) })
    cluster.subscribeAttributeCatalogList(with: MTRSubscribeParams.new(), subscriptionEstablished: nil as MTRSubscriptionEstablishedHandler?, reportHandler: { _, err in mtrExpectInvalidState(err) })
    ...
    cluster.subscribeAttributeCurrentApp(withMinInterval: n(1), maxInterval: n(1), params: MTRSubscribeParams.new(), subscriptionEstablished: nil as MTRSubscriptionEstablishedHandler?, reportHandler: { _, err in mtrExpectInvalidState(err) })
    cluster.subscribeAttributeCurrentApp(with: MTRSubscribeParams.new(), subscriptionEstablished: nil as MTRSubscriptionEstablishedHandler?, reportHandler: { _, err in mtrExpectInvalidState(err) })
...
func testBaseApplicationLauncherWriteFailClosed() {
    ...
    cluster.writeAttributeCurrentApp(withValue: MTRApplicationLauncherClusterApplicationEPStruct(), completion: { err in mtrExpectInvalidState(err) })
    cluster.writeAttributeCurrentApp(withValue: MTRApplicationLauncherClusterApplicationEP(), completionHandler: { err in mtrExpectInvalidState(err) })
