# Port `AdSupport` to Linux

Build a substantial, honest starting implementation of Apple's public `AdSupport`
...
The immutable seed contains 4 unique public precise identifiers and
10 symbol-graph relationships. The acceptance floor is
4 nondeferred (`implemented` or `declared`) identifiers: ceil(80% of exact public symbols).
...
Deliver all of the following in `full/adsupport/`:

1. Linux Swift sources for module `AdSupport` and `libAdSupport.dylib`, with every
   implementation source listed in `adsupport_guest_sources.txt`.
2. Complete exact-ID `coverage.tsv` using only the five allowed statuses.
   `implemented` evidence has the exact form
   `test:full/adsupport/tests/agent/*Tests.swift#testName`; `declared` evidence has
   the exact form `source:full/adsupport/<product-source>.swift#Symbol`. Every cited
   test/function and product source/anchor must exist, and the product source
   must be listed in `adsupport_guest_sources.txt`.
3. Nonempty `AdSupport.swift` and `README.md`, plus `oracle-questions.tsv` with
...
4. Top-level synchronous no-argument functions named `test*` in
   `tests/agent/*Tests.swift`, plus `tests/agent/AdSupportLoadSmoke.swift` whose
   exact content is `import AdSupport`, one blank line, and
   `let frameworkLoadSmokeMarker = "ADSUPPORT_AGENT_RUNTIME_OK"`. The sealed gate derives a runner
   from `implemented` coverage, loads `libAdSupport.dylib`, and calls each cited
   test exactly once before accepting marker-only output. The load/marker check
   is not behavioral evidence by itself.
5. If dependencies are declared, `tests/agent/AdSupportDependencyIdentity.swift`
