# CarPlay framework fan-out rules

This directory is an isolated clean-room starting point for the Linux `CarPlay`
port. Work only inside `full/carplay/`. Do not edit application
...
- Implement a real Linux module named `CarPlay` and a loadable
  `libCarPlay.dylib`. Put implementation Swift files in this framework
  directory, outside `tests/`, and list each one as a repo-relative path in
  `carplay_guest_sources.txt`.
- Create `coverage.tsv` with the exact header
  ...
  At least 99 rows must be `implemented` or `declared` for the `large-partitioned`
...
- Create a nonempty `README.md` describing what is real, fail-closed, and still
  deferred. The primary implementation file must be `CarPlay.swift`.
- Create `tests/agent/CarPlayRuntime.swift`. It must import `CarPlay`, exercise
  meaningful implemented behavior, and print exactly `CARPLAY_AGENT_RUNTIME_OK` on success.
