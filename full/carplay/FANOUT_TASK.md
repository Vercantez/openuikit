# Port `CarPlay` to Linux

Build a substantial, honest starting implementation of Apple's public `CarPlay`
...
The immutable seed contains 988 unique public precise identifiers and
1807 symbol-graph relationships. The acceptance floor is
99 nondeferred (`implemented` or `declared`) identifiers: min(150, max(50, ceil(10% of exact public symbols))).
...
Deliver all of the following in `full/carplay/`:

1. Linux Swift sources for module `CarPlay` and `libCarPlay.dylib`, with every
   implementation source listed in `carplay_guest_sources.txt`.
...
3. Nonempty `CarPlay.swift` and `README.md`, plus `oracle-questions.tsv` with
   header `precise	question	risk	reason` and at least one question; use
   `module` only for cross-cutting items.
4. `tests/agent/CarPlayRuntime.swift`, meaningful focused tests, and the exact
   success marker `CARPLAY_AGENT_RUNTIME_OK`.
