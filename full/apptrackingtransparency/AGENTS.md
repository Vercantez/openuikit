# AppTrackingTransparency framework fan-out rules

This directory is an isolated clean-room starting point for the Linux `AppTrackingTransparency`
port. Work only inside `full/apptrackingtransparency/`. Do not edit application
...
- Implement a real Linux module named `AppTrackingTransparency` and a loadable
  `libAppTrackingTransparency.dylib`. Put implementation Swift files in this framework
  directory, outside `tests/`, and list each one as a repo-relative path in
  `apptrackingtransparency_guest_sources.txt`.
- Create `coverage.tsv` with the exact header
  ...
  At least 10 rows must be `implemented` or `declared` for the `leaf-full`
...
- Create a nonempty `README.md` describing what is real, fail-closed, and still
  deferred. The primary implementation file must be `AppTrackingTransparency.swift`.
- Create `tests/agent/AppTrackingTransparencyRuntime.swift`. It must import `AppTrackingTransparency`, exercise
  meaningful implemented behavior, and print exactly `APPTRACKINGTRANSPARENCY_AGENT_RUNTIME_OK` on success.
