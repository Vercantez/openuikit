# NetworkExtension framework fan-out rules

This directory is an isolated clean-room starting point for the Linux `NetworkExtension`
port. Work only inside `full/networkextension/`. Do not edit application
...
- Implement a real Linux module named `NetworkExtension` and a loadable
  `libNetworkExtension.dylib`. Put implementation Swift files in this framework
  directory, outside `tests/`, and list each one as a repo-relative path in
  `networkextension_guest_sources.txt`.
- Create `coverage.tsv` with the exact header
  ...
  At least 120 rows must be `implemented` or `declared` for the `large-partitioned`
...
- Create a nonempty `README.md` describing what is real, fail-closed, and still
  deferred. The primary implementation file must be `NetworkExtension.swift`.
- Create `tests/agent/NetworkExtensionRuntime.swift`. It must import `NetworkExtension`, exercise
  meaningful implemented behavior, and print exactly `NETWORKEXTENSION_AGENT_RUNTIME_OK` on success.
