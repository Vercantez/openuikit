# CoreBluetooth framework fan-out rules

This directory is an isolated clean-room starting point for the Linux `CoreBluetooth`
port. Work only inside `full/corebluetooth/`. Do not edit application
...
- Implement a real Linux module named `CoreBluetooth` and a loadable
  `libCoreBluetooth.dylib`. Put implementation Swift files in this framework
  directory, outside `tests/`, and list each one as a repo-relative path in
  `corebluetooth_guest_sources.txt`.
- Create `coverage.tsv` with the exact header
  ...
  At least 237 rows must be `implemented` or `declared` for the `medium-full`
...
- Create a nonempty `README.md` describing what is real, fail-closed, and still
  deferred. The primary implementation file must be `CoreBluetooth.swift`.
- Create `tests/agent/CoreBluetoothRuntime.swift`. It must import `CoreBluetooth`, exercise
  meaningful implemented behavior, and print exactly `COREBLUETOOTH_AGENT_RUNTIME_OK` on success.
