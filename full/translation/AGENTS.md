# Translation framework fan-out rules

This directory is an isolated clean-room starting point for the Linux `Translation`
port. Work only inside `full/translation/`. Do not edit application
...
- Implement a real Linux module named `Translation` and a loadable
  `libTranslation.dylib`. Put implementation Swift files in this framework
  directory, outside `tests/`, and list each one as a repo-relative path in
  `translation_guest_sources.txt`.
- Create `coverage.tsv` with the exact header
  ...
  At least 72 rows must be `implemented` or `declared` for the `leaf-full`
...
- Create a nonempty `README.md` describing what is real, fail-closed, and still
  deferred. The primary implementation file must be `Translation.swift`.
- Create `tests/agent/TranslationRuntime.swift`. It must import `Translation`, exercise
  meaningful implemented behavior, and print exactly `TRANSLATION_AGENT_RUNTIME_OK` on success.
