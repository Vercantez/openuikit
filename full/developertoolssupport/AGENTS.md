# DeveloperToolsSupport framework fan-out rules

This directory is an isolated clean-room starting point for the Linux `DeveloperToolsSupport`
port. Work only inside `full/developertoolssupport/`. Do not edit application
...
- Implement a real Linux module named `DeveloperToolsSupport` and a loadable
  `libDeveloperToolsSupport.dylib`. Put implementation Swift files in this framework
  directory, outside `tests/`, and list each one as a repo-relative path in
  `developertoolssupport_guest_sources.txt`.
- Create `coverage.tsv` with the exact header
  ...
  At least 48 rows must be `implemented` or `declared` for the `leaf-full`
...
- Create a nonempty `README.md` describing what is real, fail-closed, and still
  deferred. The primary implementation file must be `DeveloperToolsSupport.swift`.
- Create `tests/agent/DeveloperToolsSupportRuntime.swift`. It must import `DeveloperToolsSupport`, exercise
  meaningful implemented behavior, and print exactly `DEVELOPERTOOLSSUPPORT_AGENT_RUNTIME_OK` on success.
