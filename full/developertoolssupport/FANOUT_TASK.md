# Port `DeveloperToolsSupport` to Linux

Build a substantial, honest starting implementation of Apple's public `DeveloperToolsSupport`
...
The immutable seed contains 59 unique public precise identifiers and
64 symbol-graph relationships. The acceptance floor is
48 nondeferred (`implemented` or `declared`) identifiers: ceil(80% of exact public symbols).
...
Deliver all of the following in `full/developertoolssupport/`:

1. Linux Swift sources for module `DeveloperToolsSupport` and `libDeveloperToolsSupport.dylib`, with every
   implementation source listed in `developertoolssupport_guest_sources.txt`.
...
3. Nonempty `DeveloperToolsSupport.swift` and `README.md`, plus `oracle-questions.tsv` with
   header `precise	question	risk	reason` and at least one question; use
   `module` only for cross-cutting items.
4. `tests/agent/DeveloperToolsSupportRuntime.swift`, meaningful focused tests, and the exact
   success marker `DEVELOPERTOOLSSUPPORT_AGENT_RUNTIME_OK`.
