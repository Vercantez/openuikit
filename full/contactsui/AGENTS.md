# ContactsUI framework fan-out rules

This directory is an isolated clean-room starting point for the Linux `ContactsUI`
port. Work only inside `full/contactsui/`. Do not edit application
...
- Implement a real Linux module named `ContactsUI` and a loadable
  `libContactsUI.dylib`. Put implementation Swift files in this framework
  directory, outside `tests/`, and list each one as a repo-relative path in
  `contactsui_guest_sources.txt`.
- Create `coverage.tsv` with the exact header
  ...
  At least 85 rows must be `implemented` or `declared` for the `large-partitioned`
...
- Create a nonempty `README.md` describing what is real, fail-closed, and still
  deferred. The primary implementation file must be `ContactsUI.swift`.
- Create `tests/agent/ContactsUIRuntime.swift`. It must import `ContactsUI`, exercise
  meaningful implemented behavior, and print exactly `CONTACTSUI_AGENT_RUNTIME_OK` on success.
