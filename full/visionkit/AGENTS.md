# VisionKit framework fan-out rules

This directory is an isolated clean-room starting point for the Linux `VisionKit`
port. Work only inside `full/visionkit/`. Do not edit application
...
- Implement a real Linux module named `VisionKit` and a loadable
  `libVisionKit.dylib`. Put implementation Swift files in this framework
  directory, outside `tests/`, and list each one as a repo-relative path in
  `visionkit_guest_sources.txt`.
- Create `coverage.tsv` with the exact header
  ...
  At least 128 rows must be `implemented` or `declared` for the `medium-full`
...
- Create a nonempty `README.md` describing what is real, fail-closed, and still
  deferred. The primary implementation file must be `VisionKit.swift`.
- Create `tests/agent/VisionKitRuntime.swift`. It must import `VisionKit`, exercise
  meaningful implemented behavior, and print exactly `VISIONKIT_AGENT_RUNTIME_OK` on success.
