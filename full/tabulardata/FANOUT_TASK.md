# Port `TabularData` to Linux

Build a substantial, honest starting implementation of Apple's public `TabularData`
...
The immutable seed contains 1768 unique public precise identifiers and
1971 symbol-graph relationships. The acceptance floor is
884 nondeferred (`implemented` or `declared`) identifiers: ceil(50% of exact public symbols).
...
Deliver all of the following in `full/tabulardata/`:

1. Linux Swift sources for module `TabularData` and `libTabularData.dylib`, with every
   implementation source listed in `tabulardata_guest_sources.txt`.
2. Complete exact-ID `coverage.tsv` using only the five allowed statuses.
   `implemented` evidence has the exact form
   `test:full/tabulardata/tests/agent/*Tests.swift#testName`; `declared` evidence has
   the exact form `source:full/tabulardata/<product-source>.swift#Symbol`. Every cited
   test/function and product source/anchor must exist, and the product source
   must be listed in `tabulardata_guest_sources.txt`.
3. Nonempty `TabularData.swift` and `README.md`, plus `oracle-questions.tsv` with
...
4. Top-level synchronous no-argument functions named `test*` in
   `tests/agent/*Tests.swift`, plus `tests/agent/TabularDataLoadSmoke.swift` whose
   exact content is `import TabularData`, one blank line, and
   `let frameworkLoadSmokeMarker = "TABULARDATA_AGENT_RUNTIME_OK"`. The sealed gate derives a runner
   from `implemented` coverage, loads `libTabularData.dylib`, and calls each cited
   test exactly once before accepting marker-only output. The load/marker check
   is not behavioral evidence by itself.
5. If dependencies are declared, `tests/agent/TabularDataDependencyIdentity.swift`
