# ClassKit framework fan-out rules

This directory is an isolated clean-room starting point for the Linux `ClassKit`
port. Work only inside `full/classkit/`. Do not edit application
...
- Implement a real Linux module named `ClassKit` and a loadable
  `libClassKit.dylib`. Put implementation Swift files in this framework
...
- Create `coverage.tsv` with the exact header
  ...
  At least 101 rows must be `implemented` or `declared` for the `medium-full`
  ...
  `test:full/classkit/tests/agent/*Tests.swift#testName`; define that exact `test*`
  function for the sealed runner to call. Every `declared` row must cite
  `source:full/classkit/<product-source>.swift#Symbol`; the source must be listed in
  `classkit_guest_sources.txt` and contain the exact identifier anchor. Deferred,
...
- Create a nonempty `README.md` describing what is real, fail-closed, and still
  deferred. The primary implementation file must be `ClassKit.swift`.
- Put focused behavioral checks in `tests/agent/*Tests.swift` as top-level,
  synchronous, no-argument functions named `test*`. Create
  `tests/agent/ClassKitLoadSmoke.swift` with exactly these three logical lines
  (including the blank line): `import ClassKit`, a blank line, and
  `let frameworkLoadSmokeMarker = "CLASSKIT_AGENT_RUNTIME_OK"`. The sealed gate generates the
...
- When dependencies are listed below, create
  `tests/agent/ClassKitDependencyIdentity.swift`. Import `ClassKit` and every
  declared dependency and pass genuine dependency values through public
  `ClassKit` APIs. This probe is for the clean EC2 integration build; the isolated
