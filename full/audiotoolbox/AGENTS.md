# AudioToolbox framework fan-out rules

This directory is an isolated clean-room starting point for the Linux `AudioToolbox`
port. Work only inside `full/audiotoolbox/`. Do not edit application
...
- Implement a real Linux module named `AudioToolbox` and a loadable
  `libAudioToolbox.dylib`. Put implementation Swift files in this framework
  directory, outside `tests/`, and list each one as a repo-relative path in
  `audiotoolbox_guest_sources.txt`.
- Create `coverage.tsv` with the exact header
  ...
  `test:full/audiotoolbox/tests/agent/*Tests.swift#testName`; define that exact `test*`
  function for the sealed runner to call. Every `declared` row must cite
  `source:full/audiotoolbox/<product-source>.swift#Symbol`; the source must be listed in
  `audiotoolbox_guest_sources.txt` and contain the exact identifier anchor. Deferred,
...
- Create a nonempty `README.md` describing what is real, fail-closed, and still
  deferred. The primary implementation file must be `AudioToolbox.swift`.
- Put focused behavioral checks in `tests/agent/*Tests.swift` as top-level,
  synchronous, no-argument functions named `test*`. Create
  `tests/agent/AudioToolboxLoadSmoke.swift` with exactly these three logical lines
  (including the blank line): `import AudioToolbox`, a blank line, and
  `let frameworkLoadSmokeMarker = "AUDIOTOOLBOX_AGENT_RUNTIME_OK"`. The sealed gate generates the
...
- When dependencies are listed below, create
  `tests/agent/AudioToolboxDependencyIdentity.swift`. Import `AudioToolbox` and every
  declared dependency and pass genuine dependency values through public
  `AudioToolbox` APIs. This probe is for the clean EC2 integration build; the isolated
