...
private func atW4ProbeOptionSet<T: OptionSet>(_ a: T, _ b: T) where T.Element == T {
    ...
    set = [a]
    atW4Expect(set.contains(a), "array literal contains")
    atW4Expect(!set.contains(b), "array literal misses other")
    ...
    set.formSymmetricDifference([a, b])
    atW4Expect(set.contains(b) && !set.contains(a), "formSymmetricDifference")
    var copy = T([a, b])
    copy.subtract(a)
    atW4Expect(!copy.contains(a) && copy.contains(b), "subtract")
    atW4Expect(a.isSubset(of: [a, b]), "isSubset")
    atW4Expect([a, b].isSuperset(of: a), "isSuperset")
    atW4Expect(a.isDisjoint(with: b), "isDisjoint")
    atW4Expect(a.isStrictSubset(of: [a, b]), "isStrictSubset")
    atW4Expect([a, b].isStrictSuperset(of: a), "isStrictSuperset")
    atW4Expect(a.union(b).contains(a) && a.union(b).contains(b), "union")
    atW4Expect(a.intersection([a, b]).contains(a), "intersection")
