# AVFAudio framework fan-out rules

This directory is an isolated clean-room starting point for the Linux `AVFAudio`
port. Work only inside `full/avfaudio/`. Do not edit application
...
- Implement a real Linux module named `AVFAudio` and a loadable
  `libAVFAudio.dylib`. Put implementation Swift files in this framework
  directory, outside `tests/`, and list each one as a repo-relative path in
  `avfaudio_guest_sources.txt`.
...
- Create a nonempty `README.md` describing what is real, fail-closed, and still
  deferred. The primary implementation file must be `AVFAudio.swift`.
- Create `tests/agent/AVFAudioRuntime.swift`. It must import `AVFAudio`, exercise
  meaningful implemented behavior, and print exactly `AVFAUDIO_AGENT_RUNTIME_OK` on success.
