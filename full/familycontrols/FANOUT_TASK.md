# Port `FamilyControls` to Linux

Build a substantial, honest starting implementation of Apple's public `FamilyControls`
...
The immutable seed contains 2407 unique public precise identifiers and
2447 symbol-graph relationships. The acceptance floor is
1926 nondeferred (`implemented` or `declared`) identifiers: ceil(80% of exact public symbols).
...
Deliver all of the following in `full/familycontrols/`:

1. Linux Swift sources for module `FamilyControls` and `libFamilyControls.dylib`, with every
   implementation source listed in `familycontrols_guest_sources.txt`.
2. Complete exact-ID `coverage.tsv` using only the five allowed statuses.
   `implemented` evidence has the exact form
   `test:full/familycontrols/tests/agent/*Tests.swift#testName`; `declared` evidence has
   the exact form `source:full/familycontrols/<product-source>.swift#Symbol`. Every cited
   test/function and product source/anchor must exist, and the product source
   must be listed in `familycontrols_guest_sources.txt`.
3. Nonempty `FamilyControls.swift` and `README.md`, plus `oracle-questions.tsv` with
...
4. Top-level synchronous no-argument functions named `test*` in
   `tests/agent/*Tests.swift`, plus `tests/agent/FamilyControlsLoadSmoke.swift` whose
   exact content is `import FamilyControls`, one blank line, and
   `let frameworkLoadSmokeMarker = "FAMILYCONTROLS_AGENT_RUNTIME_OK"`. The sealed gate derives a runner
   from `implemented` coverage, loads `libFamilyControls.dylib`, and calls each cited
   test exactly once before accepting marker-only output. The load/marker check
   is not behavioral evidence by itself.
5. If dependencies are declared, `tests/agent/FamilyControlsDependencyIdentity.swift`
