...
build_inside() {
    ...
    # Ubuntu exposes versioned LLVM tools through /usr/bin symlinks. Resolve
    # that trusted container entry point once, then hand the object auditor an
    # ordinary executable. The auditor deliberately continues to reject a
    # symlink supplied directly, preserving its fail-closed file contract.
    local llvm_nm
