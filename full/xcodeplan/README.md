...
table plus content-addressed payload bytes. Raw catalogs remain present for
provenance; the generated index and payloads are separate, byte-attested build
products rather than a claim to reproduce Apple's proprietary `Assets.car`.
Every copied or generated byte is re-read and recorded in a canonical
materialisation manifest. Source or platform symlinks, a reused output, a
missing semantic-colour/font input, an unsafe or duplicate catalog, and an
application collision with the reserved platform resource directory all
refuse. The executable host binds `OpenUIKitRuntime` to these bundle-relative
resources before the first window tick, removing checkout/build-container
paths from the runtime contract.
