# PushToTalk framework fan-out rules

This directory is an isolated clean-room starting point for the Linux `PushToTalk`
port. Work only inside `full/pushtotalk/`. Do not edit application
...
- Implement a real Linux module named `PushToTalk` and a loadable
  `libPushToTalk.dylib`. Put implementation Swift files in this framework
  directory, outside `tests/`, and list each one as a repo-relative path in
  `pushtotalk_guest_sources.txt`.
- Create `coverage.tsv` with the exact header
  ...
  At least 124 rows must be `implemented` or `declared` for the `leaf-full`
  ...
  `test:full/pushtotalk/tests/agent/*Tests.swift#testName`; define that exact `test*`
  function for the sealed runner to call. Every `declared` row must cite
  `source:full/pushtotalk/<product-source>.swift#Symbol`; the source must be listed in
  `pushtotalk_guest_sources.txt` and contain the exact identifier anchor. Deferred,
...
- Create a nonempty `README.md` describing what is real, fail-closed, and still
  deferred. The primary implementation file must be `PushToTalk.swift`.
- Put focused behavioral checks in `tests/agent/*Tests.swift` as top-level,
  synchronous, no-argument functions named `test*`. Create
  `tests/agent/PushToTalkLoadSmoke.swift` with exactly these three logical lines
  (including the blank line): `import PushToTalk`, a blank line, and
  `let frameworkLoadSmokeMarker = "PUSHTOTALK_AGENT_RUNTIME_OK"`. The sealed gate generates the
...
- When dependencies are listed below, create
  `tests/agent/PushToTalkDependencyIdentity.swift`. Import `PushToTalk` and every
  declared dependency and pass genuine dependency values through public
  `PushToTalk` APIs. This probe is for the clean EC2 integration build; the isolated
