# Port `SpriteKit` to Linux

Build a substantial, honest starting implementation of Apple's public `SpriteKit`
...
The immutable seed contains 1883 unique public precise identifiers and
2610 symbol-graph relationships. The acceptance floor is
...
Deliver all of the following in `full/spritekit/`:

1. Linux Swift sources for module `SpriteKit` and `libSpriteKit.dylib`, with every
   implementation source listed in `spritekit_guest_sources.txt`.
2. Complete exact-ID `coverage.tsv` using only the five allowed statuses.
   `implemented` evidence has the exact form
   `test:full/spritekit/tests/agent/*Tests.swift#testName`; `declared` evidence has
   the exact form `source:full/spritekit/<product-source>.swift#Symbol`. Every cited
   test/function and product source/anchor must exist, and the product source
   must be listed in `spritekit_guest_sources.txt`.
3. Nonempty `SpriteKit.swift` and `README.md`, plus `oracle-questions.tsv` with
...
4. Top-level synchronous no-argument functions named `test*` in
   `tests/agent/*Tests.swift`, plus `tests/agent/SpriteKitLoadSmoke.swift` whose
   exact content is `import SpriteKit`, one blank line, and
   `let frameworkLoadSmokeMarker = "SPRITEKIT_AGENT_RUNTIME_OK"`. The sealed gate derives a runner
   from `implemented` coverage, loads `libSpriteKit.dylib`, and calls each cited
   test exactly once before accepting marker-only output. The load/marker check
   is not behavioral evidence by itself.
5. If dependencies are declared, `tests/agent/SpriteKitDependencyIdentity.swift`
