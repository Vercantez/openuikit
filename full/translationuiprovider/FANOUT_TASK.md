# Port `TranslationUIProvider` to Linux

Build a substantial, honest starting implementation of Apple's public `TranslationUIProvider`
...
The immutable seed contains 16 unique public precise identifiers and
22 symbol-graph relationships. The acceptance floor is
13 nondeferred (`implemented` or `declared`) identifiers: ceil(80% of exact public symbols).
...
Deliver all of the following in `full/translationuiprovider/`:

1. Linux Swift sources for module `TranslationUIProvider` and `libTranslationUIProvider.dylib`, with every
   implementation source listed in `translationuiprovider_guest_sources.txt`.
2. Complete exact-ID `coverage.tsv` using only the five allowed statuses.
   `implemented` evidence has the exact form
   `test:full/translationuiprovider/tests/agent/*Tests.swift#testName`; `declared` evidence has
   the exact form `source:full/translationuiprovider/<product-source>.swift#Symbol`. Every cited
   test/function and product source/anchor must exist, and the product source
   must be listed in `translationuiprovider_guest_sources.txt`.
3. Nonempty `TranslationUIProvider.swift` and `README.md`, plus `oracle-questions.tsv` with
...
4. Top-level synchronous no-argument functions named `test*` in
   `tests/agent/*Tests.swift`, plus `tests/agent/TranslationUIProviderLoadSmoke.swift` whose
   exact content is `import TranslationUIProvider`, one blank line, and
   `let frameworkLoadSmokeMarker = "TRANSLATIONUIPROVIDER_AGENT_RUNTIME_OK"`. The sealed gate derives a runner
   from `implemented` coverage, loads `libTranslationUIProvider.dylib`, and calls each cited
   test exactly once before accepting marker-only output. The load/marker check
   is not behavioral evidence by itself.
5. If dependencies are declared, `tests/agent/TranslationUIProviderDependencyIdentity.swift`
