...
func testWave11ChartContentBatch03() {
    ...
    let symbolShape: any ChartContent = wave11Bar().symbol(BasicChartSymbolShape.square)
    ...
    let offsetXY: _ChartAttributedPlotContent = wave11Bar().offset(x: 10.0, y: 4.0)
    precondition(offsetXY.chartPlotRecords[0].layout.offsetX == 10)
    precondition(offsetXY.chartPlotRecords[0].layout.offsetY == 4)
    let offsetSize: _ChartAttributedPlotContent = wave11Bar().offset(CGSize(width: 1.0, height: 2.0))
    precondition(offsetSize.chartPlotRecords[0].layout.offsetX == 1)
    precondition(offsetSize.chartPlotRecords[0].layout.offsetY == 2)
    let offsetYSpan: _ChartAttributedPlotContent = wave11Bar().offset(x: 0.0, yStart: 2.0, yEnd: 3.0)
    ...
    let offsetXSpan: _ChartAttributedPlotContent = wave11Bar().offset(xStart: 1.0, xEnd: 2.0, y: 0.0)
    ...
    let offsetBothSpans: _ChartAttributedPlotContent = wave11Bar().offset(xStart: 0.0, xEnd: 1.0, yStart: 0.0, yEnd: 1.0)
...
func testWave11Chart3DAndAxisBatch04() {
    ...
    let markFonted: _AxisMarkAttributed<AxisValueLabel<Text>> = AxisValueLabel("Jan").font(Font.body)
    precondition(markFonted.fontName != nil)
    let markOffset: _AxisMarkAttributed<AxisValueLabel<Text>> = AxisValueLabel("Jan").offset(x: 1.0, y: 2.0)
    precondition(markOffset.offsetX == 1)
    precondition(markOffset.offsetY == 2)
    let markOffsetSize: _AxisMarkAttributed<AxisValueLabel<Text>> = AxisValueLabel("Jan").offset(CGSize(width: 3.0, height: 4.0))
    precondition(markOffsetSize.offsetX == 3)
    precondition(markOffsetSize.offsetY == 4)
    let axisLayered: _AxisContentAttributed<AxisMarks<_EmptyAxisMark>> = AxisMarks().compositingLayer()
    precondition(axisLayered.compositing == "layer")
    let axisStyledLayer: _AxisContentAttributed<AxisMarks<_EmptyAxisMark>> = AxisMarks().compositingLayer { _ in Color.blue }
