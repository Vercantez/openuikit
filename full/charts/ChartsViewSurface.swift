...
/// real module is imported; the isolated host has no SwiftUI, so Charts
/// types that conform to View expose the synthesized overlay census as
/// no-op lookalikes. Charts-owned chart* modifiers live in
/// ChartsModifiers.swift so they exist on Darwin too.
enum _ChartsViewSurfaceMarker {}
