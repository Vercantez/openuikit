# Port `Charts` to Linux

Build a substantial, honest starting implementation of Apple's public `Charts`
...
The immutable seed contains 9474 unique public precise identifiers and
9719 symbol-graph relationships. The acceptance floor is
4737 nondeferred (`implemented` or `declared`) identifiers: ceil(50% of exact public symbols).
...
Deliver all of the following in `full/charts/`:

1. Linux Swift sources for module `Charts` and `libCharts.dylib`, with every
   implementation source listed in `charts_guest_sources.txt`.
2. Complete exact-ID `coverage.tsv` using only the five allowed statuses.
   `implemented` evidence has the exact form
   `test:full/charts/tests/agent/*Tests.swift#testName`; `declared` evidence has
   the exact form `source:full/charts/<product-source>.swift#Symbol`. Every cited
   test/function and product source/anchor must exist, and the product source
   must be listed in `charts_guest_sources.txt`.
3. Nonempty `Charts.swift` and `README.md`, plus `oracle-questions.tsv` with
...
4. Top-level synchronous no-argument functions named `test*` in
   `tests/agent/*Tests.swift`, plus `tests/agent/ChartsLoadSmoke.swift` whose
   exact content is `import Charts`, one blank line, and
   `let frameworkLoadSmokeMarker = "CHARTS_AGENT_RUNTIME_OK"`. The sealed gate derives a runner
   from `implemented` coverage, loads `libCharts.dylib`, and calls each cited
   test exactly once before accepting marker-only output. The load/marker check
   is not behavioral evidence by itself.
5. If dependencies are declared, `tests/agent/ChartsDependencyIdentity.swift`
