# CoreMedia framework fan-out rules

This directory is an isolated clean-room starting point for the Linux `CoreMedia`
port. Work only inside `full/coremedia/`. Do not edit application
...
- Implement a real Linux module named `CoreMedia` and a loadable
  `libCoreMedia.dylib`. Put implementation Swift files in this framework
  directory, outside `tests/`, and list each one as a repo-relative path in
  `coremedia_guest_sources.txt`.
- Create `coverage.tsv` with the exact header
  ...
  `test:full/coremedia/tests/agent/*Tests.swift#testName`; define that exact `test*`
  function for the sealed runner to call. Every `declared` row must cite
  `source:full/coremedia/<product-source>.swift#Symbol`; the source must be listed in
  `coremedia_guest_sources.txt` and contain the exact identifier anchor. Deferred,
...
- Create a nonempty `README.md` describing what is real, fail-closed, and still
  deferred. The primary implementation file must be `CoreMedia.swift`.
- Put focused behavioral checks in `tests/agent/*Tests.swift` as top-level,
  synchronous, no-argument functions named `test*`. Create
  `tests/agent/CoreMediaLoadSmoke.swift` with exactly these three logical lines
  (including the blank line): `import CoreMedia`, a blank line, and
  `let frameworkLoadSmokeMarker = "COREMEDIA_AGENT_RUNTIME_OK"`. The sealed gate generates the
...
- When dependencies are listed below, create
  `tests/agent/CoreMediaDependencyIdentity.swift`. Import `CoreMedia` and every
  declared dependency and pass genuine dependency values through public
  `CoreMedia` APIs. This probe is for the clean EC2 integration build; the isolated
