# BackgroundAssets framework fan-out rules

This directory is an isolated clean-room starting point for the Linux `BackgroundAssets`
port. Work only inside `full/backgroundassets/`. Do not edit application
...
- Implement a real Linux module named `BackgroundAssets` and a loadable
  `libBackgroundAssets.dylib`. Put implementation Swift files in this framework
  directory, outside `tests/`, and list each one as a repo-relative path in
  `backgroundassets_guest_sources.txt`.
- Create `coverage.tsv` with the exact header
  ...
  At least 159 rows must be `implemented` or `declared` for the `leaf-full`
  ...
  `test:full/backgroundassets/tests/agent/*Tests.swift#testName`; define that exact `test*`
  function for the sealed runner to call. Every `declared` row must cite
  `source:full/backgroundassets/<product-source>.swift#Symbol`; the source must be listed in
  `backgroundassets_guest_sources.txt` and contain the exact identifier anchor. Deferred,
...
- Create a nonempty `README.md` describing what is real, fail-closed, and still
  deferred. The primary implementation file must be `BackgroundAssets.swift`.
- Put focused behavioral checks in `tests/agent/*Tests.swift` as top-level,
  synchronous, no-argument functions named `test*`. Create
  `tests/agent/BackgroundAssetsLoadSmoke.swift` with exactly these three logical lines
  (including the blank line): `import BackgroundAssets`, a blank line, and
  `let frameworkLoadSmokeMarker = "BACKGROUNDASSETS_AGENT_RUNTIME_OK"`. The sealed gate generates the
...
- When dependencies are listed below, create
  `tests/agent/BackgroundAssetsDependencyIdentity.swift`. Import `BackgroundAssets` and every
  declared dependency and pass genuine dependency values through public
  `BackgroundAssets` APIs. This probe is for the clean EC2 integration build; the isolated
