# Port `SceneKit` to Linux

Build a substantial, honest starting implementation of Apple's public `SceneKit`
...
The immutable seed contains 2611 unique public precise identifiers and
3488 symbol-graph relationships. The acceptance floor is
...
Deliver all of the following in `full/scenekit/`:

1. Linux Swift sources for module `SceneKit` and `libSceneKit.dylib`, with every
   implementation source listed in `scenekit_guest_sources.txt`.
...
3. Nonempty `SceneKit.swift` and `README.md`, plus `oracle-questions.tsv` with
   header `precise	question	risk	reason` and at least one question; use
   `module` only for cross-cutting items.
4. `tests/agent/SceneKitRuntime.swift`, meaningful focused tests, and the exact
   success marker `SCENEKIT_AGENT_RUNTIME_OK`.
