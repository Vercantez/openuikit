# SceneKit framework fan-out rules

This directory is an isolated clean-room starting point for the Linux `SceneKit`
port. Work only inside `full/scenekit/`. Do not edit application
...
- Implement a real Linux module named `SceneKit` and a loadable
  `libSceneKit.dylib`. Put implementation Swift files in this framework
  directory, outside `tests/`, and list each one as a repo-relative path in
  `scenekit_guest_sources.txt`.
...
- Create a nonempty `README.md` describing what is real, fail-closed, and still
  deferred. The primary implementation file must be `SceneKit.swift`.
- Create `tests/agent/SceneKitRuntime.swift`. It must import `SceneKit`, exercise
  meaningful implemented behavior, and print exactly `SCENEKIT_AGENT_RUNTIME_OK` on success.
