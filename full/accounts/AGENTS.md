# Accounts framework fan-out rules

This directory is an isolated clean-room starting point for the Linux `Accounts`
port. Work only inside `full/accounts/`. Do not edit application
...
- Implement a real Linux module named `Accounts` and a loadable
  `libAccounts.dylib`. Put implementation Swift files in this framework
  directory, outside `tests/`, and list each one as a repo-relative path in
  `accounts_guest_sources.txt`.
- Create `coverage.tsv` with the exact header
  ...
  At least 66 rows must be `implemented` or `declared` for the `legacy-adapter`
...
- Create a nonempty `README.md` describing what is real, fail-closed, and still
  deferred. The primary implementation file must be `Accounts.swift`.
- Create `tests/agent/AccountsRuntime.swift`. It must import `Accounts`, exercise
  meaningful implemented behavior, and print exactly `ACCOUNTS_AGENT_RUNTIME_OK` on success.
