# Port `SystemExtensions` to Linux

Build a substantial, honest starting implementation of Apple's public `SystemExtensions`
...
The immutable seed contains 57 unique public precise identifiers and
80 symbol-graph relationships. The acceptance floor is
46 nondeferred (`implemented` or `declared`) identifiers: ceil(80% of exact public symbols).
...
Deliver all of the following in `full/systemextensions/`:

1. Linux Swift sources for module `SystemExtensions` and `libSystemExtensions.dylib`, with every
   implementation source listed in `systemextensions_guest_sources.txt`.
2. Complete exact-ID `coverage.tsv` using only the five allowed statuses.
   `implemented` evidence has the exact form
   `test:full/systemextensions/tests/agent/*Tests.swift#testName`; `declared` evidence has
   the exact form `source:full/systemextensions/<product-source>.swift#Symbol`. Every cited
   test/function and product source/anchor must exist, and the product source
   must be listed in `systemextensions_guest_sources.txt`.
3. Nonempty `SystemExtensions.swift` and `README.md`, plus `oracle-questions.tsv` with
...
4. Top-level synchronous no-argument functions named `test*` in
   `tests/agent/*Tests.swift`, plus `tests/agent/SystemExtensionsLoadSmoke.swift` whose
   exact content is `import SystemExtensions`, one blank line, and
   `let frameworkLoadSmokeMarker = "SYSTEMEXTENSIONS_AGENT_RUNTIME_OK"`. The sealed gate derives a runner
   from `implemented` coverage, loads `libSystemExtensions.dylib`, and calls each cited
   test exactly once before accepting marker-only output. The load/marker check
   is not behavioral evidence by itself.
5. If dependencies are declared, `tests/agent/SystemExtensionsDependencyIdentity.swift`
