# Port `IdentityDocumentServices` to Linux

Build a substantial, honest starting implementation of Apple's public `IdentityDocumentServices`
...
The immutable seed contains 106 unique public precise identifiers and
162 symbol-graph relationships. The acceptance floor is
...
Deliver all of the following in `full/identitydocumentservices/`:

1. Linux Swift sources for module `IdentityDocumentServices` and `libIdentityDocumentServices.dylib`, with every
   implementation source listed in `identitydocumentservices_guest_sources.txt`.
2. Complete exact-ID `coverage.tsv` using only the five allowed statuses.
   `implemented` evidence has the exact form
   `test:full/identitydocumentservices/tests/agent/*Tests.swift#testName`; `declared` evidence has
   the exact form `source:full/identitydocumentservices/<product-source>.swift#Symbol`. Every cited
   test/function and product source/anchor must exist, and the product source
   must be listed in `identitydocumentservices_guest_sources.txt`.
3. Nonempty `IdentityDocumentServices.swift` and `README.md`, plus `oracle-questions.tsv` with
...
4. Top-level synchronous no-argument functions named `test*` in
   `tests/agent/*Tests.swift`, plus `tests/agent/IdentityDocumentServicesLoadSmoke.swift` whose
   exact content is `import IdentityDocumentServices`, one blank line, and
   `let frameworkLoadSmokeMarker = "IDENTITYDOCUMENTSERVICES_AGENT_RUNTIME_OK"`. The sealed gate derives a runner
   from `implemented` coverage, loads `libIdentityDocumentServices.dylib`, and calls each cited
   test exactly once before accepting marker-only output. The load/marker check
   is not behavioral evidence by itself.
5. If dependencies are declared, `tests/agent/IdentityDocumentServicesDependencyIdentity.swift`
