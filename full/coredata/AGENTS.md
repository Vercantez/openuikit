# CoreData framework fan-out rules

This directory is an isolated clean-room starting point for the Linux `CoreData`
port. Work only inside `full/coredata/`. Do not edit application
...
- Implement a real Linux module named `CoreData` and a loadable
  `libCoreData.dylib`. Put implementation Swift files in this framework
  directory, outside `tests/`, and list each one as a repo-relative path in
  `coredata_guest_sources.txt`.
- Create `coverage.tsv` with the exact header
  ...
  At least 132 rows must be `implemented` or `declared` for the `large-partitioned`
...
- Create a nonempty `README.md` describing what is real, fail-closed, and still
  deferred. The primary implementation file must be `CoreData.swift`.
- Create `tests/agent/CoreDataRuntime.swift`. It must import `CoreData`, exercise
  meaningful implemented behavior, and print exactly `COREDATA_AGENT_RUNTIME_OK` on success.
