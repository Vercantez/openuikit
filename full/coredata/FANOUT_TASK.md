# Port `CoreData` to Linux

Build a substantial, honest starting implementation of Apple's public `CoreData`
...
The immutable seed contains 1319 unique public precise identifiers and
1751 symbol-graph relationships. The acceptance floor is
132 nondeferred (`implemented` or `declared`) identifiers: min(150, max(50, ceil(10% of exact public symbols))).
...
Deliver all of the following in `full/coredata/`:

1. Linux Swift sources for module `CoreData` and `libCoreData.dylib`, with every
   implementation source listed in `coredata_guest_sources.txt`.
...
3. Nonempty `CoreData.swift` and `README.md`, plus `oracle-questions.tsv` with
   header `precise	question	risk	reason` and at least one question; use
   `module` only for cross-cutting items.
4. `tests/agent/CoreDataRuntime.swift`, meaningful focused tests, and the exact
   success marker `COREDATA_AGENT_RUNTIME_OK`.
