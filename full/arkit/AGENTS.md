# ARKit framework fan-out rules

This directory is an isolated clean-room starting point for the Linux `ARKit`
port. Work only inside `full/arkit/`. Do not edit application
...
- Implement a real Linux module named `ARKit` and a loadable
  `libARKit.dylib`. Put implementation Swift files in this framework
  directory, outside `tests/`, and list each one as a repo-relative path in
  `arkit_guest_sources.txt`.
...
- Create a nonempty `README.md` describing what is real, fail-closed, and still
  deferred. The primary implementation file must be `ARKit.swift`.
- Create `tests/agent/ARKitRuntime.swift`. It must import `ARKit`, exercise
  meaningful implemented behavior, and print exactly `ARKIT_AGENT_RUNTIME_OK` on success.
