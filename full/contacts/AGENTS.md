# Contacts framework fan-out rules

This directory is an isolated clean-room starting point for the Linux `Contacts`
port. Work only inside `full/contacts/`. Do not edit application
...
- Implement a real Linux module named `Contacts` and a loadable
  `libContacts.dylib`. Put implementation Swift files in this framework
  directory, outside `tests/`, and list each one as a repo-relative path in
  `contacts_guest_sources.txt`.
- Create `coverage.tsv` with the exact header
  ...
  At least 72 rows must be `implemented` or `declared` for the `large-partitioned`
...
- Create a nonempty `README.md` describing what is real, fail-closed, and still
  deferred. The primary implementation file must be `Contacts.swift`.
- Create `tests/agent/ContactsRuntime.swift`. It must import `Contacts`, exercise
  meaningful implemented behavior, and print exactly `CONTACTS_AGENT_RUNTIME_OK` on success.
