# Port `Contacts` to Linux

Build a substantial, honest starting implementation of Apple's public `Contacts`
...
The immutable seed contains 714 unique public precise identifiers and
819 symbol-graph relationships. The acceptance floor is
72 nondeferred (`implemented` or `declared`) identifiers: min(150, max(50, ceil(10% of exact public symbols))).
...
Deliver all of the following in `full/contacts/`:

1. Linux Swift sources for module `Contacts` and `libContacts.dylib`, with every
   implementation source listed in `contacts_guest_sources.txt`.
...
3. Nonempty `Contacts.swift` and `README.md`, plus `oracle-questions.tsv` with
   header `precise	question	risk	reason` and at least one question; use
   `module` only for cross-cutting items.
4. `tests/agent/ContactsRuntime.swift`, meaningful focused tests, and the exact
   success marker `CONTACTS_AGENT_RUNTIME_OK`.
