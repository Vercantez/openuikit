# PHASE framework fan-out rules

This directory is an isolated clean-room starting point for the Linux `PHASE`
port. Work only inside `full/phase/`. Do not edit application
...
- Implement a real Linux module named `PHASE` and a loadable
  `libPHASE.dylib`. Put implementation Swift files in this framework
  directory, outside `tests/`, and list each one as a repo-relative path in
  `phase_guest_sources.txt`.
- Create `coverage.tsv` with the exact header
  ...
  At least 64 rows must be `implemented` or `declared` for the `large-partitioned`
  ...
  `test:full/phase/tests/agent/*Tests.swift#testName`; define that exact `test*`
  function for the sealed runner to call. Every `declared` row must cite
  `source:full/phase/<product-source>.swift#Symbol`; the source must be listed in
  `phase_guest_sources.txt` and contain the exact identifier anchor. Deferred,
...
- Create a nonempty `README.md` describing what is real, fail-closed, and still
  deferred. The primary implementation file must be `PHASE.swift`.
- Put focused behavioral checks in `tests/agent/*Tests.swift` as top-level,
  synchronous, no-argument functions named `test*`. Create
  `tests/agent/PHASELoadSmoke.swift` with exactly these three logical lines
  (including the blank line): `import PHASE`, a blank line, and
  `let frameworkLoadSmokeMarker = "PHASE_AGENT_RUNTIME_OK"`. The sealed gate generates the
...
- When dependencies are listed below, create
  `tests/agent/PHASEDependencyIdentity.swift`. Import `PHASE` and every
  declared dependency and pass genuine dependency values through public
  `PHASE` APIs. This probe is for the clean EC2 integration build; the isolated
