...
Build a substantial, honest starting implementation of Apple's public `PHASE`
...
The immutable seed contains 639 unique public precise identifiers and
1170 symbol-graph relationships. The acceptance floor is
64 nondeferred (`implemented` or `declared`) identifiers: min(150, max(50, ceil(10% of exact public symbols))).
...
Deliver all of the following in `full/phase/`:

1. Linux Swift sources for module `PHASE` and `libPHASE.dylib`, with every
   implementation source listed in `phase_guest_sources.txt`.
2. Complete exact-ID `coverage.tsv` using only the five allowed statuses.
   `implemented` evidence has the exact form
   `test:full/phase/tests/agent/*Tests.swift#testName`; `declared` evidence has
   the exact form `source:full/phase/<product-source>.swift#Symbol`. Every cited
   test/function and product source/anchor must exist, and the product source
   must be listed in `phase_guest_sources.txt`.
3. Nonempty `PHASE.swift` and `README.md`, plus `oracle-questions.tsv` with
...
4. Top-level synchronous no-argument functions named `test*` in
   `tests/agent/*Tests.swift`, plus `tests/agent/PHASELoadSmoke.swift` whose
   exact content is `import PHASE`, one blank line, and
   `let frameworkLoadSmokeMarker = "PHASE_AGENT_RUNTIME_OK"`. The sealed gate derives a runner
   from `implemented` coverage, loads `libPHASE.dylib`, and calls each cited
   test exactly once before accepting marker-only output. The load/marker check
   is not behavioral evidence by itself.
5. If dependencies are declared, `tests/agent/PHASEDependencyIdentity.swift`
