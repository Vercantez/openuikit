# AudioUnit framework fan-out rules

This directory is an isolated clean-room starting point for the Linux `AudioUnit`
port. Work only inside `full/audiounit/`. Do not edit application
...
- Implement a real Linux module named `AudioUnit` and a loadable
  `libAudioUnit.dylib`. Put implementation Swift files in this framework
  directory, outside `tests/`, and list each one as a repo-relative path in
  `audiounit_guest_sources.txt`.
- Create `coverage.tsv` with the exact header
  ...
  At least 1 rows must be `implemented` or `declared` for the `legacy-adapter`
...
- Create a nonempty `README.md` describing what is real, fail-closed, and still
  deferred. The primary implementation file must be `AudioUnit.swift`.
- Create `tests/agent/AudioUnitRuntime.swift`. It must import `AudioUnit`, exercise
  meaningful implemented behavior, and print exactly `AUDIOUNIT_AGENT_RUNTIME_OK` on success.
