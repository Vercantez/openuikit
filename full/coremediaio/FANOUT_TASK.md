# Port `CoreMediaIO` to Linux

Build a substantial, honest starting implementation of Apple's public `CoreMediaIO`
...
The immutable seed contains 0 unique public precise identifiers and
0 symbol-graph relationships. The acceptance floor is
0 nondeferred (`implemented` or `declared`) identifiers: ceil(80% of exact public symbols).
...
Deliver all of the following in `full/coremediaio/`:

1. Linux Swift sources for module `CoreMediaIO` and `libCoreMediaIO.dylib`, with every
   implementation source listed in `coremediaio_guest_sources.txt`.
...
3. Nonempty `CoreMediaIO.swift` and `README.md`, plus `oracle-questions.tsv` with
   header `precise	question	risk	reason` and at least one question; use
   `module` only for cross-cutting items.
4. `tests/agent/CoreMediaIORuntime.swift`, meaningful focused tests, and the exact
   success marker `COREMEDIAIO_AGENT_RUNTIME_OK`.
