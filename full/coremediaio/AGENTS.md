# CoreMediaIO framework fan-out rules

This directory is an isolated clean-room starting point for the Linux `CoreMediaIO`
port. Work only inside `full/coremediaio/`. Do not edit application
...
- Implement a real Linux module named `CoreMediaIO` and a loadable
  `libCoreMediaIO.dylib`. Put implementation Swift files in this framework
  directory, outside `tests/`, and list each one as a repo-relative path in
  `coremediaio_guest_sources.txt`.
- Create `coverage.tsv` with the exact header
  ...
  At least 0 rows must be `implemented` or `declared` for the `legacy-adapter`
...
- Create a nonempty `README.md` describing what is real, fail-closed, and still
  deferred. The primary implementation file must be `CoreMediaIO.swift`.
- Create `tests/agent/CoreMediaIORuntime.swift`. It must import `CoreMediaIO`, exercise
  meaningful implemented behavior, and print exactly `COREMEDIAIO_AGENT_RUNTIME_OK` on success.
