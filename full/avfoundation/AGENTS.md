# AVFoundation framework fan-out rules

This directory is an isolated clean-room starting point for the Linux `AVFoundation`
port. Work only inside `full/avfoundation/`. Do not edit application
...
- Implement a real Linux module named `AVFoundation` and a loadable
  `libAVFoundation.dylib`. Put implementation Swift files in this framework
  directory, outside `tests/`, and list each one as a repo-relative path in
  `avfoundation_guest_sources.txt`.
- Create `coverage.tsv` with the exact header
  ...
  At least 2816 rows must be `implemented` or `declared` for the `medium-full`
  ...
  `test:full/avfoundation/tests/agent/*Tests.swift#testName`; define that exact `test*`
  function for the sealed runner to call. Every `declared` row must cite
  `source:full/avfoundation/<product-source>.swift#Symbol`; the source must be listed in
  `avfoundation_guest_sources.txt` and contain the exact identifier anchor. Deferred,
...
- Create a nonempty `README.md` describing what is real, fail-closed, and still
  deferred. The primary implementation file must be `AVFoundation.swift`.
- Put focused behavioral checks in `tests/agent/*Tests.swift` as top-level,
  synchronous, no-argument functions named `test*`. Create
  `tests/agent/AVFoundationLoadSmoke.swift` with exactly these three logical lines
  (including the blank line): `import AVFoundation`, a blank line, and
  `let frameworkLoadSmokeMarker = "AVFOUNDATION_AGENT_RUNTIME_OK"`. The sealed gate generates the
...
- When dependencies are listed below, create
  `tests/agent/AVFoundationDependencyIdentity.swift`. Import `AVFoundation` and every
  declared dependency and pass genuine dependency values through public
  `AVFoundation` APIs. This probe is for the clean EC2 integration build; the isolated
