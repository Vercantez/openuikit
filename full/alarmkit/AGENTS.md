# AlarmKit framework fan-out rules

This directory is an isolated clean-room starting point for the Linux `AlarmKit`
port. Work only inside `full/alarmkit/`. Do not edit application
...
- Implement a real Linux module named `AlarmKit` and a loadable
  `libAlarmKit.dylib`. Put implementation Swift files in this framework
  directory, outside `tests/`, and list each one as a repo-relative path in
  `alarmkit_guest_sources.txt`.
- Create `coverage.tsv` with the exact header
  ...
  At least 131 rows must be `implemented` or `declared` for the `medium-full`
...
- Create a nonempty `README.md` describing what is real, fail-closed, and still
  deferred. The primary implementation file must be `AlarmKit.swift`.
- Create `tests/agent/AlarmKitRuntime.swift`. It must import `AlarmKit`, exercise
  meaningful implemented behavior, and print exactly `ALARMKIT_AGENT_RUNTIME_OK` on success.
