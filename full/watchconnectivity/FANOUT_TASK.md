# Port `WatchConnectivity` to Linux

Build a substantial, honest starting implementation of Apple's public `WatchConnectivity`
...
The immutable seed contains 119 unique public precise identifiers and
161 symbol-graph relationships. The acceptance floor is
96 nondeferred (`implemented` or `declared`) identifiers: ceil(80% of exact public symbols).
...
Deliver all of the following in `full/watchconnectivity/`:

1. Linux Swift sources for module `WatchConnectivity` and `libWatchConnectivity.dylib`, with every
   implementation source listed in `watchconnectivity_guest_sources.txt`.
...
3. Nonempty `WatchConnectivity.swift` and `README.md`, plus `oracle-questions.tsv` with
   header `precise	question	risk	reason` and at least one question; use
   `module` only for cross-cutting items.
4. `tests/agent/WatchConnectivityRuntime.swift`, meaningful focused tests, and the exact
   success marker `WATCHCONNECTIVITY_AGENT_RUNTIME_OK`.
