# WatchConnectivity framework fan-out rules

This directory is an isolated clean-room starting point for the Linux `WatchConnectivity`
port. Work only inside `full/watchconnectivity/`. Do not edit application
...
- Implement a real Linux module named `WatchConnectivity` and a loadable
  `libWatchConnectivity.dylib`. Put implementation Swift files in this framework
  directory, outside `tests/`, and list each one as a repo-relative path in
  `watchconnectivity_guest_sources.txt`.
- Create `coverage.tsv` with the exact header
  ...
  At least 96 rows must be `implemented` or `declared` for the `leaf-full`
...
- Create a nonempty `README.md` describing what is real, fail-closed, and still
  deferred. The primary implementation file must be `WatchConnectivity.swift`.
- Create `tests/agent/WatchConnectivityRuntime.swift`. It must import `WatchConnectivity`, exercise
  meaningful implemented behavior, and print exactly `WATCHCONNECTIVITY_AGENT_RUNTIME_OK` on success.
