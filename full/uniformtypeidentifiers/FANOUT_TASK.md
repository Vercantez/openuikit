# Port `UniformTypeIdentifiers` to Linux

Build a substantial, honest starting implementation of Apple's public `UniformTypeIdentifiers`
...
The immutable seed contains 235 unique public precise identifiers and
272 symbol-graph relationships. The acceptance floor is
118 nondeferred (`implemented` or `declared`) identifiers: ceil(50% of exact public symbols).
...
Deliver all of the following in `full/uniformtypeidentifiers/`:

1. Linux Swift sources for module `UniformTypeIdentifiers` and `libUniformTypeIdentifiers.dylib`, with every
   implementation source listed in `uniformtypeidentifiers_guest_sources.txt`.
2. Complete exact-ID `coverage.tsv` using only the five allowed statuses.
   `implemented` evidence has the exact form
   `test:full/uniformtypeidentifiers/tests/agent/*Tests.swift#testName`; `declared` evidence has
   the exact form `source:full/uniformtypeidentifiers/<product-source>.swift#Symbol`. Every cited
   test/function and product source/anchor must exist, and the product source
   must be listed in `uniformtypeidentifiers_guest_sources.txt`.
3. Nonempty `UniformTypeIdentifiers.swift` and `README.md`, plus `oracle-questions.tsv` with
...
4. Top-level synchronous no-argument functions named `test*` in
   `tests/agent/*Tests.swift`, plus `tests/agent/UniformTypeIdentifiersLoadSmoke.swift` whose
   exact content is `import UniformTypeIdentifiers`, one blank line, and
   `let frameworkLoadSmokeMarker = "UNIFORMTYPEIDENTIFIERS_AGENT_RUNTIME_OK"`. The sealed gate derives a runner
   from `implemented` coverage, loads `libUniformTypeIdentifiers.dylib`, and calls each cited
   test exactly once before accepting marker-only output. The load/marker check
   is not behavioral evidence by itself.
5. If dependencies are declared, `tests/agent/UniformTypeIdentifiersDependencyIdentity.swift`
