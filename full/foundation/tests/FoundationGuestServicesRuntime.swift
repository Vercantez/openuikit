...
func emitDateFormatterRows() {
    ...
    for localeID in ["en_US_POSIX", "en_GB", "fr_FR"] {
        for zoneID in ["GMT", "America/Chicago"] {
            for pattern in patterns {
                let formatter = FoundationGuestServices.DateFormatter()
    ...
    for localeID in ["en_US_POSIX", "en_GB", "fr_FR"] {
        for dateStyle in [FoundationGuestServices.DateFormatter.Style.none, .short, .medium, .long, .full] {
            for timeStyle in [FoundationGuestServices.DateFormatter.Style.none, .short, .medium, .long] {
