# Port `AddressBook` to Linux

Build a substantial, honest starting implementation of Apple's public `AddressBook`
...
The immutable seed contains 225 unique public precise identifiers and
25 symbol-graph relationships. The acceptance floor is
180 nondeferred (`implemented` or `declared`) identifiers: ceil(80% of exact public symbols).
...
Deliver all of the following in `full/addressbook/`:

1. Linux Swift sources for module `AddressBook` and `libAddressBook.dylib`, with every
   implementation source listed in `addressbook_guest_sources.txt`.
2. Complete exact-ID `coverage.tsv` using only the five allowed statuses.
   `implemented` evidence has the exact form
   `test:full/addressbook/tests/agent/*Tests.swift#testName`; `declared` evidence has
   the exact form `source:full/addressbook/<product-source>.swift#Symbol`. Every cited
   test/function and product source/anchor must exist, and the product source
   must be listed in `addressbook_guest_sources.txt`.
3. Nonempty `AddressBook.swift` and `README.md`, plus `oracle-questions.tsv` with
...
4. Top-level synchronous no-argument functions named `test*` in
   `tests/agent/*Tests.swift`, plus `tests/agent/AddressBookLoadSmoke.swift` whose
   exact content is `import AddressBook`, one blank line, and
   `let frameworkLoadSmokeMarker = "ADDRESSBOOK_AGENT_RUNTIME_OK"`. The sealed gate derives a runner
   from `implemented` coverage, loads `libAddressBook.dylib`, and calls each cited
   test exactly once before accepting marker-only output. The load/marker check
   is not behavioral evidence by itself.
5. If dependencies are declared, `tests/agent/AddressBookDependencyIdentity.swift`
