# Port `EventKit` to Linux

Build a substantial, honest starting implementation of Apple's public `EventKit`
...
734 symbol-graph relationships. The acceptance floor is
...
Deliver all of the following in `full/eventkit/`:

1. Linux Swift sources for module `EventKit` and `libEventKit.dylib`, with every
   implementation source listed in `eventkit_guest_sources.txt`.
...
3. Nonempty `EventKit.swift` and `README.md`, plus `oracle-questions.tsv` with
   header `precise	question	risk	reason` and at least one question; use
   `module` only for cross-cutting items.
4. `tests/agent/EventKitRuntime.swift`, meaningful focused tests, and the exact
   success marker `EVENTKIT_AGENT_RUNTIME_OK`.
