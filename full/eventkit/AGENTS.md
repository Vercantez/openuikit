# EventKit framework fan-out rules

This directory is an isolated clean-room starting point for the Linux `EventKit`
port. Work only inside `full/eventkit/`. Do not edit application
...
- Implement a real Linux module named `EventKit` and a loadable
  `libEventKit.dylib`. Put implementation Swift files in this framework
  directory, outside `tests/`, and list each one as a repo-relative path in
  `eventkit_guest_sources.txt`.
...
- Create a nonempty `README.md` describing what is real, fail-closed, and still
  deferred. The primary implementation file must be `EventKit.swift`.
- Create `tests/agent/EventKitRuntime.swift`. It must import `EventKit`, exercise
  meaningful implemented behavior, and print exactly `EVENTKIT_AGENT_RUNTIME_OK` on success.
