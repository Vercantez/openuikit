# Port `WebKit` to Linux

Build a substantial, honest starting implementation of Apple's public `WebKit`
...
The immutable seed contains 2233 unique public precise identifiers and
2977 symbol-graph relationships. The acceptance floor is
1117 nondeferred (`implemented` or `declared`) identifiers: ceil(50% of exact public symbols).
...
Deliver all of the following in `full/webkit/`:

1. Linux Swift sources for module `WebKit` and `libWebKit.dylib`, with every
   implementation source listed in `webkit_guest_sources.txt`.
2. Complete exact-ID `coverage.tsv` using only the five allowed statuses.
   `implemented` evidence has the exact form
   `test:full/webkit/tests/agent/*Tests.swift#testName`; `declared` evidence has
   the exact form `source:full/webkit/<product-source>.swift#Symbol`. Every cited
   test/function and product source/anchor must exist, and the product source
   must be listed in `webkit_guest_sources.txt`.
3. Nonempty `WebKit.swift` and `README.md`, plus `oracle-questions.tsv` with
...
4. Top-level synchronous no-argument functions named `test*` in
   `tests/agent/*Tests.swift`, plus `tests/agent/WebKitLoadSmoke.swift` whose
   exact content is `import WebKit`, one blank line, and
   `let frameworkLoadSmokeMarker = "WEBKIT_AGENT_RUNTIME_OK"`. The sealed gate derives a runner
   from `implemented` coverage, loads `libWebKit.dylib`, and calls each cited
   test exactly once before accepting marker-only output. The load/marker check
   is not behavioral evidence by itself.
5. If dependencies are declared, `tests/agent/WebKitDependencyIdentity.swift`
