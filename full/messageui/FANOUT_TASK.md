# Port `MessageUI` to Linux

Build a substantial, honest starting implementation of Apple's public `MessageUI`
...
The immutable seed contains 84 unique public precise identifiers and
146 symbol-graph relationships. The acceptance floor is
42 nondeferred (`implemented` or `declared`) identifiers: ceil(50% of exact public symbols).
...
Deliver all of the following in `full/messageui/`:

1. Linux Swift sources for module `MessageUI` and `libMessageUI.dylib`, with every
   implementation source listed in `messageui_guest_sources.txt`.
2. Complete exact-ID `coverage.tsv` using only the five allowed statuses.
   `implemented` evidence has the exact form
   `test:full/messageui/tests/agent/*Tests.swift#testName`; `declared` evidence has
   the exact form `source:full/messageui/<product-source>.swift#Symbol`. Every cited
   test/function and product source/anchor must exist, and the product source
   must be listed in `messageui_guest_sources.txt`.
3. Nonempty `MessageUI.swift` and `README.md`, plus `oracle-questions.tsv` with
...
4. Top-level synchronous no-argument functions named `test*` in
   `tests/agent/*Tests.swift`, plus `tests/agent/MessageUILoadSmoke.swift` whose
   exact content is `import MessageUI`, one blank line, and
   `let frameworkLoadSmokeMarker = "MESSAGEUI_AGENT_RUNTIME_OK"`. The sealed gate derives a runner
   from `implemented` coverage, loads `libMessageUI.dylib`, and calls each cited
   test exactly once before accepting marker-only output. The load/marker check
   is not behavioral evidence by itself.
5. If dependencies are declared, `tests/agent/MessageUIDependencyIdentity.swift`
