# Port `SharedWithYouCore` to Linux

Build a substantial, honest starting implementation of Apple's public `SharedWithYouCore`
...
Deliver all of the following in `full/sharedwithyoucore/`:

1. Linux Swift sources for module `SharedWithYouCore` and `libSharedWithYouCore.dylib`, with every
   implementation source listed in `sharedwithyoucore_guest_sources.txt`.
2. Complete exact-ID `coverage.tsv` using only the five allowed statuses.
   `implemented` evidence has the exact form
   `test:full/sharedwithyoucore/tests/agent/*Tests.swift#testName`; `declared` evidence has
   the exact form `source:full/sharedwithyoucore/<product-source>.swift#Symbol`. Every cited
   test/function and product source/anchor must exist, and the product source
   must be listed in `sharedwithyoucore_guest_sources.txt`.
3. Nonempty `SharedWithYouCore.swift` and `README.md`, plus `oracle-questions.tsv` with
...
4. Top-level synchronous no-argument functions named `test*` in
   `tests/agent/*Tests.swift`, plus `tests/agent/SharedWithYouCoreLoadSmoke.swift` whose
   exact content is `import SharedWithYouCore`, one blank line, and
   `let frameworkLoadSmokeMarker = "SHAREDWITHYOUCORE_AGENT_RUNTIME_OK"`. The sealed gate derives a runner
   from `implemented` coverage, loads `libSharedWithYouCore.dylib`, and calls each cited
   test exactly once before accepting marker-only output. The load/marker check
   is not behavioral evidence by itself.
5. If dependencies are declared, `tests/agent/SharedWithYouCoreDependencyIdentity.swift`
