# Port `MediaToolbox` to Linux

Build a substantial, honest starting implementation of Apple's public `MediaToolbox`
...
The immutable seed contains 32 unique public precise identifiers and
15 symbol-graph relationships. The acceptance floor is
16 nondeferred (`implemented` or `declared`) identifiers: ceil(50% of exact public symbols).
...
Deliver all of the following in `full/mediatoolbox/`:

1. Linux Swift sources for module `MediaToolbox` and `libMediaToolbox.dylib`, with every
   implementation source listed in `mediatoolbox_guest_sources.txt`.
2. Complete exact-ID `coverage.tsv` using only the five allowed statuses.
   `implemented` evidence has the exact form
   `test:full/mediatoolbox/tests/agent/*Tests.swift#testName`; `declared` evidence has
   the exact form `source:full/mediatoolbox/<product-source>.swift#Symbol`. Every cited
   test/function and product source/anchor must exist, and the product source
   must be listed in `mediatoolbox_guest_sources.txt`.
3. Nonempty `MediaToolbox.swift` and `README.md`, plus `oracle-questions.tsv` with
...
4. Top-level synchronous no-argument functions named `test*` in
   `tests/agent/*Tests.swift`, plus `tests/agent/MediaToolboxLoadSmoke.swift` whose
   exact content is `import MediaToolbox`, one blank line, and
   `let frameworkLoadSmokeMarker = "MEDIATOOLBOX_AGENT_RUNTIME_OK"`. The sealed gate derives a runner
   from `implemented` coverage, loads `libMediaToolbox.dylib`, and calls each cited
   test exactly once before accepting marker-only output. The load/marker check
   is not behavioral evidence by itself.
5. If dependencies are declared, `tests/agent/MediaToolboxDependencyIdentity.swift`
