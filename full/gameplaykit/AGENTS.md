# GameplayKit framework fan-out rules

This directory is an isolated clean-room starting point for the Linux `GameplayKit`
port. Work only inside `full/gameplaykit/`. Do not edit application
...
- Implement a real Linux module named `GameplayKit` and a loadable
  `libGameplayKit.dylib`. Put implementation Swift files in this framework
  directory, outside `tests/`, and list each one as a repo-relative path in
  `gameplaykit_guest_sources.txt`.
- Create `coverage.tsv` with the exact header
  ...
  At least 246 rows must be `implemented` or `declared` for the `medium-full`
...
- Create a nonempty `README.md` describing what is real, fail-closed, and still
  deferred. The primary implementation file must be `GameplayKit.swift`.
- Create `tests/agent/GameplayKitRuntime.swift`. It must import `GameplayKit`, exercise
  meaningful implemented behavior, and print exactly `GAMEPLAYKIT_AGENT_RUNTIME_OK` on success.
