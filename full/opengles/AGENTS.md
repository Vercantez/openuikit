# OpenGLES framework fan-out rules

This directory is an isolated clean-room starting point for the Linux `OpenGLES`
port. Work only inside `full/opengles/`. Do not edit application
...
- Implement a real Linux module named `OpenGLES` and a loadable
  `libOpenGLES.dylib`. Put implementation Swift files in this framework
  directory, outside `tests/`, and list each one as a repo-relative path in
  `opengles_guest_sources.txt`.
- Create `coverage.tsv` with the exact header
  ...
  At least 1270 rows must be `implemented` or `declared` for the `legacy-adapter`
  ...
  `test:full/opengles/tests/agent/*Tests.swift#testName`; define that exact `test*`
  function for the sealed runner to call. Every `declared` row must cite
  `source:full/opengles/<product-source>.swift#Symbol`; the source must be listed in
  `opengles_guest_sources.txt` and contain the exact identifier anchor. Deferred,
...
- Create a nonempty `README.md` describing what is real, fail-closed, and still
  deferred. The primary implementation file must be `OpenGLES.swift`.
- Put focused behavioral checks in `tests/agent/*Tests.swift` as top-level,
  synchronous, no-argument functions named `test*`. Create
  `tests/agent/OpenGLESLoadSmoke.swift` with exactly these three logical lines
  (including the blank line): `import OpenGLES`, a blank line, and
  `let frameworkLoadSmokeMarker = "OPENGLES_AGENT_RUNTIME_OK"`. The sealed gate generates the
...
- When dependencies are listed below, create
  `tests/agent/OpenGLESDependencyIdentity.swift`. Import `OpenGLES` and every
  declared dependency and pass genuine dependency values through public
  `OpenGLES` APIs. This probe is for the clean EC2 integration build; the isolated
