...
func testLAContextFailClosed() {
    ...
    context.evaluatePolicy(
        ...
        replies.store(replies.load() + 1)
    }
    precondition(replies.load() == 1)
...
func testLARightFailClosed() {
    ...
    let canAuthorize = LALocked<(any Error)?>(nil)
    let canAuthorizeCalls = LALocked(0)
    right.checkCanAuthorize { error in
        canAuthorize.store(error)
        canAuthorizeCalls.store(canAuthorizeCalls.load() + 1)
    }
    precondition(canAuthorizeCalls.load() == 1)
    precondition((canAuthorize.load() as? LAError)?.code == .biometryNotAvailable)
...
func testLARightStoreFailClosed() {
    ...
    let removeAll = LALocked<(any Error)?>(nil)
    let removeAllCalls = LALocked(0)
    store.removeAllRights { error in
        removeAll.store(error)
        removeAllCalls.store(removeAllCalls.load() + 1)
    }
    precondition(removeAllCalls.load() == 1)
    precondition((removeAll.load() as? LAError)?.code == .biometryNotAvailable)
}
...
/// Schema-v2 host compilation uses a generated top-level runner. Swift 6.2.4
/// parses multi-file executables as libraries, so `*Tests.swift` cannot be
/// compiled beside that runner. These `test*` functions are the focused
/// checks; invoke them through this probe or a later single-file harness.
func localAuthenticationRuntimeProbe() {
    testLAErrorDomain()
    testLAErrorCodes()
    testLAErrorEqualityAndHash()
    testLAErrorPatternMatch()
    testLAErrorNSErrorBridge()
    testLAErrorProtocolConformance()
    testLAPolicyRawValues()
    testLABiometryTypeRawValues()
    testLACompanionTypeRawValues()
    testLACredentialTypeRawValues()
    testLAAccessControlOperationRawValues()
    testLARightStateRawValues()
    testPublicDefinesMacros()
    testLAContextFailClosed()
    testLAContextInvalidate()
    testLAContextCredentials()
    testLAContextDomainState()
    testLAAuthenticationRequirement()
    testLARightFailClosed()
    testLARightStoreFailClosed()
    testLAEnvironmentFailClosed()
}
