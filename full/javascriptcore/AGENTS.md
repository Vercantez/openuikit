# JavaScriptCore framework fan-out rules

This directory is an isolated clean-room starting point for the Linux `JavaScriptCore`
port. Work only inside `full/javascriptcore/`. Do not edit application
...
- Implement a real Linux module named `JavaScriptCore` and a loadable
  `libJavaScriptCore.dylib`. Put implementation Swift files in this framework
  directory, outside `tests/`, and list each one as a repo-relative path in
  `javascriptcore_guest_sources.txt`.
- Create `coverage.tsv` with the exact header
  ...
  At least 179 rows must be `implemented` or `declared` for the `medium-full`
...
- Create a nonempty `README.md` describing what is real, fail-closed, and still
  deferred. The primary implementation file must be `JavaScriptCore.swift`.
- Create `tests/agent/JavaScriptCoreRuntime.swift`. It must import `JavaScriptCore`, exercise
  meaningful implemented behavior, and print exactly `JAVASCRIPTCORE_AGENT_RUNTIME_OK` on success.
