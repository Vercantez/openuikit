# Port `JavaScriptCore` to Linux

Build a substantial, honest starting implementation of Apple's public `JavaScriptCore`
...
The immutable seed contains 357 unique public precise identifiers and
222 symbol-graph relationships. The acceptance floor is
179 nondeferred (`implemented` or `declared`) identifiers: ceil(50% of exact public symbols).
...
Deliver all of the following in `full/javascriptcore/`:

1. Linux Swift sources for module `JavaScriptCore` and `libJavaScriptCore.dylib`, with every
   implementation source listed in `javascriptcore_guest_sources.txt`.
...
3. Nonempty `JavaScriptCore.swift` and `README.md`, plus `oracle-questions.tsv` with
   header `precise	question	risk	reason` and at least one question; use
   `module` only for cross-cutting items.
4. `tests/agent/JavaScriptCoreRuntime.swift`, meaningful focused tests, and the exact
   success marker `JAVASCRIPTCORE_AGENT_RUNTIME_OK`.
