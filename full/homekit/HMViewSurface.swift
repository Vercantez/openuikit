...
/// `_HomeKit_SwiftUI` overlay: `CameraView` renders a particular
/// `HMCameraSource`. Darwin marks it `@MainActor @preconcurrency`; the
/// isolated-host `View` lookalike is nonisolated, so Linux omits the actor
/// annotation to compile without SwiftUI (FamilyControls playbook). Linux
/// stores the source and renders `EmptyView`; it never presents camera UI.
///
/// Identity `View` modifiers synthesized onto `CameraView` compile as no-op
/// `Self` returns when SwiftUI is absent. Each member takes a single
/// optional `Any` parameter so identity calls compile with zero arguments.
/// Linux renders EmptyView; these pin the no-op identity behavior.
enum _HomeKitViewSurfaceMarker {}
...
/// Linux-host `CameraView`. Stores the `HMCameraSource` to render and
/// exposes an `EmptyView` body. Camera RTP/HAP sessions stay fail-closed.
public struct CameraView: View {
    public typealias Body = EmptyView

    public let source: HMCameraSource

    public init(source: HMCameraSource) {
        self.source = source
    }
