# HomeKit framework fan-out rules

This directory is an isolated clean-room starting point for the Linux `HomeKit`
port. Work only inside `full/homekit/`. Do not edit application
...
- Implement a real Linux module named `HomeKit` and a loadable
  `libHomeKit.dylib`. Put implementation Swift files in this framework
  directory, outside `tests/`, and list each one as a repo-relative path in
  `homekit_guest_sources.txt`.
- Create `coverage.tsv` with the exact header
  ...
  `test:full/homekit/tests/agent/*Tests.swift#testName`; define that exact `test*`
  function for the sealed runner to call. Every `declared` row must cite
  `source:full/homekit/<product-source>.swift#Symbol`; the source must be listed in
  `homekit_guest_sources.txt` and contain the exact identifier anchor. Deferred,
...
- Create a nonempty `README.md` describing what is real, fail-closed, and still
  deferred. The primary implementation file must be `HomeKit.swift`.
- Put focused behavioral checks in `tests/agent/*Tests.swift` as top-level,
  synchronous, no-argument functions named `test*`. Create
  `tests/agent/HomeKitLoadSmoke.swift` with exactly these three logical lines
  (including the blank line): `import HomeKit`, a blank line, and
  `let frameworkLoadSmokeMarker = "HOMEKIT_AGENT_RUNTIME_OK"`. The sealed gate generates the
...
- When dependencies are listed below, create
  `tests/agent/HomeKitDependencyIdentity.swift`. Import `HomeKit` and every
  declared dependency and pass genuine dependency values through public
  `HomeKit` APIs. This probe is for the clean EC2 integration build; the isolated
