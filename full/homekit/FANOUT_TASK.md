# Port `HomeKit` to Linux

Build a substantial, honest starting implementation of Apple's public `HomeKit`
...
The immutable seed contains 2295 unique public precise identifiers and
2979 symbol-graph relationships. The acceptance floor is
...
Deliver all of the following in `full/homekit/`:

1. Linux Swift sources for module `HomeKit` and `libHomeKit.dylib`, with every
   implementation source listed in `homekit_guest_sources.txt`.
2. Complete exact-ID `coverage.tsv` using only the five allowed statuses.
   `implemented` evidence has the exact form
   `test:full/homekit/tests/agent/*Tests.swift#testName`; `declared` evidence has
   the exact form `source:full/homekit/<product-source>.swift#Symbol`. Every cited
   test/function and product source/anchor must exist, and the product source
   must be listed in `homekit_guest_sources.txt`.
3. Nonempty `HomeKit.swift` and `README.md`, plus `oracle-questions.tsv` with
...
4. Top-level synchronous no-argument functions named `test*` in
   `tests/agent/*Tests.swift`, plus `tests/agent/HomeKitLoadSmoke.swift` whose
   exact content is `import HomeKit`, one blank line, and
   `let frameworkLoadSmokeMarker = "HOMEKIT_AGENT_RUNTIME_OK"`. The sealed gate derives a runner
   from `implemented` coverage, loads `libHomeKit.dylib`, and calls each cited
   test exactly once before accepting marker-only output. The load/marker check
   is not behavioral evidence by itself.
5. If dependencies are declared, `tests/agent/HomeKitDependencyIdentity.swift`
