# AddressBookUI framework fan-out rules

This directory is an isolated clean-room starting point for the Linux `AddressBookUI`
port. Work only inside `full/addressbookui/`. Do not edit application
...
- Implement a real Linux module named `AddressBookUI` and a loadable
  `libAddressBookUI.dylib`. Put implementation Swift files in this framework
  directory, outside `tests/`, and list each one as a repo-relative path in
  `addressbookui_guest_sources.txt`.
- Create `coverage.tsv` with the exact header
  ...
  At least 53 rows must be `implemented` or `declared` for the `legacy-adapter`
  ...
  `test:full/addressbookui/tests/agent/*Tests.swift#testName`; define that exact `test*`
  function for the sealed runner to call. Every `declared` row must cite
  `source:full/addressbookui/<product-source>.swift#Symbol`; the source must be listed in
  `addressbookui_guest_sources.txt` and contain the exact identifier anchor. Deferred,
...
- Create a nonempty `README.md` describing what is real, fail-closed, and still
  deferred. The primary implementation file must be `AddressBookUI.swift`.
- Put focused behavioral checks in `tests/agent/*Tests.swift` as top-level,
  synchronous, no-argument functions named `test*`. Create
  `tests/agent/AddressBookUILoadSmoke.swift` with exactly these three logical lines
  (including the blank line): `import AddressBookUI`, a blank line, and
  `let frameworkLoadSmokeMarker = "ADDRESSBOOKUI_AGENT_RUNTIME_OK"`. The sealed gate generates the
...
- When dependencies are listed below, create
  `tests/agent/AddressBookUIDependencyIdentity.swift`. Import `AddressBookUI` and every
  declared dependency and pass genuine dependency values through public
  `AddressBookUI` APIs. This probe is for the clean EC2 integration build; the isolated
