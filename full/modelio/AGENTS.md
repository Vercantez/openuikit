# ModelIO framework fan-out rules

This directory is an isolated clean-room starting point for the Linux `ModelIO`
port. Work only inside `full/modelio/`. Do not edit application
...
- Implement a real Linux module named `ModelIO` and a loadable
  `libModelIO.dylib`. Put implementation Swift files in this framework
  directory, outside `tests/`, and list each one as a repo-relative path in
  `modelio_guest_sources.txt`.
- Create `coverage.tsv` with the exact header
  ...
  At least 94 rows must be `implemented` or `declared` for the `large-partitioned`
  ...
  `test:full/modelio/tests/agent/*Tests.swift#testName`; define that exact `test*`
  function for the sealed runner to call. Every `declared` row must cite
  `source:full/modelio/<product-source>.swift#Symbol`; the source must be listed in
  `modelio_guest_sources.txt` and contain the exact identifier anchor. Deferred,
...
- Create a nonempty `README.md` describing what is real, fail-closed, and still
  deferred. The primary implementation file must be `ModelIO.swift`.
- Put focused behavioral checks in `tests/agent/*Tests.swift` as top-level,
  synchronous, no-argument functions named `test*`. Create
  `tests/agent/ModelIOLoadSmoke.swift` with exactly these three logical lines
  (including the blank line): `import ModelIO`, a blank line, and
  `let frameworkLoadSmokeMarker = "MODELIO_AGENT_RUNTIME_OK"`. The sealed gate generates the
...
- When dependencies are listed below, create
  `tests/agent/ModelIODependencyIdentity.swift`. Import `ModelIO` and every
  declared dependency and pass genuine dependency values through public
  `ModelIO` APIs. This probe is for the clean EC2 integration build; the isolated
