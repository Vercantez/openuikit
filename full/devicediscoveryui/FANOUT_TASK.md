...
2. Complete exact-ID `coverage.tsv` using only the five allowed statuses.
   ...
   `test:full/devicediscoveryui/tests/agent/*Tests.swift#testName`; `declared` evidence has
   the exact form `source:full/devicediscoveryui/<product-source>.swift#Symbol`. Every cited
   test/function and product source/anchor must exist, and the product source
   must be listed in `devicediscoveryui_guest_sources.txt`.
...
4. Top-level synchronous no-argument functions named `test*` in
   `tests/agent/*Tests.swift`, plus `tests/agent/DeviceDiscoveryUILoadSmoke.swift` whose
   exact content is `import DeviceDiscoveryUI`, one blank line, and
   `let frameworkLoadSmokeMarker = "DEVICEDISCOVERYUI_AGENT_RUNTIME_OK"`. The sealed gate derives a runner
   from `implemented` coverage, loads `libDeviceDiscoveryUI.dylib`, and calls each cited
   test exactly once before accepting marker-only output. The load/marker check
   is not behavioral evidence by itself.
5. If dependencies are declared, `tests/agent/DeviceDiscoveryUIDependencyIdentity.swift`
