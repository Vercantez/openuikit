# UserNotificationsUI framework fan-out rules

This directory is an isolated clean-room starting point for the Linux `UserNotificationsUI`
port. Work only inside `full/usernotificationsui/`. Do not edit application
...
- Implement a real Linux module named `UserNotificationsUI` and a loadable
  `libUserNotificationsUI.dylib`. Put implementation Swift files in this framework
  directory, outside `tests/`, and list each one as a repo-relative path in
  `usernotificationsui_guest_sources.txt`.
...
- Create a nonempty `README.md` describing what is real, fail-closed, and still
  deferred. The primary implementation file must be `UserNotificationsUI.swift`.
- Create `tests/agent/UserNotificationsUIRuntime.swift`. It must import `UserNotificationsUI`, exercise
  meaningful implemented behavior, and print exactly `USERNOTIFICATIONSUI_AGENT_RUNTIME_OK` on success.
