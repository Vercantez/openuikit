...
Build a substantial, honest starting implementation of Apple's public `Metal`
...
The immutable seed contains 4547 unique public precise identifiers and
6394 symbol-graph relationships. The acceptance floor is
...
Deliver all of the following in `full/metal/`:

1. Linux Swift sources for module `Metal` and `libMetal.dylib`, with every
   implementation source listed in `metal_guest_sources.txt`.
...
3. Nonempty `Metal.swift` and `README.md`, plus `oracle-questions.tsv` with
   header `precise	question	risk	reason` and at least one question; use
   `module` only for cross-cutting items.
4. `tests/agent/MetalRuntime.swift`, meaningful focused tests, and the exact
   success marker `METAL_AGENT_RUNTIME_OK`.
