# Metal framework fan-out rules

This directory is an isolated clean-room starting point for the Linux `Metal`
port. Work only inside `full/metal/`. Do not edit application
...
- Implement a real Linux module named `Metal` and a loadable
  `libMetal.dylib`. Put implementation Swift files in this framework
  directory, outside `tests/`, and list each one as a repo-relative path in
  `metal_guest_sources.txt`.
...
- Create a nonempty `README.md` describing what is real, fail-closed, and still
  deferred. The primary implementation file must be `Metal.swift`.
- Create `tests/agent/MetalRuntime.swift`. It must import `Metal`, exercise
  meaningful implemented behavior, and print exactly `METAL_AGENT_RUNTIME_OK` on success.
