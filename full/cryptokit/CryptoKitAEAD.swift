...
private func _aesMul(_ a: UInt8, _ b: UInt8) -> UInt8 {
    var left = a
    var right = b
    var product: UInt8 = 0
    for _ in 0..<8 {
...
private func _aesDecryptBlock(_ input: [UInt8], roundKeys: [[UInt8]]) -> [UInt8] {
    var state = input
    func addRoundKey(_ round: Int) {
        for index in 0..<16 { state[index] ^= roundKeys[round][index] }
    }
    func invSubBytes() {
        for index in 0..<16 { state[index] = _aesInvSBox[Int(state[index])] }
    }
    func invShiftRows() {
        state = [
    ...
    func invMixColumns() {
        ...
        for column in 0..<4 {
            let i = column * 4
            let a = state[i], b = state[i + 1], c = state[i + 2], d = state[i + 3]
            mixed[i] = _aesMul(a, 0x0e) ^ _aesMul(b, 0x0b) ^ _aesMul(c, 0x0d) ^ _aesMul(d, 0x09)
            mixed[i + 1] = _aesMul(a, 0x09) ^ _aesMul(b, 0x0e) ^ _aesMul(c, 0x0b) ^ _aesMul(d, 0x0d)
            mixed[i + 2] = _aesMul(a, 0x0d) ^ _aesMul(b, 0x09) ^ _aesMul(c, 0x0e) ^ _aesMul(d, 0x0b)
            mixed[i + 3] = _aesMul(a, 0x0b) ^ _aesMul(b, 0x0d) ^ _aesMul(c, 0x09) ^ _aesMul(d, 0x0e)
        }
        state = mixed
    }
    addRoundKey(roundKeys.count - 1)
...
private func _aesKeyUnwrap(kek: [UInt8], wrapped: [UInt8]) throws -> [UInt8] {
    // RFC 3394 unwrap. Measured against the 128-bit KEK / 128-bit key vector
    // in RFC 3394 §4.1: wrap 1FA68B0A8112B447AEF34BD8FB5A7B829D3E862371D2CFE5.
    let roundKeys = _aesExpandKey(kek)
    let n = (wrapped.count / 8) - 1
    var a: UInt64 = 0
    for byte in 0..<8 { a = (a << 8) | UInt64(wrapped[byte]) }
    var r = (0..<n).map { index -> UInt64 in
        var word: UInt64 = 0
        for byte in 0..<8 { word = (word << 8) | UInt64(wrapped[(index + 1) * 8 + byte]) }
        return word
    }
    for j in stride(from: 5, through: 0, by: -1) {
        for i in stride(from: n - 1, through: 0, by: -1) {
            ...
            let decrypted = _aesDecryptBlock(block, roundKeys: roundKeys)
            a = 0
            for byte in 0..<8 { a = (a << 8) | UInt64(decrypted[byte]) }
            r[i] = 0
            for byte in 8..<16 { r[i] = (r[i] << 8) | UInt64(decrypted[byte]) }
        }
    }
    guard a == 0xa6a6a6a6a6a6a6a6 else { throw CryptoKitError.unwrapFailure }
    var output: [UInt8] = []
    for word in r {
        var value = word
        var chunk = [UInt8](repeating: 0, count: 8)
