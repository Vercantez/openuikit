...
func testHostDispatcherRoutesHandlers() {
    ...
    precondition(fallback is INIntentHandlerProviding)
}
...
func testNSCodingRoundTrips() {
    ...
    precondition(depthArchiveRoundTrip(INBalanceAmount()) is INBalanceAmount)
    precondition(depthArchiveRoundTrip(INBillDetails()) is INBillDetails)
    let payee = INBillPayee()
    payee.accountNumber = "123"
    precondition(depthArchiveRoundTrip(payee).accountNumber == "123")
    precondition(depthArchiveRoundTrip(INBoatTrip()) is INBoatTrip)
    precondition(depthArchiveRoundTrip(INBusTrip()) is INBusTrip)
    ...
    precondition(depthArchiveRoundTrip(INCallRecordFilter(participants: nil, callTypes: [], callCapability: .unknown)) is INCallRecordFilter)
    let car = INCar()
    car.make = "Open"
    ...
    precondition(depthArchiveRoundTrip(INIntentDonationMetadata()) is INIntentDonationMetadata)
    let response = INIntentResponse()
    precondition(depthArchiveRoundTrip(response) is INIntentResponse)
    ...
    precondition(depthArchiveRoundTrip(INPaymentAccount()) is INPaymentAccount)
    precondition(depthArchiveRoundTrip(INPaymentAmount()) is INPaymentAmount)
    ...
    precondition(depthArchiveRoundTrip(INPaymentRecord()) is INPaymentRecord)
    let handle = INPersonHandle(value: "a@b.c", type: .emailAddress, label: .work)
    precondition(depthArchiveRoundTrip(handle).value == "a@b.c")
    let option = INRideOption(name: "Pool", estimatedPickupDate: Date(timeIntervalSince1970: 0))
    precondition(depthArchiveRoundTrip(option).name == "Pool")
    precondition(depthArchiveRoundTrip(INPriceRange()) is INPriceRange)
    precondition(depthArchiveRoundTrip(INRecurrenceRule()) is INRecurrenceRule)
    precondition(depthArchiveRoundTrip(INRelevanceProvider()) is INRelevanceProvider)
    ...
    precondition(depthArchiveRoundTrip(INReservationAction()) is INReservationAction)
    let restaurant = INRestaurant()
    restaurant.name = "Cafe"
    precondition(depthArchiveRoundTrip(restaurant).name == "Cafe")
    precondition(depthArchiveRoundTrip(INRestaurantGuestDisplayPreferences()) is INRestaurantGuestDisplayPreferences)
    ...
    precondition(depthArchiveRoundTrip(INRideFareLineItem()) is INRideFareLineItem)
    ...
    precondition(depthArchiveRoundTrip(INSpatialEventTrigger()) is INSpatialEventTrigger)
    let sticker = INSticker(type: .emoji, emoji: "⭐")
    precondition(depthArchiveRoundTrip(sticker).emoji == "⭐")
    precondition(depthArchiveRoundTrip(INTask()) is INTask)
    precondition(depthArchiveRoundTrip(INTaskList()) is INTaskList)
    precondition(depthArchiveRoundTrip(INTemporalEventTrigger()) is INTemporalEventTrigger)
    ...
    precondition(depthArchiveRoundTrip(INTicketedEvent()) is INTicketedEvent)
    let train = INTrainTrip()
    train.trainNumber = "1"
    precondition(depthArchiveRoundTrip(train).trainNumber == "1")
    precondition(depthArchiveRoundTrip(INUserContext()) is INUserContext)
