# WiFiAware framework fan-out rules

This directory is an isolated clean-room starting point for the Linux `WiFiAware`
port. Work only inside `full/wifiaware/`. Do not edit application
...
- Implement a real Linux module named `WiFiAware` and a loadable
  `libWiFiAware.dylib`. Put implementation Swift files in this framework
  directory, outside `tests/`, and list each one as a repo-relative path in
  `wifiaware_guest_sources.txt`.
...
- Create a nonempty `README.md` describing what is real, fail-closed, and still
  deferred. The primary implementation file must be `WiFiAware.swift`.
- Create `tests/agent/WiFiAwareRuntime.swift`. It must import `WiFiAware`, exercise
  meaningful implemented behavior, and print exactly `WIFIAWARE_AGENT_RUNTIME_OK` on success.
