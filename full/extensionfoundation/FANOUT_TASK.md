# Port `ExtensionFoundation` to Linux

Build a substantial, honest starting implementation of Apple's public `ExtensionFoundation`
...
The immutable seed contains 81 unique public precise identifiers and
106 symbol-graph relationships. The acceptance floor is
65 nondeferred (`implemented` or `declared`) identifiers: ceil(80% of exact public symbols).
...
Deliver all of the following in `full/extensionfoundation/`:

1. Linux Swift sources for module `ExtensionFoundation` and `libExtensionFoundation.dylib`, with every
   implementation source listed in `extensionfoundation_guest_sources.txt`.
2. Complete exact-ID `coverage.tsv` using only the five allowed statuses.
   `implemented` evidence has the exact form
   `test:full/extensionfoundation/tests/agent/*Tests.swift#testName`; `declared` evidence has
   the exact form `source:full/extensionfoundation/<product-source>.swift#Symbol`. Every cited
   test/function and product source/anchor must exist, and the product source
   must be listed in `extensionfoundation_guest_sources.txt`.
3. Nonempty `ExtensionFoundation.swift` and `README.md`, plus `oracle-questions.tsv` with
...
4. Top-level synchronous no-argument functions named `test*` in
   `tests/agent/*Tests.swift`, plus `tests/agent/ExtensionFoundationLoadSmoke.swift` whose
   exact content is `import ExtensionFoundation`, one blank line, and
   `let frameworkLoadSmokeMarker = "EXTENSIONFOUNDATION_AGENT_RUNTIME_OK"`. The sealed gate derives a runner
   from `implemented` coverage, loads `libExtensionFoundation.dylib`, and calls each cited
   test exactly once before accepting marker-only output. The load/marker check
   is not behavioral evidence by itself.
5. If dependencies are declared, `tests/agent/ExtensionFoundationDependencyIdentity.swift`
