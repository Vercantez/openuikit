# CoreNFC framework fan-out rules

This directory is an isolated clean-room starting point for the Linux `CoreNFC`
port. Work only inside `full/corenfc/`. Do not edit application
...
- Implement a real Linux module named `CoreNFC` and a loadable
  `libCoreNFC.dylib`. Put implementation Swift files in this framework
  directory, outside `tests/`, and list each one as a repo-relative path in
  `corenfc_guest_sources.txt`.
- Create `coverage.tsv` with the exact header
  ...
  At least 59 rows must be `implemented` or `declared` for the `large-partitioned`
...
- Create a nonempty `README.md` describing what is real, fail-closed, and still
  deferred. The primary implementation file must be `CoreNFC.swift`.
- Create `tests/agent/CoreNFCRuntime.swift`. It must import `CoreNFC`, exercise
  meaningful implemented behavior, and print exactly `CORENFC_AGENT_RUNTIME_OK` on success.
