# FileProvider framework fan-out rules

This directory is an isolated clean-room starting point for the Linux `FileProvider`
port. Work only inside `full/fileprovider/`. Do not edit application
...
- Implement a real Linux module named `FileProvider` and a loadable
  `libFileProvider.dylib`. Put implementation Swift files in this framework
...
- Create `coverage.tsv` with the exact header
  ...
  At least 57 rows must be `implemented` or `declared` for the `large-partitioned`
...
- Create a nonempty `README.md` describing what is real, fail-closed, and still
  deferred. The primary implementation file must be `FileProvider.swift`.
- Create `tests/agent/FileProviderRuntime.swift`. It must import `FileProvider`, exercise
  meaningful implemented behavior, and print exactly `FILEPROVIDER_AGENT_RUNTIME_OK` on success.
