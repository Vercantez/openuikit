# MetalKit framework fan-out rules

This directory is an isolated clean-room starting point for the Linux `MetalKit`
port. Work only inside `full/metalkit/`. Do not edit application
...
- Implement a real Linux module named `MetalKit` and a loadable
  `libMetalKit.dylib`. Put implementation Swift files in this framework
  directory, outside `tests/`, and list each one as a repo-relative path in
  `metalkit_guest_sources.txt`.
...
- Create a nonempty `README.md` describing what is real, fail-closed, and still
  deferred. The primary implementation file must be `MetalKit.swift`.
- Create `tests/agent/MetalKitRuntime.swift`. It must import `MetalKit`, exercise
  meaningful implemented behavior, and print exactly `METALKIT_AGENT_RUNTIME_OK` on success.
