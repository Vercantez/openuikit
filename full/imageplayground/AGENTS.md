# ImagePlayground framework fan-out rules

This directory is an isolated clean-room starting point for the Linux `ImagePlayground`
port. Work only inside `full/imageplayground/`. Do not edit application
...
- Implement a real Linux module named `ImagePlayground` and a loadable
  `libImagePlayground.dylib`. Put implementation Swift files in this framework
  directory, outside `tests/`, and list each one as a repo-relative path in
  `imageplayground_guest_sources.txt`.
- Create `coverage.tsv` with the exact header
  ...
  At least 45 rows must be `implemented` or `declared` for the `medium-full`
...
- Create a nonempty `README.md` describing what is real, fail-closed, and still
  deferred. The primary implementation file must be `ImagePlayground.swift`.
- Create `tests/agent/ImagePlaygroundRuntime.swift`. It must import `ImagePlayground`, exercise
  meaningful implemented behavior, and print exactly `IMAGEPLAYGROUND_AGENT_RUNTIME_OK` on success.
