...
func testChartDescriptor() {
    ...
    precondition(chart.contentFrame.origin.x == 0 && chart.contentFrame.origin.y == 0 && chart.contentFrame.size.width == 0 && chart.contentFrame.size.height == 0)
    chart.contentDirection = .topToBottom
    chart.contentFrame = CGRect(origin: CGPoint(x: 1, y: 2), size: CGSize(width: 3, height: 4))
    precondition(chart.contentDirection == .topToBottom)
    precondition(chart.contentFrame.size.width == 3)
