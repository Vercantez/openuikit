# _PassKit_SwiftUI framework fan-out rules

This directory is an isolated clean-room starting point for the Linux `_PassKit_SwiftUI`
port. Work only inside `full/passkit-swiftui/`. Do not edit application
...
- Implement a real Linux module named `_PassKit_SwiftUI` and a loadable
  `lib_PassKit_SwiftUI.dylib`. Put implementation Swift files in this framework
  directory, outside `tests/`, and list each one as a repo-relative path in
  `passkit-swiftui_guest_sources.txt`.
- Create `coverage.tsv` with the exact header
  ...
  At least 3208 rows must be `implemented` or `declared` for the `leaf-full`
...
- Create a nonempty `README.md` describing what is real, fail-closed, and still
  deferred. The primary implementation file must be `_PassKit_SwiftUI.swift`.
- Create `tests/agent/_PassKit_SwiftUIRuntime.swift`. It must import `_PassKit_SwiftUI`, exercise
  meaningful implemented behavior, and print exactly `PASSKIT_SWIFTUI_AGENT_RUNTIME_OK` on success.
