# Port `LiveCommunicationKit` to Linux

Build a substantial, honest starting implementation of Apple's public `LiveCommunicationKit`
...
The immutable seed contains 289 unique public precise identifiers and
399 symbol-graph relationships. The acceptance floor is
145 nondeferred (`implemented` or `declared`) identifiers: ceil(50% of exact public symbols).
...
Deliver all of the following in `full/livecommunicationkit/`:

1. Linux Swift sources for module `LiveCommunicationKit` and `libLiveCommunicationKit.dylib`, with every
   implementation source listed in `livecommunicationkit_guest_sources.txt`.
2. Complete exact-ID `coverage.tsv` using only the five allowed statuses.
   `implemented` evidence has the exact form
   `test:full/livecommunicationkit/tests/agent/*Tests.swift#testName`; `declared` evidence has
   the exact form `source:full/livecommunicationkit/<product-source>.swift#Symbol`. Every cited
   test/function and product source/anchor must exist, and the product source
   must be listed in `livecommunicationkit_guest_sources.txt`.
3. Nonempty `LiveCommunicationKit.swift` and `README.md`, plus `oracle-questions.tsv` with
...
4. Top-level synchronous no-argument functions named `test*` in
   `tests/agent/*Tests.swift`, plus `tests/agent/LiveCommunicationKitLoadSmoke.swift` whose
   exact content is `import LiveCommunicationKit`, one blank line, and
   `let frameworkLoadSmokeMarker = "LIVECOMMUNICATIONKIT_AGENT_RUNTIME_OK"`. The sealed gate derives a runner
   from `implemented` coverage, loads `libLiveCommunicationKit.dylib`, and calls each cited
   test exactly once before accepting marker-only output. The load/marker check
   is not behavioral evidence by itself.
5. If dependencies are declared, `tests/agent/LiveCommunicationKitDependencyIdentity.swift`
