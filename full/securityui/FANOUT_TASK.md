# Port `SecurityUI` to Linux

Build a substantial, honest starting implementation of Apple's public `SecurityUI`
...
The immutable seed contains 9 unique public precise identifiers and
15 symbol-graph relationships. The acceptance floor is
8 nondeferred (`implemented` or `declared`) identifiers: ceil(80% of exact public symbols).
...
Deliver all of the following in `full/securityui/`:

1. Linux Swift sources for module `SecurityUI` and `libSecurityUI.dylib`, with every
   implementation source listed in `securityui_guest_sources.txt`.
2. Complete exact-ID `coverage.tsv` using only the five allowed statuses.
   `implemented` evidence has the exact form
   `test:full/securityui/tests/agent/*Tests.swift#testName`; `declared` evidence has
   the exact form `source:full/securityui/<product-source>.swift#Symbol`. Every cited
   test/function and product source/anchor must exist, and the product source
   must be listed in `securityui_guest_sources.txt`.
3. Nonempty `SecurityUI.swift` and `README.md`, plus `oracle-questions.tsv` with
...
4. Top-level synchronous no-argument functions named `test*` in
   `tests/agent/*Tests.swift`, plus `tests/agent/SecurityUILoadSmoke.swift` whose
   exact content is `import SecurityUI`, one blank line, and
   `let frameworkLoadSmokeMarker = "SECURITYUI_AGENT_RUNTIME_OK"`. The sealed gate derives a runner
   from `implemented` coverage, loads `libSecurityUI.dylib`, and calls each cited
   test exactly once before accepting marker-only output. The load/marker check
   is not behavioral evidence by itself.
5. If dependencies are declared, `tests/agent/SecurityUIDependencyIdentity.swift`
