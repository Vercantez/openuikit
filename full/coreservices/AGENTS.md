# CoreServices framework fan-out rules

This directory is an isolated clean-room starting point for the Linux `CoreServices`
port. Work only inside `full/coreservices/`. Do not edit application
...
- Implement a real Linux module named `CoreServices` and a loadable
  `libCoreServices.dylib`. Put implementation Swift files in this framework
  directory, outside `tests/`, and list each one as a repo-relative path in
  `coreservices_guest_sources.txt`.
- Create `coverage.tsv` with the exact header
  ...
  At least 120 rows must be `implemented` or `declared` for the `legacy-adapter`
...
- Create a nonempty `README.md` describing what is real, fail-closed, and still
  deferred. The primary implementation file must be `CoreServices.swift`.
- Create `tests/agent/CoreServicesRuntime.swift`. It must import `CoreServices`, exercise
  meaningful implemented behavior, and print exactly `CORESERVICES_AGENT_RUNTIME_OK` on success.
