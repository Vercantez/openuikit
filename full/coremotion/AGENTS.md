# CoreMotion framework fan-out rules

This directory is an isolated clean-room starting point for the Linux `CoreMotion`
port. Work only inside `full/coremotion/`. Do not edit application
...
- Implement a real Linux module named `CoreMotion` and a loadable
  `libCoreMotion.dylib`. Put implementation Swift files in this framework
  directory, outside `tests/`, and list each one as a repo-relative path in
  `coremotion_guest_sources.txt`.
- Create `coverage.tsv` with the exact header
  ...
  At least 218 rows must be `implemented` or `declared` for the `medium-full`
...
- Create a nonempty `README.md` describing what is real, fail-closed, and still
  deferred. The primary implementation file must be `CoreMotion.swift`.
- Create `tests/agent/CoreMotionRuntime.swift`. It must import `CoreMotion`, exercise
  meaningful implemented behavior, and print exactly `COREMOTION_AGENT_RUNTIME_OK` on success.
