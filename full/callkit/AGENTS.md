# CallKit framework fan-out rules

This directory is an isolated clean-room starting point for the Linux `CallKit`
port. Work only inside `full/callkit/`. Do not edit application
...
- Implement a real Linux module named `CallKit` and a loadable
  `libCallKit.dylib`. Put implementation Swift files in this framework
  directory, outside `tests/`, and list each one as a repo-relative path in
  `callkit_guest_sources.txt`.
...
- Create a nonempty `README.md` describing what is real, fail-closed, and still
  deferred. The primary implementation file must be `CallKit.swift`.
- Create `tests/agent/CallKitRuntime.swift`. It must import `CallKit`, exercise
  meaningful implemented behavior, and print exactly `CALLKIT_AGENT_RUNTIME_OK` on success.
