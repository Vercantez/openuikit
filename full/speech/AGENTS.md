# Speech framework fan-out rules

This directory is an isolated clean-room starting point for the Linux `Speech`
port. Work only inside `full/speech/`. Do not edit application
...
- Implement a real Linux module named `Speech` and a loadable
  `libSpeech.dylib`. Put implementation Swift files in this framework
  directory, outside `tests/`, and list each one as a repo-relative path in
  `speech_guest_sources.txt`.
- Create `coverage.tsv` with the exact header
  ...
  At least 301 rows must be `implemented` or `declared` for the `medium-full`
...
- Create a nonempty `README.md` describing what is real, fail-closed, and still
  deferred. The primary implementation file must be `Speech.swift`.
- Create `tests/agent/SpeechRuntime.swift`. It must import `Speech`, exercise
  meaningful implemented behavior, and print exactly `SPEECH_AGENT_RUNTIME_OK` on success.
