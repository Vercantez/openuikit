# Vision framework fan-out rules

This directory is an isolated clean-room starting point for the Linux `Vision`
port. Work only inside `full/vision/`. Do not edit application
...
- Implement a real Linux module named `Vision` and a loadable
  `libVision.dylib`. Put implementation Swift files in this framework
  directory, outside `tests/`, and list each one as a repo-relative path in
  `vision_guest_sources.txt`.
...
- Create a nonempty `README.md` describing what is real, fail-closed, and still
  deferred. The primary implementation file must be `Vision.swift`.
- Create `tests/agent/VisionRuntime.swift`. It must import `Vision`, exercise
  meaningful implemented behavior, and print exactly `VISION_AGENT_RUNTIME_OK` on success.
