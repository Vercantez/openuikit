...
- All 58 `deferred` rows are `VNVideoProcessor` / cadence /
  ...
  video-processor policy they stay deferred.
- Sealed gate re-run on this macOS host (Apple Swift 6.2.1,
  arm64-apple-macosx) is RED at baseline with the same documented
  `#if !canImport(...)` lookalike host/SDK mismatch
  (`VisionRequest.swift: cannot find type 'CMSampleBuffer' in scope`). No
  Linux Swift toolchain is available on this host to re-run the Linux gate;
  no file was changed by this audit, so the last Linux-green ledger stands
  unaltered.

## Depth pass 2026-09 (pi-wave10 re-audit, no gain)

Campaign `pi-wave10`, framework `Vision` (3584 IDs). Work stays inside
`full/vision/`. Recounted `coverage.tsv` from scratch, re-checked every
non-implemented row for a convertible sync path, re-checked the overlay
OVERRIDE trigger.
...
- All 58 `deferred` rows are `VNVideoProcessor` / cadence /
  ...
  they stay deferred.
- Every `implemented` row cites a well-formed
  `test:full/vision/tests/agent/<File>Tests.swift#testName` anchor; all
  cited test files exist (118 distinct anchors; largest
...
## Depth pass 2026-09 (pi-wave12 re-audit, no gain)

Campaign `pi-wave12`, framework `Vision` (3584 IDs). Work stays inside
`full/vision/`. Recounted `coverage.tsv` from scratch, re-checked every
non-implemented row for a convertible sync path, re-checked the overlay
OVERRIDE trigger, re-validated every implemented evidence anchor.
...
- Overlay OVERRIDE inapplicable: **0** precise IDs mention `View` or
  ...
  identity view modifiers). No `not-applicable` protocol-witness
  conversions apply.
- All 214 `declared` rows contain the async marker (verified: zero declared
  rows lack `YaK`; 210 end `YaKF`, 4 end `YaK` — all are async
  `ImageProcessingRequest` `perform(on:orientation:)` witnesses /
  synthesized conformer occurrences plus async `ImageRequestHandler` /
  `TargetedImageRequestHandler` `perform` overloads). No sync path exists
  for these exact mangled IDs, and the sealed synchronous-test rule forbids
  `await`, semaphores, and run-loop waits in cited tests, so they stay
  declared.
- All 58 `deferred` rows are `VNVideoProcessor` / cadence /
  processing-options (40) and `VNCoreMLFeatureValueObservation` (18): no
  Linux AVFoundation video pipeline, no Apple Core ML runtime. Per-contract
...
- Every `implemented` row cites a well-formed
  `test:full/vision/tests/agent/<File>Tests.swift#testName` anchor (0
  malformed; 0 missing files; 0 missing/ambiguous test funcs; 118 distinct
  anchors; largest `testOverlayRevisionComparableOperators` at 319 rows,
  9.63%, below the 40% limit). No product, coverage, manifest, or test file
  was changed by this audit apart from this README record, so the last
