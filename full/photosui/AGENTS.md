# PhotosUI framework fan-out rules

This directory is an isolated clean-room starting point for the Linux `PhotosUI`
port. Work only inside `full/photosui/`. Do not edit application
...
- Implement a real Linux module named `PhotosUI` and a loadable
  `libPhotosUI.dylib`. Put implementation Swift files in this framework
  directory, outside `tests/`, and list each one as a repo-relative path in
  `photosui_guest_sources.txt`.
- Create `coverage.tsv` with the exact header
  ...
  At least 519 rows must be `implemented` or `declared` for the `medium-full`
  ...
  `test:full/photosui/tests/agent/*Tests.swift#testName`; define that exact `test*`
  function for the sealed runner to call. Every `declared` row must cite
  `source:full/photosui/<product-source>.swift#Symbol`; the source must be listed in
  `photosui_guest_sources.txt` and contain the exact identifier anchor. Deferred,
...
- Create a nonempty `README.md` describing what is real, fail-closed, and still
  deferred. The primary implementation file must be `PhotosUI.swift`.
- Put focused behavioral checks in `tests/agent/*Tests.swift` as top-level,
  synchronous, no-argument functions named `test*`. Create
  `tests/agent/PhotosUILoadSmoke.swift` with exactly these three logical lines
  (including the blank line): `import PhotosUI`, a blank line, and
  `let frameworkLoadSmokeMarker = "PHOTOSUI_AGENT_RUNTIME_OK"`. The sealed gate generates the
...
- When dependencies are listed below, create
  `tests/agent/PhotosUIDependencyIdentity.swift`. Import `PhotosUI` and every
  declared dependency and pass genuine dependency values through public
  `PhotosUI` APIs. This probe is for the clean EC2 integration build; the isolated
