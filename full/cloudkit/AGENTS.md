# CloudKit framework fan-out rules

This directory is an isolated clean-room starting point for the Linux `CloudKit`
port. Work only inside `full/cloudkit/`. Do not edit application
...
- Implement a real Linux module named `CloudKit` and a loadable
  `libCloudKit.dylib`. Put implementation Swift files in this framework
  directory, outside `tests/`, and list each one as a repo-relative path in
  `cloudkit_guest_sources.txt`.
...
- Create a nonempty `README.md` describing what is real, fail-closed, and still
  deferred. The primary implementation file must be `CloudKit.swift`.
- Create `tests/agent/CloudKitRuntime.swift`. It must import `CloudKit`, exercise
  meaningful implemented behavior, and print exactly `CLOUDKIT_AGENT_RUNTIME_OK` on success.
