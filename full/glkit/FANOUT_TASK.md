...
Build a substantial, honest starting implementation of Apple's public `GLKit`
...
The immutable seed contains 550 unique public precise identifiers and
525 symbol-graph relationships. The acceptance floor is
440 nondeferred (`implemented` or `declared`) identifiers: ceil(80% of exact public symbols).
...
Deliver all of the following in `full/glkit/`:

1. Linux Swift sources for module `GLKit` and `libGLKit.dylib`, with every
   implementation source listed in `glkit_guest_sources.txt`.
...
3. Nonempty `GLKit.swift` and `README.md`, plus `oracle-questions.tsv` with
   header `precise	question	risk	reason` and at least one question; use
   `module` only for cross-cutting items.
4. `tests/agent/GLKitRuntime.swift`, meaningful focused tests, and the exact
   success marker `GLKIT_AGENT_RUNTIME_OK`.
