# GLKit framework fan-out rules

This directory is an isolated clean-room starting point for the Linux `GLKit`
port. Work only inside `full/glkit/`. Do not edit application
...
- Implement a real Linux module named `GLKit` and a loadable
  `libGLKit.dylib`. Put implementation Swift files in this framework
  directory, outside `tests/`, and list each one as a repo-relative path in
  `glkit_guest_sources.txt`.
- Create `coverage.tsv` with the exact header
  ...
  At least 440 rows must be `implemented` or `declared` for the `legacy-adapter`
...
- Create a nonempty `README.md` describing what is real, fail-closed, and still
  deferred. The primary implementation file must be `GLKit.swift`.
- Create `tests/agent/GLKitRuntime.swift`. It must import `GLKit`, exercise
  meaningful implemented behavior, and print exactly `GLKIT_AGENT_RUNTIME_OK` on success.
