# MetricKit framework fan-out rules

This directory is an isolated clean-room starting point for the Linux `MetricKit`
port. Work only inside `full/metrickit/`. Do not edit application
...
- Implement a real Linux module named `MetricKit` and a loadable
  `libMetricKit.dylib`. Put implementation Swift files in this framework
  directory, outside `tests/`, and list each one as a repo-relative path in
  `metrickit_guest_sources.txt`.
- Create `coverage.tsv` with the exact header
  ...
  At least 130 rows must be `implemented` or `declared` for the `medium-full`
...
- Create a nonempty `README.md` describing what is real, fail-closed, and still
  deferred. The primary implementation file must be `MetricKit.swift`.
- Create `tests/agent/MetricKitRuntime.swift`. It must import `MetricKit`, exercise
  meaningful implemented behavior, and print exactly `METRICKIT_AGENT_RUNTIME_OK` on success.
