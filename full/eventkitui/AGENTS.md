# EventKitUI framework fan-out rules

This directory is an isolated clean-room starting point for the Linux `EventKitUI`
port. Work only inside `full/eventkitui/`. Do not edit application
...
- Implement a real Linux module named `EventKitUI` and a loadable
  `libEventKitUI.dylib`. Put implementation Swift files in this framework
  directory, outside `tests/`, and list each one as a repo-relative path in
  `eventkitui_guest_sources.txt`.
- Create `coverage.tsv` with the exact header
  ...
  At least 31 rows must be `implemented` or `declared` for the `medium-full`
...
- Create a nonempty `README.md` describing what is real, fail-closed, and still
  deferred. The primary implementation file must be `EventKitUI.swift`.
- Create `tests/agent/EventKitUIRuntime.swift`. It must import `EventKitUI`, exercise
  meaningful implemented behavior, and print exactly `EVENTKITUI_AGENT_RUNTIME_OK` on success.
