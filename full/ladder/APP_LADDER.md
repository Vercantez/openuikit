...
Rows regenerated 2026-09-10 (see the dated paragraph below): blocking counts and names from `gap-classes-nextrungs-2026-09-10.json`, route/score from `ladder-scores-nextrungs-2026-09-10.json`, both computed from `ladder-census-nextrungs-2026-09-10.json` against `openuikit_types-2026-09-10b.txt` (437 names, `uikit/Sources/OpenUIKit` at main `481c0f81`). A "(was: …, landed …)" note names the previous row's top names that were already declared when the row was written, with the commit that declared each.
...
| simplenote-ios | NEAR / 7 | 0 / 0 | none | No blocking UIKit row (remaining 6 uses are stubbable: `UITextInputPasswordRules` 3, `UIViewControllerRestoration` 2, `UISimpleTextPrintFormatter` 1). Next: measure Simperium and other unmeasured dependencies; validate CoreData sync/persistence against app demand; verify the mixed ObjC/nib boundary (22.3% ObjC, 26 IBOutlet files). DEP=? remains a hole. |
| Signal-iOS | MID / 10 | 4 / 8 | `UITab` 3 · `UINavigationBarDelegate` 2 · `NSIndexPath` 2 | (was: `UICoordinateSpace` 13, landed `1244733d`; `UIScrollEdgeElementContainerInteraction` 12, landed `47ea334c`; `UICollectionViewLayoutInvalidationContext` 6, landed `c781adea`.) The fourth and last blocking row is `UICornerConfiguration` 1; the one stub-able row is `UILocalizedIndexedCollation` 4. Next: the iOS 18 `UITab` model, the navigation-bar delegate, `NSIndexPath`; networking remains NET=3 and SIZE=3. |
| firefox-ios | MID / 10 | 2 / 3 | `UIMenuBuilder` 2 · `UICommandAlternate` 1 | (was: `UISwipeGestureRecognizer` 10, landed `a1c0c6b9`; `UIToolbarDelegate` 2 and `NSCollectionLayoutAnchor` 2, landed `8427d5a0`; `UITextItem` 1, landed `16068eff` while this regeneration was in flight.) 24 of the 27 remaining missing uses are stub-able (`UILargeContentViewerInteraction` 11 the largest). Next: menu builder / command alternates; a browser engine remains outside the declaration score. |
| NetNewsWire | NEAR / 9 under the iOS target scope (§9.8; the whole-repo row in §9.2 stays MID / 11) | 0 / 0 | none | (was: `NSToolbarItem` 48 — Mac-target demand, excluded by the iOS scope rather than declared; `UISplitViewController` 16, landed `ca711a5e`; `UICollectionViewController` 2, landed `c781adea`.) No blocking UIKit row in phone scope; UIK 1→0 and UI 2→1 under the scope move the score below the NEAR line. DEP=? still needs dependency measurement (Zip, Tidemark, 17 local packages); 175 `#selector` sites in scope keep SEL=3. |
| ios-oss | MID / 11 | 0 / 0 | none | (was: `UICollectionViewController` 1, landed `c781adea`.) No blocking UIKit row remains; the one remaining missing use is a stubbable `UITableViewDataSourcePrefetching`. Next: compile the load-bearing Apollo/ReactiveSwift boundary and actual nib-backed flows. |
| Telegram-iOS | MID / 11 | 19 / 56 | `NSTextLocation` 6 · `NSTextLayoutFragment` 6 · `NSIndexPath` 5 | (was: `UIEditMenuInteraction` 7, landed `50c5ffcf`; score 12 → 11 because the genuine gap is 97 uses after FREE/OOS, below the UIK=2 line of 100.) 41 of the 97 are stub-able (`UIFindSession` 10 the largest). Next: TextKit-2 layout (`NSTextLayoutManager` 5, `NSTextRange` 4, `NSTextContentStorage` 3); whole repository size/build/dependencies still dominate. |
| WordPress-iOS | MID / 13 | 3 / 4 | `NSTextTab` 2 · `NSTextLayoutManager` 1 · `NSTextLocation` 1 | (was: `UISplitViewController` 20, landed `ca711a5e`; `UITextItem` 12 and `UIPopoverPresentationControllerSourceItem` 10, landed `16068eff` while this regeneration was in flight.) 18 of the 22 remaining missing uses are stub-able (`UIAttachmentBehavior` 3 the largest). Next: the TextKit-2 tail; retain NET=3 and SIZE=3. |

Shared remaining reach, recomputed over the 20-app union (`uikit-union-2026-09-16.json` demand, the same classifier rules, the 437-name list): **48 blocking types / 230 uses** (66 / 487 under the 411-name list; 50 / 262 under the 435-name list). By app-reach: `NSIndexPath` 5 apps/10 uses; `NSTextLocation` 3/8; `NSTextLayoutManager` 3/7; `UIMenuBuilder` 3/6; `NSToolbarItem` 2/85 (whole-repo, Mac-shaped); `NSTextContentStorage` 2/9. The four names this paragraph used to lead with — `UISwipeGestureRecognizer` 10/54, `UICollectionViewController` 7/10, `UISplitViewController` 5/49, `UIEditMenuInteraction` 5/24 — are all declared now, and so is `UITextItem` 3/21 since `16068eff`. The next UIKit tail is TextKit 2, and it is small. The zero/few-row NEAR and ios-oss cases are a reason to measure actual target builds and services next, not to keep expanding the type alphabet without evidence.

**Regenerated 2026-09-10 — why the previous rows were wrong, and what was run.** The rows above replaced rows whose four "top blocking" entries for Signal-iOS, firefox-ios, ios-oss and NetNewsWire/WordPress-iOS named types that were already declared in `uikit/Sources/OpenUIKit` when the rows were written. Root cause (found by `uikit/docs/agent_reports/netnewswire-ios-target.md`): the committed `openuikit_types-2026-09-16.txt` has 411 names, but `ladder-census-2026-09-16.json` records `_meta.ours = 427` from a 2026-09-07 regeneration whose list was never committed. The prose was computed against the 411 list and the committed JSONs against the 427 list, so `gap-classes-2026-09-16.json` already disagreed with this table on six rows before any of this work (Signal-iOS 14/57 vs 6/21, firefox-ios 6/18 vs 5/8, NetNewsWire 6/71 vs 2/50, ios-oss 1/1 vs 0/0, Telegram-iOS 26/78 vs 19/56, WordPress-iOS 9/51 vs 5/26). The regeneration: the type list was regenerated with the `remeasure-2026-09-16.sh` generator (same regex, `uikit/Sources/OpenUIKit`), first at main `a7f94380` (`uikit/Sources` identical to `5b2a7364`), giving 435 names byte-identical to the committed `openuikit_types-2026-09-10.txt` — 24 names over the 411 list, none removed; main then advanced during the run (`16068eff`, merged as `d8dca8ad`/`481c0f81`, declares `UITextItem` and `UIPopoverPresentationControllerSourceItem`), so the list was regenerated again at `481c0f81` — 437 names, committed as `openuikit_types-2026-09-10b.txt`, and every row above is from that list (the 435-list pass differed only in firefox-ios 3 / 4 and WordPress-iOS 5 / 26). Each of the nine apps was censused as its own bounded run from a one-symlink mini-corpus at its pinned HEAD (all nine equal `corpus-pins-2026-09-16.tsv`; simplenote-ios from a clean detached `--shared` clone because the shared checkout carries an ignored `BuildTools/.build/`), NetNewsWire under `--target-scope=target-scope-netnewswire-ios-2026-09-10.json`; the per-app outputs were concatenated into `ladder-census-nextrungs-2026-09-10.json` and pushed through the unchanged `classify_gaps.py` and `score_ladder.py` with the 09-16 imports / nibdeps / dep-classes inputs, which are invariant (for the eight whole-repo apps every non-UIKit census field and the UIKit uses / distinct-type totals equal `ladder-census-2026-09-16.json`). Reproducibility: firefox-ios and WordPress-iOS were run twice against the 437 list (and firefox-ios and NetNewsWire against the 435 list) and the census and gap JSONs are byte-identical between runs (`SHA256SUMS-nextrungs-2026-09-10.txt`). Two caveats: the `_union` block inside `gap-classes-nextrungs-2026-09-10.json` reads the union file's `ours` flag from the 09-07 run and is not used here (the union numbers in the paragraph above were recomputed against the 437 list); and §9.2 is left as the whole-repo 411-list record — under the 437 list only Telegram-iOS's score there would change (12 → 11), and NetNewsWire's 9 is a scoped score, not a whole-repo one. No other line in this file is a direct sum of this table, so no other count was changed; §9.8's closing sentence was updated because this table now carries the scoped NetNewsWire row. Report: `uikit/docs/agent_reports/ladder-table-regen.md`.
...
Only NetNewsWire is scoped; every other row in §9.2–9.5 is still whole-repo. Since the 2026-09-10 regeneration, §9.6's NetNewsWire row is the scoped one (NEAR / 9, 0 / 0) and its other eight rows are whole-repo.
