# MediaPlayer framework fan-out rules

This directory is an isolated clean-room starting point for the Linux `MediaPlayer`
port. Work only inside `full/mediaplayer/`. Do not edit application
...
- Implement a real Linux module named `MediaPlayer` and a loadable
  `libMediaPlayer.dylib`. Put implementation Swift files in this framework
  directory, outside `tests/`, and list each one as a repo-relative path in
  `mediaplayer_guest_sources.txt`.
- Create `coverage.tsv` with the exact header
  ...
  At least 90 rows must be `implemented` or `declared` for the `large-partitioned`
...
- Create a nonempty `README.md` describing what is real, fail-closed, and still
  deferred. The primary implementation file must be `MediaPlayer.swift`.
- Create `tests/agent/MediaPlayerRuntime.swift`. It must import `MediaPlayer`, exercise
  meaningful implemented behavior, and print exactly `MEDIAPLAYER_AGENT_RUNTIME_OK` on success.
