# CoreTelephony framework fan-out rules

This directory is an isolated clean-room starting point for the Linux `CoreTelephony`
port. Work only inside `full/coretelephony/`. Do not edit application
...
- Implement a real Linux module named `CoreTelephony` and a loadable
  `libCoreTelephony.dylib`. Put implementation Swift files in this framework
  directory, outside `tests/`, and list each one as a repo-relative path in
  `coretelephony_guest_sources.txt`.
- Create `coverage.tsv` with the exact header
  ...
  At least 90 rows must be `implemented` or `declared` for the `leaf-full`
...
- Create a nonempty `README.md` describing what is real, fail-closed, and still
  deferred. The primary implementation file must be `CoreTelephony.swift`.
- Create `tests/agent/CoreTelephonyRuntime.swift`. It must import `CoreTelephony`, exercise
  meaningful implemented behavior, and print exactly `CORETELEPHONY_AGENT_RUNTIME_OK` on success.
