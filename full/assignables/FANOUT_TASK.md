# Port `Assignables` to Linux

Build a substantial, honest starting implementation of Apple's public `Assignables`
...
The immutable seed contains 1886 unique public precise identifiers and
2032 symbol-graph relationships. The acceptance floor is
943 nondeferred (`implemented` or `declared`) identifiers: ceil(50% of exact public symbols).
...
Deliver all of the following in `full/assignables/`:

1. Linux Swift sources for module `Assignables` and `libAssignables.dylib`, with every
   implementation source listed in `assignables_guest_sources.txt`.
2. Complete exact-ID `coverage.tsv` using only the five allowed statuses.
   `implemented` evidence has the exact form
   `test:full/assignables/tests/agent/*Tests.swift#testName`; `declared` evidence has
   the exact form `source:full/assignables/<product-source>.swift#Symbol`. Every cited
   test/function and product source/anchor must exist, and the product source
   must be listed in `assignables_guest_sources.txt`.
3. Nonempty `Assignables.swift` and `README.md`, plus `oracle-questions.tsv` with
...
4. Top-level synchronous no-argument functions named `test*` in
   `tests/agent/*Tests.swift`, plus `tests/agent/AssignablesLoadSmoke.swift` whose
   exact content is `import Assignables`, one blank line, and
   `let frameworkLoadSmokeMarker = "ASSIGNABLES_AGENT_RUNTIME_OK"`. The sealed gate derives a runner
   from `implemented` coverage, loads `libAssignables.dylib`, and calls each cited
   test exactly once before accepting marker-only output. The load/marker check
   is not behavioral evidence by itself.
5. If dependencies are declared, `tests/agent/AssignablesDependencyIdentity.swift`
