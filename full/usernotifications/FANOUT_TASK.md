# Port `UserNotifications` to Linux

Build a substantial, honest starting implementation of Apple's public `UserNotifications`
...
The immutable seed contains 375 unique public precise identifiers and
645 symbol-graph relationships. The acceptance floor is
188 nondeferred (`implemented` or `declared`) identifiers: ceil(50% of exact public symbols).
...
Deliver all of the following in `full/usernotifications/`:

1. Linux Swift sources for module `UserNotifications` and `libUserNotifications.dylib`, with every
   implementation source listed in `usernotifications_guest_sources.txt`.
2. Complete exact-ID `coverage.tsv` using only the five allowed statuses.
   `implemented` evidence has the exact form
   `test:full/usernotifications/tests/agent/*Tests.swift#testName`; `declared` evidence has
   the exact form `source:full/usernotifications/<product-source>.swift#Symbol`. Every cited
   test/function and product source/anchor must exist, and the product source
   must be listed in `usernotifications_guest_sources.txt`.
3. Nonempty `UserNotifications.swift` and `README.md`, plus `oracle-questions.tsv` with
...
4. Top-level synchronous no-argument functions named `test*` in
   `tests/agent/*Tests.swift`, plus `tests/agent/UserNotificationsLoadSmoke.swift` whose
   exact content is `import UserNotifications`, one blank line, and
   `let frameworkLoadSmokeMarker = "USERNOTIFICATIONS_AGENT_RUNTIME_OK"`. The sealed gate derives a runner
   from `implemented` coverage, loads `libUserNotifications.dylib`, and calls each cited
   test exactly once before accepting marker-only output. The load/marker check
   is not behavioral evidence by itself.
5. If dependencies are declared, `tests/agent/UserNotificationsDependencyIdentity.swift`
