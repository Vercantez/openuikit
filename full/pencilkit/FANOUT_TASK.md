# Port `PencilKit` to Linux

Build a substantial, honest starting implementation of Apple's public `PencilKit`
...
The immutable seed contains 443 unique public precise identifiers and
676 symbol-graph relationships. The acceptance floor is
355 nondeferred (`implemented` or `declared`) identifiers: ceil(80% of exact public symbols).
...
Deliver all of the following in `full/pencilkit/`:

1. Linux Swift sources for module `PencilKit` and `libPencilKit.dylib`, with every
   implementation source listed in `pencilkit_guest_sources.txt`.
2. Complete exact-ID `coverage.tsv` using only the five allowed statuses.
   `implemented` evidence has the exact form
   `test:full/pencilkit/tests/agent/*Tests.swift#testName`; `declared` evidence has
   the exact form `source:full/pencilkit/<product-source>.swift#Symbol`. Every cited
   test/function and product source/anchor must exist, and the product source
   must be listed in `pencilkit_guest_sources.txt`.
3. Nonempty `PencilKit.swift` and `README.md`, plus `oracle-questions.tsv` with
...
4. Top-level synchronous no-argument functions named `test*` in
   `tests/agent/*Tests.swift`, plus `tests/agent/PencilKitLoadSmoke.swift` whose
   exact content is `import PencilKit`, one blank line, and
   `let frameworkLoadSmokeMarker = "PENCILKIT_AGENT_RUNTIME_OK"`. The sealed gate derives a runner
   from `implemented` coverage, loads `libPencilKit.dylib`, and calls each cited
   test exactly once before accepting marker-only output. The load/marker check
   is not behavioral evidence by itself.
5. If dependencies are declared, `tests/agent/PencilKitDependencyIdentity.swift`
