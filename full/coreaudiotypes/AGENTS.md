# CoreAudioTypes framework fan-out rules

This directory is an isolated clean-room starting point for the Linux `CoreAudioTypes`
port. Work only inside `full/coreaudiotypes/`. Do not edit application
...
- Implement a real Linux module named `CoreAudioTypes` and a loadable
  `libCoreAudioTypes.dylib`. Put implementation Swift files in this framework
  directory, outside `tests/`, and list each one as a repo-relative path in
  `coreaudiotypes_guest_sources.txt`.
- Create `coverage.tsv` with the exact header
  ...
  At least 349 rows must be `implemented` or `declared` for the `medium-full`
...
- Create a nonempty `README.md` describing what is real, fail-closed, and still
  deferred. The primary implementation file must be `CoreAudioTypes.swift`.
- Create `tests/agent/CoreAudioTypesRuntime.swift`. It must import `CoreAudioTypes`, exercise
  meaningful implemented behavior, and print exactly `COREAUDIOTYPES_AGENT_RUNTIME_OK` on success.
