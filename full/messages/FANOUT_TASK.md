# Port `Messages` to Linux

Build a substantial, honest starting implementation of Apple's public `Messages`
...
388 symbol-graph relationships. The acceptance floor is
...
Deliver all of the following in `full/messages/`:

1. Linux Swift sources for module `Messages` and `libMessages.dylib`, with every
   implementation source listed in `messages_guest_sources.txt`.
2. Complete exact-ID `coverage.tsv` using only the five allowed statuses.
   `implemented` evidence has the exact form
   `test:full/messages/tests/agent/*Tests.swift#testName`; `declared` evidence has
   the exact form `source:full/messages/<product-source>.swift#Symbol`. Every cited
   test/function and product source/anchor must exist, and the product source
   must be listed in `messages_guest_sources.txt`.
3. Nonempty `Messages.swift` and `README.md`, plus `oracle-questions.tsv` with
...
4. Top-level synchronous no-argument functions named `test*` in
   `tests/agent/*Tests.swift`, plus `tests/agent/MessagesLoadSmoke.swift` whose
   exact content is `import Messages`, one blank line, and
   `let frameworkLoadSmokeMarker = "MESSAGES_AGENT_RUNTIME_OK"`. The sealed gate derives a runner
   from `implemented` coverage, loads `libMessages.dylib`, and calls each cited
   test exactly once before accepting marker-only output. The load/marker check
   is not behavioral evidence by itself.
5. If dependencies are declared, `tests/agent/MessagesDependencyIdentity.swift`
