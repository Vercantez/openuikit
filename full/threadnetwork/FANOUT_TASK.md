...
2. Complete exact-ID `coverage.tsv` using only the five allowed statuses.
   ...
   `test:full/threadnetwork/tests/agent/*Tests.swift#testName`; `declared` evidence has
   the exact form `source:full/threadnetwork/<product-source>.swift#Symbol`. Every cited
   test/function and product source/anchor must exist, and the product source
   must be listed in `threadnetwork_guest_sources.txt`.
...
4. Top-level synchronous no-argument functions named `test*` in
   `tests/agent/*Tests.swift`, plus `tests/agent/ThreadNetworkLoadSmoke.swift` whose
   exact content is `import ThreadNetwork`, one blank line, and
   `let frameworkLoadSmokeMarker = "THREADNETWORK_AGENT_RUNTIME_OK"`. The sealed gate derives a runner
   from `implemented` coverage, loads `libThreadNetwork.dylib`, and calls each cited
   test exactly once before accepting marker-only output. The load/marker check
   is not behavioral evidence by itself.
5. If dependencies are declared, `tests/agent/ThreadNetworkDependencyIdentity.swift`
