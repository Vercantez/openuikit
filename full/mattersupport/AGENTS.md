# MatterSupport framework fan-out rules

This directory is an isolated clean-room starting point for the Linux `MatterSupport`
port. Work only inside `full/mattersupport/`. Do not edit application
...
- Implement a real Linux module named `MatterSupport` and a loadable
  `libMatterSupport.dylib`. Put implementation Swift files in this framework
  directory, outside `tests/`, and list each one as a repo-relative path in
  `mattersupport_guest_sources.txt`.
- Create `coverage.tsv` with the exact header
  ...
  At least 100 rows must be `implemented` or `declared` for the `leaf-full`
...
- Create a nonempty `README.md` describing what is real, fail-closed, and still
  deferred. The primary implementation file must be `MatterSupport.swift`.
- Create `tests/agent/MatterSupportRuntime.swift`. It must import `MatterSupport`, exercise
  meaningful implemented behavior, and print exactly `MATTERSUPPORT_AGENT_RUNTIME_OK` on success.
