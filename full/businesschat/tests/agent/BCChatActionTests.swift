...
func testChatActionClass() {
    let typeName = String(reflecting: BCChatAction.self)
    precondition(typeName == "BusinessChat.BCChatAction")
    let superCls: AnyClass? = BCChatAction.superclass
    precondition(superCls == NSObject.self)
}
