# ExternalAccessory framework fan-out rules

This directory is an isolated clean-room starting point for the Linux `ExternalAccessory`
port. Work only inside `full/externalaccessory/`. Do not edit application
...
- Implement a real Linux module named `ExternalAccessory` and a loadable
  `libExternalAccessory.dylib`. Put implementation Swift files in this framework
  directory, outside `tests/`, and list each one as a repo-relative path in
  `externalaccessory_guest_sources.txt`.
- Create `coverage.tsv` with the exact header
  ...
  At least 62 rows must be `implemented` or `declared` for the `medium-full`
  ...
  `test:full/externalaccessory/tests/agent/*Tests.swift#testName`; define that exact `test*`
  function for the sealed runner to call. Every `declared` row must cite
  `source:full/externalaccessory/<product-source>.swift#Symbol`; the source must be listed in
  `externalaccessory_guest_sources.txt` and contain the exact identifier anchor. Deferred,
...
- Create a nonempty `README.md` describing what is real, fail-closed, and still
  deferred. The primary implementation file must be `ExternalAccessory.swift`.
- Put focused behavioral checks in `tests/agent/*Tests.swift` as top-level,
  synchronous, no-argument functions named `test*`. Create
  `tests/agent/ExternalAccessoryLoadSmoke.swift` with exactly these three logical lines
  (including the blank line): `import ExternalAccessory`, a blank line, and
  `let frameworkLoadSmokeMarker = "EXTERNALACCESSORY_AGENT_RUNTIME_OK"`. The sealed gate generates the
...
- When dependencies are listed below, create
  `tests/agent/ExternalAccessoryDependencyIdentity.swift`. Import `ExternalAccessory` and every
  declared dependency and pass genuine dependency values through public
  `ExternalAccessory` APIs. This probe is for the clean EC2 integration build; the isolated
