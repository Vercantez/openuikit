# Port `PushKit` to Linux

Build a substantial, honest starting implementation of Apple's public `PushKit`
...
The immutable seed contains 24 unique public precise identifiers and
47 symbol-graph relationships. The acceptance floor is
20 nondeferred (`implemented` or `declared`) identifiers: ceil(80% of exact public symbols).
...
Deliver all of the following in `full/pushkit/`:

1. Linux Swift sources for module `PushKit` and `libPushKit.dylib`, with every
   implementation source listed in `pushkit_guest_sources.txt`.
...
3. Nonempty `PushKit.swift` and `README.md`, plus `oracle-questions.tsv` with
   header `precise	question	risk	reason` and at least one question; use
   `module` only for cross-cutting items.
4. `tests/agent/PushKitRuntime.swift`, meaningful focused tests, and the exact
   success marker `PUSHKIT_AGENT_RUNTIME_OK`.
