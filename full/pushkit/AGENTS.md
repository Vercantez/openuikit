# PushKit framework fan-out rules

This directory is an isolated clean-room starting point for the Linux `PushKit`
port. Work only inside `full/pushkit/`. Do not edit application
...
- Implement a real Linux module named `PushKit` and a loadable
  `libPushKit.dylib`. Put implementation Swift files in this framework
  directory, outside `tests/`, and list each one as a repo-relative path in
  `pushkit_guest_sources.txt`.
- Create `coverage.tsv` with the exact header
  ...
  At least 20 rows must be `implemented` or `declared` for the `leaf-full`
...
- Create a nonempty `README.md` describing what is real, fail-closed, and still
  deferred. The primary implementation file must be `PushKit.swift`.
- Create `tests/agent/PushKitRuntime.swift`. It must import `PushKit`, exercise
  meaningful implemented behavior, and print exactly `PUSHKIT_AGENT_RUNTIME_OK` on success.
