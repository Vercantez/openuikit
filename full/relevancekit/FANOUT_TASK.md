# Port `RelevanceKit` to Linux

Build a substantial, honest starting implementation of Apple's public `RelevanceKit`
...
The immutable seed contains 29 unique public precise identifiers and
28 symbol-graph relationships. The acceptance floor is
24 nondeferred (`implemented` or `declared`) identifiers: ceil(80% of exact public symbols).
...
Deliver all of the following in `full/relevancekit/`:

1. Linux Swift sources for module `RelevanceKit` and `libRelevanceKit.dylib`, with every
   implementation source listed in `relevancekit_guest_sources.txt`.
2. Complete exact-ID `coverage.tsv` using only the five allowed statuses.
   `implemented` evidence has the exact form
   `test:full/relevancekit/tests/agent/*Tests.swift#testName`; `declared` evidence has
   the exact form `source:full/relevancekit/<product-source>.swift#Symbol`. Every cited
   test/function and product source/anchor must exist, and the product source
   must be listed in `relevancekit_guest_sources.txt`.
3. Nonempty `RelevanceKit.swift` and `README.md`, plus `oracle-questions.tsv` with
...
4. Top-level synchronous no-argument functions named `test*` in
   `tests/agent/*Tests.swift`, plus `tests/agent/RelevanceKitLoadSmoke.swift` whose
   exact content is `import RelevanceKit`, one blank line, and
   `let frameworkLoadSmokeMarker = "RELEVANCEKIT_AGENT_RUNTIME_OK"`. The sealed gate derives a runner
   from `implemented` coverage, loads `libRelevanceKit.dylib`, and calls each cited
   test exactly once before accepting marker-only output. The load/marker check
   is not behavioral evidence by itself.
5. If dependencies are declared, `tests/agent/RelevanceKitDependencyIdentity.swift`
