# NotificationCenter framework fan-out rules

This directory is an isolated clean-room starting point for the Linux `NotificationCenter`
port. Work only inside `full/notificationcenter/`. Do not edit application
...
- Implement a real Linux module named `NotificationCenter` and a loadable
  `libNotificationCenter.dylib`. Put implementation Swift files in this framework
  directory, outside `tests/`, and list each one as a repo-relative path in
  `notificationcenter_guest_sources.txt`.
- Create `coverage.tsv` with the exact header
  ...
  At least 23 rows must be `implemented` or `declared` for the `leaf-full`
...
- Create a nonempty `README.md` describing what is real, fail-closed, and still
  deferred. The primary implementation file must be `NotificationCenter.swift`.
- Create `tests/agent/NotificationCenterRuntime.swift`. It must import `NotificationCenter`, exercise
  meaningful implemented behavior, and print exactly `NOTIFICATIONCENTER_AGENT_RUNTIME_OK` on success.
