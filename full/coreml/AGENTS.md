# CoreML framework fan-out rules

This directory is an isolated clean-room starting point for the Linux `CoreML`
port. Work only inside `full/coreml/`. Do not edit application
...
- Implement a real Linux module named `CoreML` and a loadable
  `libCoreML.dylib`. Put implementation Swift files in this framework
  directory, outside `tests/`, and list each one as a repo-relative path in
  `coreml_guest_sources.txt`.
...
- Create a nonempty `README.md` describing what is real, fail-closed, and still
  deferred. The primary implementation file must be `CoreML.swift`.
- Create `tests/agent/CoreMLRuntime.swift`. It must import `CoreML`, exercise
  meaningful implemented behavior, and print exactly `COREML_AGENT_RUNTIME_OK` on success.
