# Port `DeviceDiscoveryExtension` to Linux

Build a substantial, honest starting implementation of Apple's public `DeviceDiscoveryExtension`
...
The immutable seed contains 173 unique public precise identifiers and
246 symbol-graph relationships. The acceptance floor is
139 nondeferred (`implemented` or `declared`) identifiers: ceil(80% of exact public symbols).
...
Deliver all of the following in `full/devicediscoveryextension/`:

1. Linux Swift sources for module `DeviceDiscoveryExtension` and `libDeviceDiscoveryExtension.dylib`, with every
   implementation source listed in `devicediscoveryextension_guest_sources.txt`.
2. Complete exact-ID `coverage.tsv` using only the five allowed statuses.
   `implemented` evidence has the exact form
   `test:full/devicediscoveryextension/tests/agent/*Tests.swift#testName`; `declared` evidence has
   the exact form `source:full/devicediscoveryextension/<product-source>.swift#Symbol`. Every cited
   test/function and product source/anchor must exist, and the product source
   must be listed in `devicediscoveryextension_guest_sources.txt`.
3. Nonempty `DeviceDiscoveryExtension.swift` and `README.md`, plus `oracle-questions.tsv` with
...
4. Top-level synchronous no-argument functions named `test*` in
   `tests/agent/*Tests.swift`, plus `tests/agent/DeviceDiscoveryExtensionLoadSmoke.swift` whose
   exact content is `import DeviceDiscoveryExtension`, one blank line, and
   `let frameworkLoadSmokeMarker = "DEVICEDISCOVERYEXTENSION_AGENT_RUNTIME_OK"`. The sealed gate derives a runner
   from `implemented` coverage, loads `libDeviceDiscoveryExtension.dylib`, and calls each cited
   test exactly once before accepting marker-only output. The load/marker check
   is not behavioral evidence by itself.
5. If dependencies are declared, `tests/agent/DeviceDiscoveryExtensionDependencyIdentity.swift`
