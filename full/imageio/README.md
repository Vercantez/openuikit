...
- All 902 public precise IDs are `implemented`. The 750 `kCG*` / `kIIO*`
  CFString keys carry the Darwin payloads measured 2026-09-05 (short
  names such as `PixelWidth` / `DateTimeOriginal` / `{Exif}`; option keys
  such as `kCGImageSourceShouldCache` keep the C identifier when that is
  the runtime string). PNG filter macros are Int32 (`NONE=8`, `SUB=16`,
  `UP=32`, `AVG=64`, `PAETH=128`, `NO_FILTERS=0`).
...
- Property dictionaries parse real PNG chunks (IHDR, pHYs, tEXt, gAMA)
  ...
  tEXt Title → `{PNG}.Title` and `{IPTC}.ObjectName`.
...
- `CGImageMetadataCreateXMPData` / `CreateFromXMPData` round-trip a
  minimal RDF packet for in-memory tags. Destination types remain
  `public.png` and `com.microsoft.bmp` on the isolated host (JPEG encode
  is CQuartz-only).

## Fail-closed boundaries

- JPEG/GIF **pixel** decode and JPEG destination encode need CQuartz.
