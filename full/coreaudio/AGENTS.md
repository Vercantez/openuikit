# CoreAudio framework fan-out rules

This directory is an isolated clean-room starting point for the Linux `CoreAudio`
port. Work only inside `full/coreaudio/`. Do not edit application
...
- Implement a real Linux module named `CoreAudio` and a loadable
  `libCoreAudio.dylib`. Put implementation Swift files in this framework
  directory, outside `tests/`, and list each one as a repo-relative path in
  `coreaudio_guest_sources.txt`.
...
- Create a nonempty `README.md` describing what is real, fail-closed, and still
  deferred. The primary implementation file must be `CoreAudio.swift`.
- Create `tests/agent/CoreAudioRuntime.swift`. It must import `CoreAudio`, exercise
  meaningful implemented behavior, and print exactly `COREAUDIO_AGENT_RUNTIME_OK` on success.
