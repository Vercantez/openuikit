# Port `CoreAudio` to Linux

Build a substantial, honest starting implementation of Apple's public `CoreAudio`
...
The immutable seed contains 347 unique public precise identifiers and
...
Deliver all of the following in `full/coreaudio/`:

1. Linux Swift sources for module `CoreAudio` and `libCoreAudio.dylib`, with every
   implementation source listed in `coreaudio_guest_sources.txt`.
...
3. Nonempty `CoreAudio.swift` and `README.md`, plus `oracle-questions.tsv` with
   header `precise	question	risk	reason` and at least one question; use
   `module` only for cross-cutting items.
4. `tests/agent/CoreAudioRuntime.swift`, meaningful focused tests, and the exact
   success marker `COREAUDIO_AGENT_RUNTIME_OK`.
