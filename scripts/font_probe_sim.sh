#!/bin/zsh
# Runs Tools/oracle2/fontprobe in the iOS 26 simulator and copies
# <Documents>/font_metrics_ios.json to <outdir>. See the probe's header for what it
...
APP=Tools/oracle2/FontProbe.app
mkdir -p "$APP"
swiftc -O -target arm64-apple-ios26.0-simulator -sdk "$SDK" \
  Tools/oracle2/fontprobe/main.swift -o "$APP/fontprobe"
cp Tools/oracle2/FontProbe-Info.plist "$APP/Info.plist"
...
CONTAINER=$(xcrun simctl get_app_container "$UDID" com.openuikit.fontprobe data)
rm -f "$CONTAINER"/Documents/*.json "$CONTAINER"/Documents/DONE
xcrun simctl launch --console-pty "$UDID" com.openuikit.fontprobe >/dev/null || true
