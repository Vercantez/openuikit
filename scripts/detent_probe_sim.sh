#!/bin/zsh
# Runs Tools/oracle2/detentprobe in the iOS 26 simulator and copies
# <Documents>/detents_ios.json to <outdir>. See the probe's header for what it
...
APP=Tools/oracle2/DetentProbe.app
mkdir -p "$APP"
swiftc -O -target arm64-apple-ios26.0-simulator -sdk "$SDK" \
  Tools/oracle2/detentprobe/main.swift -o "$APP/detentprobe"
cp Tools/oracle2/DetentProbe-Info.plist "$APP/Info.plist"
...
CONTAINER=$(xcrun simctl get_app_container "$UDID" com.openuikit.detentprobe data)
rm -f "$CONTAINER"/Documents/*.json "$CONTAINER"/Documents/DONE
xcrun simctl launch --console-pty "$UDID" com.openuikit.detentprobe >/dev/null || true
