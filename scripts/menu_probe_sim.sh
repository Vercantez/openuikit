#!/bin/zsh
# Runs the iOS Simulator MENU oracle (MenuProbe) end-to-end: builds the app,
...
# <outdir>.
#
#   scripts/menu_probe_sim.sh <outdir>
#
# Same pattern as scripts/alert_probe_sim.sh. Measures the iOS UIMenu platter:
# geometry / corner radius, row heights and separators, title & item
# typography, destructive styling, and the flat-colour equivalent of the blur
# (several known base colours). Mac Catalyst turns a UIMenu into an AppKit
# NSMenu, so only real iOS can supply these numbers.
# See Tools/oracle2/menuprobe/main.swift.
set -e
setopt null_glob
cd "$(dirname "$0")/.."
OUTDIR=${1:?usage: menu_probe_sim.sh <outdir>}
mkdir -p "$OUTDIR"

SDK=$(xcrun --show-sdk-path --sdk iphonesimulator)
APP=Tools/oracle2/MenuProbe.app
mkdir -p "$APP"
swiftc -O -target arm64-apple-ios26.0-simulator -sdk "$SDK" \
  Tools/oracle2/menuprobe/main.swift \
  -o "$APP/menuprobe"
cp Tools/oracle2/MenuProbe-Info.plist "$APP/Info.plist"
echo "built $APP"
...
CONTAINER=$(xcrun simctl get_app_container "$UDID" com.openuikit.menuprobe data)
...
xcrun simctl launch --console-pty "$UDID" com.openuikit.menuprobe list >/dev/null || true
CONFIGS=$(cat "$CONTAINER/Documents/configs.txt")
echo "configurations: $CONFIGS"
for name in ${=CONFIGS}; do
  echo "  probing $name ..."
  xcrun simctl launch --console-pty "$UDID" com.openuikit.menuprobe "$name" >/dev/null || true
done
