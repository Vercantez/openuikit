...
expect_grep 'STAGE_FE_CLANG_SYSROOT' "$PHASE2" \
    "VM sibling incompleteness is CANNOT_STAGE_FE_CLANG_SYSROOT, not SYS overlays"
...
expect_grep 'phase2_ensure_selected_compile_sysroot "$SYS" "$ARM_SYS"' "$PHASE2" \
    "phase2 ensures the selected compile sysroot (sibling on VM, SYS on box) before os-module/FE/ud-guest"
...
expect_grep 'phase2_ensure_selected_compile_sysroot' "$COMMON" \
    "ensure helper stages overlay-darwin onto *-fe-clang only when select is fe-clang"
...
echo "== VM path: overlay-darwin Darwin.modulemap lands on sibling; SYS stays bare"
VMSEL=$(mktemp -d /tmp/phase2-vm-overlay-darwin.XXXXXX)
mkdir -p "$VMSEL/sys/usr/include" "$VMSEL/arm"
if [ -d "$ROOT/machorun/sdk/usr/include" ]; then
    cp -R "$ROOT/machorun/sdk/usr/include/." "$VMSEL/sys/usr/include/"
fi
rm -f "$VMSEL/sys/usr/include/Darwin.modulemap" \
    "$VMSEL/sys/usr/include/Darwin_C.modulemap" \
    "$VMSEL/sys/usr/include/DarwinFoundation1.modulemap"
[ ! -f "$VMSEL/sys/usr/include/Darwin.modulemap" ] \
    && ok "fresh-VM SYS fixture has no Darwin.modulemap (box identity)" \
    || die_test "SYS fixture still has Darwin.modulemap"
sel_fresh=$(phase2_select_fe_compile_sysroot "$VMSEL/sys" "$VMSEL/arm")
echo "$sel_fresh"
case "$sel_fresh" in
    FE_SYSROOT_SELECT=fe-clang\ reason=no-arm64-darwin-family\ sys="$VMSEL/sys-fe-clang")
        ok "fresh VM (no arm64 Darwin family) selects fe-clang reason=no-arm64-darwin-family"
        ;;
    *)
        die_test "expected fe-clang reason=no-arm64-darwin-family got: $sel_fresh"
        ;;
esac
sel_noarm=$(phase2_select_fe_compile_sysroot "$VMSEL/sys")
echo "$sel_noarm"
case "$sel_noarm" in
    FE_SYSROOT_SELECT=fe-clang\ reason=no-arm64-darwin-family\ sys="$VMSEL/sys-fe-clang")
        ok "absent arm64 sysroot also selects fe-clang reason=no-arm64-darwin-family"
        ;;
    *)
        die_test "expected no-arm64-darwin-family with no ARM arg got: $sel_noarm"
        ;;
esac
if phase2_stage_fe_clang_sysroot "$VMSEL/sys" "$ROOT"; then
    vm_fe=$(phase2_fe_clang_sysroot "$VMSEL/sys")
    if [ -f "$vm_fe/usr/include/Darwin.modulemap" ] \
        && grep -q 'GENERATED by stage_overlay_darwin.sh' "$vm_fe/usr/include/Darwin.modulemap"; then
        ok "stage_overlay_darwin wrote Darwin.modulemap onto the fe-clang sibling"
    else
        die_test "sibling missing overlay-darwin Darwin.modulemap"
    fi
    if [ -f "$VMSEL/sys/usr/include/Darwin.modulemap" ]; then
        die_test "overlay-copied SYS gained Darwin.modulemap (must stay bare on the VM)"
    else
        ok "overlay-copied SYS still has no Darwin.modulemap"
    fi
    vm_kind=fe-clang
    vm_argv=$(phase2_os_module_compile_argv_summary "$ROOT" "$vm_fe" "$vm_kind")
    echo "$vm_argv"
    case "$vm_argv" in
        sys="$vm_fe"\ posix_xcc=-Xcc\ -I*)
            ok "VM os-module argv points at fe-clang sibling (overlay-posix -I allowed)"
            ;;
        sys="$vm_fe"\ posix_xcc=none)
            ok "VM os-module argv points at fe-clang sibling"
            ;;
        *)
            die_test "VM os-module argv not pointed at sibling: $vm_argv"
            ;;
    esac
    art_if=$ROOT/swiftcore-macho/artifacts/swift-macosx/Darwin.swiftmodule/x86_64-apple-macos.swiftinterface
    if [ -f "$art_if" ]; then
        if [ -f "$vm_fe/usr/lib/swift/Darwin.swiftmodule/x86_64-apple-macos.swiftinterface" ]; then
            ok "artifact Darwin.swiftinterface landed on the fe-clang sibling"
        else
            die_test "sibling missing artifact Darwin.swiftinterface"
        fi
        if [ -e "$VMSEL/sys/usr/lib/swift/Darwin.swiftmodule" ]; then
            die_test "overlay-copied SYS gained Darwin.swiftmodule (VM overlays are sibling-only)"
        else
            ok "overlay-copied SYS has no Darwin.swiftmodule"
        fi
    fi
else
    die_test "phase2_stage_fe_clang_sysroot failed on the no-Darwin.modulemap VM fixture"
fi
rm -rf "$VMSEL"
