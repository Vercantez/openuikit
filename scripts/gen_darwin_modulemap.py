...
class Module:
    ...
    def prune(self, present, kept_externs):
        """Drop absent headers and empty children. True if anything survives."""
        if self.unbuildable:
            return False
        self.headers = [h for h in self.headers if h[1] in present]
        self.children = [c for c in self.children if c.prune(present, kept_externs)]
...
def main():
    ...
    for name in family:
        ...
        roots = [r for r in roots if r.prune(present, kept)]
